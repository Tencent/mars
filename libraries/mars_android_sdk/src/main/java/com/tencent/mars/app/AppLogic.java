...
public class AppLogic {
    ...
    public interface ICallBack {
        /**
         * 网络组件会将网络连接的实用信息进行存储，如连网IPPort策略、心跳策略等，此类信息将会被存储在客户端上层指定的目录下
         * @return APP目录
         */
        String getAppFilePath();

        /**
         * 网络组件会根据客户端用户活跃状态进行网络连接策略的动态调整，当用户非活跃时，网络会将连接的频率降低等，
        ...
        /**
         * 客户端版本号能够帮助网络组件清晰区分存储的网络策略配置文件，也能够更直观的帮助客户端查看日志跟踪问题
         * @return 客户端版本号
         */
        int getClientVersion();

        /**
         * 客户端通过获取设备类型，加入到不同的上报统计回调内容中，供客户端进行数据分析
         * @return
         */
