...
def build_android(incremental, arch, target_option='', wconan_elf=''):
    ...
    build_cmd = ANDROID_BUILD_CMD %(SCRIPT_PATH, ANDROID_GENERATOR, arch, NDK_ROOT, NDK_ROOT, wconan_elf, target_option)
...
def main(incremental, archs, target_option='', tag='', wconan_elf=''):
    ...
    for arch in archs:
        if not build_android(incremental, arch, target_option, wconan_elf=wconan_elf):
            return


def parse_arg():
    parser = argparse.ArgumentParser(description="Parse archs and output arguments")

    parser.add_argument('--archs', nargs='+', default="arm64-v8a", type=str, required=False, help='List of architectures')
    parser.add_argument('--target_option', default='', required=False, help='target_option')
    parser.add_argument('--tag', default='mars', required=False, help='tag')
    parser.add_argument('--wconan_elf', required=True, help='wconan_elf')
...
if __name__ == '__main__':
    ...
    while True:
        if len(sys.argv) >= 3:
            args = parse_arg()
            main(False, args.archs, args.target_option, args.tag, args.wconan_elf)
            break
        else:
            archs = {'arm64-v8a'}
