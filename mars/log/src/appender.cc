...
void XloggerAppender::SetMode(TAppenderMode _mode) {
    ...
    if (kAppenderAsync == mode_ && !thread_async_.isruning()) {
        thread_async_.start();
    }
}
...
void XloggerAppender::Open(TAppenderMode _mode, const char* _cachedir,
    ...
    if (!cache_logdir_.empty()) {
        boost::filesystem::create_directories(cache_logdir_);

        Thread(boost::bind(&XloggerAppender::__DelTimeoutFile, this, cache_logdir_)).start_after(2 * 60 * 1000);
        Thread(boost::bind(&XloggerAppender::__MoveOldFiles, this, cache_logdir_, logdir_, logfileprefix_)).start_after(3 * 60 * 1000);
#ifdef __APPLE__
        setAttrProtectionNone(cache_logdir_.c_str());
#endif
    }

    Thread(boost::bind(&XloggerAppender::__DelTimeoutFile, this, logdir_)).start_after(2 * 60 * 1000);
    boost::filesystem::create_directories(logdir_);
#ifdef __APPLE__
    setAttrProtectionNone(logdir_.c_str());
#endif
    ...
    snprintf(logmsg, sizeof(logmsg), "log appender mode:%d, use mmap:%d", (int)mode_, use_mmap);
    Write(nullptr, logmsg);
    
    if (!cache_logdir_.empty()) {
        boost::filesystem::space_info info = boost::filesystem::space(cache_logdir_);
    ...
    boost::filesystem::space_info info = boost::filesystem::space(logdir_);
...
long XloggerAppender::__GetNextFileIndex(const std::string& _fileprefix, const std::string& _fileext) {
    std::vector<std::string> filename_vec;
    __GetFileNamesByPrefix(logdir_, _fileprefix, _fileext, filename_vec);
    if (!cache_logdir_.empty()) {
        __GetFileNamesByPrefix(cache_logdir_, _fileprefix, _fileext, filename_vec);
    }
...
void XloggerAppender::__MoveOldFiles(const std::string& _src_path, const std::string& _dest_path,
    ...
    for (boost::filesystem::directory_iterator iter(path); iter != end_iter; ++iter) {
        ...
        if (cache_log_days_ > 0) {
            time_t file_modify_time = boost::filesystem::last_write_time(iter->path());
            if (now_time > file_modify_time && (now_time - file_modify_time) < cache_log_days_ * 24 * 60 * 60) {
                continue;
            }
        }
        
        if (!__AppendFile(iter->path().string(), logdir_ + "/" + iter->path().filename().string())) {
            break;
        }
...
bool XloggerAppender::__OpenLogFile(const std::string& _log_dir) {
    ...
    __MakeLogFileName(tv, _log_dir, logfileprefix_.c_str(), LOG_EXT, logfilepath , 1024);
...
bool XloggerAppender::__CacheLogs() {
    if (cache_logdir_.empty() || cache_log_days_ <= 0) {
        return false;
    }
    ...
    gettimeofday(&tv, nullptr);
    char logfilepath[1024] = {0};
    __MakeLogFileName(tv, logdir_, logfileprefix_.c_str(), LOG_EXT, logfilepath , 1024);
    if (boost::filesystem::exists(logfilepath)) {
        return false;
    }
...
void XloggerAppender::__Log2File(const void* _data, size_t _len, bool _move_file) {
    if (nullptr == _data || 0 == _len || logdir_.empty()) {
        return;
    }
    ...
    if (cache_logdir_.empty()) {
        if (__OpenLogFile(logdir_)) {
            __WriteFile(_data, _len, logfile_);
            if (kAppenderAsync == mode_) {
                __CloseLogFile();
            }
        }
        return;
    ...
    __MakeLogFileName(tv, cache_logdir_, logfileprefix_.c_str(), LOG_EXT, logcachefilepath , 1024);
    
    bool cache_logs = __CacheLogs();
    if ((cache_logs || boost::filesystem::exists(logcachefilepath)) && __OpenLogFile(cache_logdir_)) {
        __WriteFile(_data, _len, logfile_);
        if (kAppenderAsync == mode_) {
            __CloseLogFile();
        }
    ...
    bool open_success = __OpenLogFile(logdir_);
    if (open_success) {
    ...
    if (!write_success) {
        if (open_success && kAppenderSync == mode_) {
            __CloseLogFile();
        }

        if (__OpenLogFile(cache_logdir_)) {
            __WriteFile(_data, _len, logfile_);
            if (kAppenderAsync == mode_) {
                __CloseLogFile();
            }
...
const char* XloggerAppender::Dump(const void* _dumpbuffer, size_t _len) {
    ...
    std::string filepath =  logdir_ + "/" + forder_name + "/";
...
bool XloggerAppender::GetCurrentLogPath(char* _log_path, unsigned int _len) {
    ...
    strncpy(_log_path, logdir_.c_str(), _len - 1);
    _log_path[_len - 1] = '\0';
    return true;
}
...
bool XloggerAppender::GetCurrentLogCachePath(char* _logPath, unsigned int _len) {
    ...
    if (cache_logdir_.empty())  return false;
    strncpy(_logPath, cache_logdir_.c_str(), _len - 1);
    _logPath[_len - 1] = '\0';
    return true;
}
...
bool XloggerAppender::GetfilepathFromTimespan(int _timespan, const char* _prefix,
    ...
    __GetFilePathsFromTimeval(tv, logdir_, _prefix, LOG_EXT, _filepath_vec);
    if (!cache_logdir_.empty()) {
        __GetFilePathsFromTimeval(tv, cache_logdir_, _prefix, LOG_EXT, _filepath_vec);
    }
    return true;
}
...
bool XloggerAppender::MakeLogfileName(int _timespan, const char* _prefix,
    ...
    __MakeLogFileName(tv, logdir_, _prefix, LOG_EXT, log_path, sizeof(log_path));
    ...
    __MakeLogFileName(tv, cache_logdir_, _prefix, LOG_EXT, cache_log_path, sizeof(cache_log_path));
...
void appender_open_with_cache(TAppenderMode _mode, const std::string& _cachedir, const std::string& _logdir,
    ...
    if (nullptr != sg_default_appender) {
        sg_default_appender->WriteTips2File("appender has already ben opened. _dir:%s _nameprefix:%s", _logdir.c_str(), _nameprefix);
        return; 
    }
