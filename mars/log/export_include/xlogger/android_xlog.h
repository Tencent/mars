...
#define __LOG__(LEVEL, LOG_TAG, FMT, ...) \
    if ((!xlogger_IsEnabledFor(LEVEL)))   \
        ;                                 \
    else                                  \
        __ComLog(LEVEL, LOG_TAG, __FILE__, __FUNCTION__, __LINE__, FMT, ##__VA_ARGS__)
#define __LOGV__(LEVEL, LOG_TAG, FMT, VA_LIST) \
    if ((!xlogger_IsEnabledFor(LEVEL)))        \
        ;                                      \
    else                                       \
        __ComLogV(LEVEL, LOG_TAG, __FILE__, __FUNCTION__, __LINE__, FMT, VA_LIST)
...
#define __android_log_assert(COND, TAG, FMT, ...)                                                    \
    if (((COND) || !xlogger_IsEnabledFor(kLevelFatal)))                                              \
        ;                                                                                            \
    else {                                                                                           \
        XLoggerInfo info = {kLevelFatal, TAG, __FILE__, __FUNCTION__, __LINE__, {0, 0}, -1, -1, -1}; \
        gettimeofday(&info.timeval, NULL);                                                           \
