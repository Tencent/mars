...
class StnManager : public mars::stn::BaseStnManager {
 public:
    explicit StnManager(mars::boot::BaseContext* _context);
    ~StnManager() override;
    void Init() override;
 ...
 public:
    ...
    const std::vector<std::string>& GetLongLinkHosts() override;
    void SetLongLink(const std::vector<std::string>& _hosts, const std::vector<uint16_t>& _ports, const std::string& _debugip) override;
    void SetShortlink(const uint16_t _port, const std::string& _debugip) override;
    // Add for Android
