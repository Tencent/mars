...
#else
    return C2Java_MakesureAuthed(_host);
#endif
}
...
#ifndef ANDROID
    xassert2(sg_callback != NULL);
    return sg_callback->Req2Buf(_taskid, _user_context, outbuffer, extend, error_code, channel_select, host);
#else
    return C2Java_Req2Buf(_taskid, _user_context, outbuffer, extend, error_code, channel_select, host);
#endif
}
...
#ifndef ANDROID
    xassert2(sg_callback != NULL);
    return sg_callback->Buf2Resp(_taskid, _user_context, _inbuffer, _extend, _error_code, _channel_select);
#else
    return C2Java_Buf2Resp(_taskid, _user_context, _inbuffer, _extend, _error_code, _channel_select);
#endif
}
...
#ifndef ANDROID
    xassert2(sg_callback != NULL);
    return sg_callback->OnTaskEnd(_taskid, _user_context, _error_type, _error_code);
#else
    return C2Java_OnTaskEnd(_taskid, _user_context, _error_type, _error_code);
#endif
}
...
#ifndef ANDROID
    xassert2(sg_callback != NULL);
    return sg_callback->GetLonglinkIdentifyCheckBuffer(_identify_buffer, _buffer_hash, _cmdid);
#else
    return C2Java_GetLonglinkIdentifyCheckBuffer(_identify_buffer, _buffer_hash, _cmdid);
#endif
}
...
#ifndef ANDROID
    xassert2(sg_callback != NULL);
    return sg_callback->OnLonglinkIdentifyResponse(_response_buffer, _identify_buffer_hash);
#else
    return C2Java_OnLonglinkIdentifyResponse(_response_buffer, _identify_buffer_hash);
#endif
}
...
bool MakesureAuthed(const std::string& _host) {
    ...
    return sg_callback_bridge->MakesureAuthed(_host);
};
...
bool Req2Buf(uint32_t taskid,  void* const user_context, AutoBuffer& outbuffer, AutoBuffer& extend, int& error_code, const int channel_select, const std::string& host) {
    xassert2(sg_callback_bridge != NULL);
    return sg_callback_bridge->Req2Buf(taskid, user_context, outbuffer, extend, error_code, channel_select, host);
};
//底层回包返回给上层解析
int Buf2Resp(uint32_t taskid, void* const user_context, const AutoBuffer& inbuffer, const AutoBuffer& extend, int& error_code, const int channel_select) {
    xassert2(sg_callback_bridge != NULL);
    return sg_callback_bridge->Buf2Resp(taskid, user_context, inbuffer, extend, error_code, channel_select);
};
//任务执行结束
int  OnTaskEnd(uint32_t taskid, void* const user_context, int error_type, int error_code) {
    xassert2(sg_callback_bridge != NULL);
    return sg_callback_bridge->OnTaskEnd(taskid, user_context, error_type, error_code);
};
...
int  GetLonglinkIdentifyCheckBuffer(AutoBuffer& identify_buffer, AutoBuffer& buffer_hash, int32_t& cmdid) {
    xassert2(sg_callback_bridge != NULL);
    return sg_callback_bridge->GetLonglinkIdentifyCheckBuffer(identify_buffer, buffer_hash, cmdid);
};
//长连信令校验回包
bool OnLonglinkIdentifyResponse(const AutoBuffer& response_buffer, const AutoBuffer& identify_buffer_hash) {
    xassert2(sg_callback_bridge != NULL);
    return sg_callback_bridge->OnLonglinkIdentifyResponse(response_buffer, identify_buffer_hash);
};
