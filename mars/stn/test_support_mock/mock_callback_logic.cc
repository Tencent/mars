...
int onTaskEnd(const int errType,
              const int errCode,
              const char* const errMsg,
              const int hashCode,
              const AutoBuffer& cookies) {
    return 0;
}
...
bool req2Buf(const int hashCode, AutoBuffer& encodeBuffer) {
    return true;
}
int buf2Resp(const int hashCode, const AutoBuffer& decodeBuffer, AutoBuffer& autoBuffer) {
    return -1;
}
void sessionTimeout() {
}
...
void reportFlowData(const int wifiRecvDataSize,
                    const int wifiSendDataSize,
                    int mobileRecvDataSize,
...
int getLongLinkIdentifyCheckBuffer(AutoBuffer& identifyReqBuf,
                                   int& reqCmdId,
                                   int& respCmdId,
                                   AutoBuffer& hashCodeBuffer) {
    return 2;
}
