...
NetSource::DnsUtil::DnsUtil(boot::Context* _context) : context_(_context) {
    ...
    if (context_) {
        new_dns_.SetDnsFunc(
            std::bind(&NetSource::DnsUtil::__OnNewDns, this, std::placeholders::_1, std::placeholders::_2));
    }
}
 ...
 */
void NetSource::SetLongLink(const std::vector<std::string>& _hosts,
...
bool NetSource::__GetLonglinkDebugIPPort(const struct LonglinkConfig& _config, std::vector<IPPortItem>& _ipport_items) {
    for (std::vector<std::string>::iterator ip_iter = sg_longlink_hosts.begin(); ip_iter != sg_longlink_hosts.end();
        ...
        if (sg_host_debugip_mapping.find(*ip_iter) != sg_host_debugip_mapping.end()) {
            for (std::vector<uint16_t>::iterator iter = sg_longlink_ports.begin(); iter != sg_longlink_ports.end();
                 ++iter) {
                IPPortItem item;
...
void NetSource::__GetIPPortItems(std::vector<IPPortItem>& _ipport_items,
    ...
    if (active_logic_.IsActive()) {
        ...
        for (std::vector<std::string>::const_iterator iter = _hostlist.begin(); iter != _hostlist.end(); ++iter) {
            ...
            if (0 < __MakeIPPorts(_ipport_items, *iter, makelist_count, _dns_util, /*_isbackup=*/false, _islonglink))
                merge_type_count++;
        }
...
size_t NetSource::__MakeIPPorts(std::vector<IPPortItem>& _ip_items,
    ...
    if (!_isbackup) {
        ...
        if (!iplist.empty() && !ports.empty()) {
            ...
            if (require_cnt < ports_cnt)
                require_cnt += ports_cnt;
    ...
    xinfo2(TSF "[dual-channel] enable:%_",
           context_->GetManager<AppManager>()->GetAppConfig()->IsCellularNetworkEnable());
            ...
            if (!ips.empty()) {
                ...
                if (_ip_items.size() > 0) {
                    ...
                    for (std::vector<IPPortItem>::iterator ip = _ip_items.begin(); ip != _ip_items.end(); ++ip) {
                        xdebug2(TSF "[dual-channel]ip:%_ port:%_ isCellular:%_",
                                ip->str_ip,
                                ip->port,
                                ip->is_bind_cellular_network);
                    }
...
std::string NetSource::DumpTable(const std::vector<IPPortItem>& _ipport_items) {
    ...
    for (unsigned int i = 0; i < _ipport_items.size(); ++i) {
        stream << _ipport_items[i].str_ip << kItemDelimiter << _ipport_items[i].port << kItemDelimiter
               << _ipport_items[i].str_host << kItemDelimiter << IPSourceTypeString[_ipport_items[i].source_type];
