...
class LongLinkConnectObserver : public MComplexConnect {
 public:
    ...
    : longlink_(_longlink), ip_items_(_iplist) {
    ...
    virtual void OnConnected(unsigned int _index, const socket_address& _addr, SOCKET _socket, int _error, int _rtt) {
        if (0 == _error) {
            ...
            xwarn2(TSF "index:%_, connnet fail host:%_, iptype:%_",
                   _index,
                   ip_items_[_index].str_host,
    ...
    virtual bool OnVerifySend(unsigned int _index,
        ...
        longlink_.Encoder().longlink_pack(longlink_.Encoder().longlink_noop_cmdid(),
                                          Task::kNoopTaskID,
                                          body,
                                          extension,
                                          _buffer_send,
    ...
    virtual bool OnVerifyRecv(unsigned int _index,
        ...
        AutoBuffer extension;
        int ret =
            longlink_.Encoder().longlink_unpack(_buffer_recv, cmdid, taskid, pack_len, bufferbody, extension, NULL);
        ...
        if (!longlink_.Encoder().longlink_noop_isresp(taskid, cmdid, taskid, bufferbody, extension)) {
            xwarn2(TSF "index:%_, sock:%_, %_, ret:%_, cmdid:%_, taskid:%_, pack_len:%_, recv_len:%_",
                   _index,
                   _socket,
                   _addr.url(),
                   ret,
...
LongLink::LongLink(Context* _context,
...
bool LongLink::SendWhenNoData(const AutoBuffer& _body,
...
bool LongLink::__NoopResp(uint32_t _cmdid,
                          uint32_t _taskid,
                          AutoBuffer& _buf,
...
void LongLink::__RunResponseError(ErrCmdType _error_type,
                                  int _error_code,
                                  ConnectProfile& _profile,
                                  bool _networkreport) {
    AutoBuffer buf;
...
void LongLink::__UpdateProfile(const ConnectProfile _conn_profile) {
    ...
    if (0 != conn_profile_.disconn_time)
        broadcast_linkstatus_signal_(conn_profile_);
}
...
void LongLink::__Run() {
    ...
    xinfo_function(TSF "LongLink Rebuild span:%_, net:%_, channel name:%_, linktype:%_",
                   conn_profile_.disconn_time != 0 ? cur_time - conn_profile_.disconn_time : 0,
                   getNetInfo(),
                   config_.name,
    ...
    __SetBindCellularNetwork(conn_profile.is_bind_cellular_network);
...
SOCKET LongLink::__RunConnect(ConnectProfile& _conn_profile) {
    ...
    __SetBindCellularNetwork(_conn_profile.is_bind_cellular_network);
    ...
    bool use_proxy = proxy_info.IsValid() && mars::comm::kProxyNone != proxy_info.type
                     && mars::comm::kProxyHttp != proxy_info.type && netsource_->GetLongLinkDebugIP().empty();
    ...
    xinfo2(TSF "task socket dns ip:%_ proxytype:%_ useproxy:%_",
           netsource_->DumpTable(ip_items),
           proxy_info.type,
           use_proxy);
    ...
    for (unsigned int i = 0; i < ip_items.size(); ++i) {
        if (use_proxy) {
            vecaddr.push_back(
                socket_address(ip_items[i].str_ip.c_str(), ip_items[i].port, ip_items[i].is_bind_cellular_network));
        } else {
            vecaddr.push_back(
                socket_address(ip_items[i].str_ip.c_str(), ip_items[i].port, ip_items[i].is_bind_cellular_network)
    ...
    xinfo2(TSF
           ...
           ChannelTypeString[_conn_profile.link_type],
           com_connect.TotalCost(),
           com_connect.IndexRtt(),
           com_connect.IndexTotalCost(),
           com_connect.Index(),
           ::getNetInfo());
    __ConnectStatus(kConnected);
    __SetBindCellularNetwork(conn_profile_.is_bind_cellular_network);
    __UpdateProfile(_conn_profile);

    xerror2_if(0 != socket_disable_nagle(sock, 1),
               TSF "socket_disable_nagle sock:%0, %1(%2)",
               sock,
               socket_errno,
...
void LongLink::__RunReadWrite(SOCKET _sock, ErrCmdType& _errtype, int& _errcode, ConnectProfile& _profile) {
    ...
    while (true) {
        while (!alarmnoopinterval.IsWaiting()) {
            ...
            uint64_t last_noop_actual_interval =
                (alarmnoopinterval.Status() == Alarm::kOnAlarm) ? alarmnoopinterval.ElapseTime() : 0;
        ...
        if (isnooping_ && (alarmnooptimeout.Status() == Alarm::kInit || alarmnooptimeout.Status() == Alarm::kCancel)) {
        ...
        if (LongLinkErrCode::kNone != disconnectinternalcode_) {
            xwarn2(TSF "task socket close sock:%0, user disconnect:%1, nread:%_, nwrite:%_",
                   _sock,
                   disconnectinternalcode_,
                   socket_nread(_sock),
                   socket_nwrite(_sock))
                >> close_log;
        ...
        if (0 > retsel) {
            xfatal2(TSF "task socket close sock:%0, 0 > retsel, errno:%_, nread:%_, nwrite:%_",
                    _sock,
                    sel.Errno(),
                    socket_nread(_sock),
                    socket_nwrite(_sock))
        ...
        if (sel.IsException()) {
            xerror2(TSF "task socket close sock:%0, socketselect excptoin:%1(%2), nread:%_, nwrite:%_",
                    _sock,
                    socket_errno,
                    socket_strerror(socket_errno),
                    socket_nread(_sock),
        ...
        if (sel.Exception_FD_ISSET(_sock)) {
            ...
            xerror2(TSF "task socket close sock:%0, excptoin:%1(%2), nread:%_, nwrite:%_",
                    _sock,
                    error,
        ...
        if (sel.Write_FD_ISSET(_sock) && !lstsenddata_.empty()) {
            ...
            // ssize_t writelen = ::send(_sock, lstsenddata_.begin()->data.PosPtr(),
            // lstsenddata_.begin()->data.PosLength(), 0);
            ssize_t writelen =
                ::send(_sock, lstsenddata_.begin()->second->PosPtr(), lstsenddata_.begin()->second->PosLength(), 0);
#endif
            ...
            while (it != lstsenddata_.end() && 0 < writelen) {
                if (0 == it->second->Pos() && OnSend)
                    OnSend(it->first.taskid);

                if ((size_t)writelen >= it->second->PosLength()) {
                    xinfo2(TSF "sub send taskid:%_, cmdid:%_, %_, len(S:%_, %_/%_), ",
                           it->first.taskid,
                           it->first.cmdid,
                           it->first.cgi,
                           it->second->PosLength(),
                    ...
                    if (!it->first.send_only) {
                        sent_taskids[it->first.taskid].task = it->first;
                    }
        ...
        if (sel.Read_FD_ISSET(_sock)) {
            ...
            if (0 > recvlen && !IS_NOBLOCK_READ_ERRNO(socket_errno)) {
                _errtype = kEctSocket;
                _errcode = socket_errno;
                xerror2(TSF "task socket close sock:%0, recv len: %1 errno:%2(%3)",
                        _sock,
                        recvlen,
                        socket_errno,
                        socket_strerror(socket_errno))
            ...
            while (0 < bufrecv.Length()) {
                ...
                int unpackret =
                    Encoder().longlink_unpack(bufrecv, cmdid, taskid, packlen, body, extension, tracker_.get());

                if (LONGLINK_UNPACK_FALSE == unpackret) {
                    xerror2(TSF "task socket recv sock:%0, unpack error dump:%1",
                            _sock,
                            xlogger_memory_dump(bufrecv.Ptr(), bufrecv.Length()));
                ...
                xinfo2(TSF "task socket recv sock:%_, pack recv %_ taskid:%_, cmdid:%_, %_, packlen:(%_/%_)",
                       _sock,
                       LONGLINK_UNPACK_CONTINUE == unpackret ? "continue" : "finish",
                       taskid,
                       cmdid,
                       stream_resp.task.cgi,
                       LONGLINK_UNPACK_CONTINUE == unpackret ? bufrecv.Length() : packlen,
                       packlen);
                ...
                xassert2(unpackret == LONGLINK_UNPACK_STREAM_END || unpackret == LONGLINK_UNPACK_OK
...
End:
    ...
    if (nwrite_size > 0 && !nsent_datas.empty()) {
        ...
        ssize_t maxnwrite = 0;
        for (std::vector<LongLinkNWriteData>::reverse_iterator it = nsent_datas.rbegin(); it != nsent_datas.rend();
             ++it) {
            if (nwrite_size <= (maxnwrite + it->writelen)) {
                xinfo2(TSF "taskid:%_, cmdid:%_, cgi:%_ ; ", it->task.taskid, it->task.cmdid, it->task.cgi)
                    >> close_log;
                break;
            } else {
...
void LongLink::__SetBindCellularNetwork(bool _flag) {
    is_bind_cellular_network_ = _flag;
}
