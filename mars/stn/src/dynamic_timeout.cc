...
void DynamicTimeout::CgiTaskStatistic(std::string _cgi_uri, unsigned int _total_size, uint64_t _cost_time) {
    int task_status =
        (_total_size == kDynTimeTaskFailedPkgLen || _cost_time == 0) ? kDynTimeTaskFailedTag : KDynTimeTaskNormalTag;
...
void DynamicTimeout::__StatusSwitch(std::string _cgi_uri, int _task_status) {
    ...
    xdebug2(
        TSF
        "task_status:%_, good_count:%_, good_time:%_, dyntime_status:%_, dyntime_failed_normal_count_NORMAL:%_, cgi:%_",
        _task_status,
        dyntime_continuous_good_count_,
