...
SOCKET ShortLink::__RunConnect(ConnectProfile& _conn_profile) {
    ...
    if (use_proxy && mars::comm::kProxyHttp == _conn_profile.proxy_info.type) {
        ...
        for (size_t i = 0; i < _conn_profile.ip_items.size(); ++i) {
            if (!use_proxy || mars::comm::kProxyNone == _conn_profile.proxy_info.type) {
                vecaddr.push_back(socket_address(_conn_profile.ip_items[i].str_ip.c_str(), _conn_profile.ip_items[i].port, _conn_profile.ip_items[i].is_bind_cellular_network).v4tov6_address(local_stack));
            } else {
                vecaddr.push_back(socket_address(_conn_profile.ip_items[i].str_ip.c_str(), _conn_profile.ip_items[i].port, _conn_profile.ip_items[i].is_bind_cellular_network));
            }
        }
    }
