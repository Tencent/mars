...
// LongLinkTaskManager::get_real_host_; boost::function<void (uint32_t _version, mars::stn::TlsHandshakeFrom _from)>
// LongLinkTaskManager::on_handshake_ready_; boost::function<bool (int _error_code)>
// LongLinkTaskManager::should_intercept_result_;

static int longlink_id = 0;
// std::set<std::string> LongLinkTaskManager::forbid_tls_host_;

LongLinkTaskManager::LongLinkTaskManager(mars::boot::Context* _context,
    ...
    while (first != last) {
        ...
        if (first->running_id) {
            ...
            __SingleRespHandle(first,
                               ...
                               __GetConnectionProfile(longlink));
        }
...
void LongLinkTaskManager::RetryTasks(ErrCmdType _err_type,
                                     int _err_code,
                                     int _fail_handle,
...
void LongLinkTaskManager::__RunOnTimeout() {
    ...
    for (auto item : batchMap) {
        if (item.second.first == kEctLongTaskTimeout) {
            __BatchErrorRespHandle(item.first,
                                   kEctNetMsgXP,
                                   kEctLocalTaskTimeout,
                                   kTaskFailHandleDefault,
                                   item.second.second);
            ...
            if (longlink) {
                ...
                if (longlink_channel) {
                    fun_notify_network_err_(item.first,
                                            __LINE__,
                                            kEctNetMsgXP,
                                            item.second.first,
                                            longlink_channel->Profile().ip,
...
void LongLinkTaskManager::__RunOnStartTask() {
    ...
    while (first != last) {
        ...
        if (get_real_host_) {
            get_real_host_(task.user_id, task.longlink_host_list, /*_strict_match=*/false, task.host_extra_info);
        }
        std::string host = "";
        ...
        if (first->task.need_authed) {
            ...
            xinfo2(TSF "makesureauth host:%_, auth result:%_, cgi:%_, channal name:%_",
                   host,
        ...
        if (!first->running_id) {
            xwarn2(TSF "task add into longlink readwrite fail cgi:%_, cmdid:%_, taskid:%_",
                   first->task.cgi,
                   first->task.cmdid,
                   first->task.taskid);
            first = next;
        ...
        xinfo2(TSF
               "task add into longlink readwrite suc cgi:%_, cmdid:%_, taskid:%_, size:%_, channel name:%_, "
               "timeout(firstpkg:%_, rw:%_, task:%_), retry:%_, curtime:%_, start_send_time:%_, sendonly:%_",
               first->task.cgi,
               first->task.cmdid,
               first->task.taskid,
               ...
               curtime,
               first->start_task_time,
               first->task.send_only);
...
bool LongLinkTaskManager::__SingleRespHandle(std::list<TaskProfile>::iterator _it,
    ...
    if (0 >= _it->remain_retry_count || kEctOK == _err_type || kTaskFailHandleTaskEnd == _fail_handle
        ...
        xlog2(kEctOK == _err_type ? kLevelInfo : kLevelWarn,
              TSF "task end callback  long cmdid:%_, err(%_, %_, %_), ",
              _it->task.cmdid,
              _err_type,
              _err_code,
            ...
            receive_data_size - received_size ? "/" : "",
            receive_data_size,
            ...
            _it->remain_retry_count)(TSF "cgi:%_, taskid:%_, tid:%_, context id:%_",
                                     _it->task.cgi,
                                     _it->task.taskid,
                                     _connect_profile.tid,
                                     _it->task.user_id);
    ...
    xlog2(kEctOK == _err_type ? kLevelInfo : kLevelWarn,
          TSF "task end retry  long cmdid:%_, err(%_, %_, %_), ",
          _it->task.cmdid,
          _err_type,
          _err_code,
          _fail_handle)(TSF "svr(%_:%_, %_, %_), ",
        ...
        (_it->transfer_profile.start_send_time == 0 ? 0 : curtime - _it->transfer_profile.start_send_time),
        (curtime - _it->start_task_time),
        _it->remain_retry_count)(TSF "cgi:%_, taskid:%_, tid:%_",
                                 _it->task.cgi,
                                 _it->task.taskid,
                                 _connect_profile.tid);
...
void LongLinkTaskManager::__BatchErrorRespHandleByUserId(const std::string& _user_id,
                                                         ErrCmdType _err_type,
                                                         int _err_code,
                                                         int _fail_handle,
                                                         uint32_t _src_taskid,
    ...
    while (first != temp.end()) {
        ...
        __BatchErrorRespHandle(first->channel_name,
                               _err_type,
                               _err_code,
                               _fail_handle,
                               _src_taskid,
...
void LongLinkTaskManager::__BatchErrorRespHandle(const std::string _name,
                                                 ErrCmdType _err_type,
                                                 int _err_code,
                                                 int _fail_handle,
                                                 uint32_t _src_taskid,
                                                 bool _callback_runing_task_only) {
    xinfo2(TSF "batch error, channel name:%_, src id:%_ callback:%_, errcode:%_, errtype:%_, fail handle:%_",
           _name,
           _src_taskid,
           _callback_runing_task_only,
           _err_code,
...
struct find_task {
 public:
    bool operator()(const TaskProfile& value) {
        return taskid == value.task.taskid;
    }
...
std::list<TaskProfile>::iterator LongLinkTaskManager::__Locate(uint32_t _taskid) {
    if (Task::kInvalidTaskID == _taskid)
        return lst_cmd_.end();
...
void LongLinkTaskManager::__OnResponse(const std::string& _name,
    ...
    RETURN_LONKLINK_SYNC2ASYNC_FUNC_TITLE(boost::bind(&LongLinkTaskManager::__OnResponse,
                                                      this,
                                                      _name,
                                                      _error_type,
                                                      _error_code,
    ...
    if (kEctOK != _error_type) {
        xwarn2(TSF "task error, taskid:%_, cmdid:%_, error_type:%_, error_code:%_",
               _taskid,
               _cmdid,
               _error_type,
               _error_code);
    ...
    if (lst_cmd_.end() == it) {
        xwarn2_if(Task::kInvalidTaskID != _taskid,
                  TSF "task no found task:%0, cmdid:%1, ect:%2, errcode:%3",
                  _taskid,
                  _cmdid,
                  _error_type,
                  _error_code);
    ...
    switch (handle_type) {
        case kTaskFailHandleNoError: {
            dynamic_timeout_.CgiTaskStatistic(
                it->task.cgi,
                (unsigned int)it->transfer_profile.send_data_size + (unsigned int)body->Length(),
        ...
        case kTaskFailHandleTaskEnd: {
            xwarn2(TSF "task decode error taskid:%_, cmdid:%_, handle_type:%_",
                   it->task.taskid,
                   it->task.cmdid,
                   handle_type);
        ...
        case kTaskFailHandleDefault: {
            xerror2(TSF "task decode error taskid:%_, handle_type:%_, err_code:%_, body dump:%_",
                    it->task.taskid,
                    handle_type,
                    err_code,
                    xlogger_memory_dump(body->Ptr(), body->Length()));
            __BatchErrorRespHandle(it->channel_name, kEctEnDecode, err_code, handle_type, it->task.taskid);
            xassert2(fun_notify_network_err_);
            fun_notify_network_err_(_name,
                                    __LINE__,
                                    kEctEnDecode,
                                    err_code,
                                    _connect_profile.ip,
        ...
        default: {
            xassert2(false,
                     TSF "task decode error fail_handle:%_, taskid:%_, context id:%_",
                     handle_type,
                     it->task.taskid,
                     it->task.user_id);
            __BatchErrorRespHandle(it->channel_name, kEctEnDecode, err_code, handle_type, it->task.taskid);
...
void LongLinkTaskManager::__OnRecv(uint32_t _taskid, size_t _cachedsize, size_t _totalsize) {
    RETURN_LONKLINK_SYNC2ASYNC_FUNC(
        boost::bind(&LongLinkTaskManager::__OnRecv, this, _taskid, _cachedsize, _totalsize));
    xverbose_function();
    std::list<TaskProfile>::iterator it = __Locate(_taskid);
    ...
    if (lst_cmd_.end() != it) {
        if (it->transfer_profile.last_receive_pkg_time == 0)
            ...
            netsource_->GetWeakNetworkLogic()->OnPkgEvent(
                true,
...
void LongLinkTaskManager::FixMinorRealhost(Task& _task) {
    if (get_real_host_) {
        get_real_host_(_task.user_id, _task.minorlong_host_list, /*_strict_match=*/false, _task.host_extra_info);
    }
}
