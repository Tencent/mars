...
LongLinkTaskManager::LongLinkTaskManager(mars::boot::BaseContext* _context, NetSource& _netsource, ActiveLogic& _activelogic, DynamicTimeout& _dynamictimeout, MessageQueue::MessageQueue_t  _messagequeue_id)
    : context_(_context)
...
void LongLinkTaskManager::__RunOnStartTask() {
    ...
    while (first != last) {
        ...
        if (first->task.need_authed) {
            bool ismakesureauthsuccess = context_->GetStnManager()->MakesureAuthed(host, first->task.user_id);
        ...
        if (!first->antiavalanche_checked) {
			if (!context_->GetStnManager()->Req2Buf(first->task.taskid, first->task.user_context, first->task.user_id, bufreq, buffer_extension, error_code, longlink->Config().link_type, host)) {
        ...
        if (task_intercept_.GetInterceptTaskInfo(first->task.cgi, intercept_data)) {
            ...
            int handle_type = context_->GetStnManager()->Buf2Resp(first->task.taskid, first->task.user_context, first->task.user_id, body, extension, err_code, longlink->Config().link_type);
...
bool LongLinkTaskManager::__SingleRespHandle(std::list<TaskProfile>::iterator _it, ErrCmdType _err_type, int _err_code, int _fail_handle, const ConnectProfile& _connect_profile) {
    ...
    if (0 >= _it->remain_retry_count || kEctOK == _err_type || kTaskFailHandleTaskEnd == _fail_handle || kTaskFailHandleTaskTimeout == _fail_handle) {
        ...
        context_->GetStnManager()->ReportTaskProfile(*_it);
...
void LongLinkTaskManager::__OnResponse(const std::string& _name, ErrCmdType _error_type, int _error_code, uint32_t _cmdid, uint32_t _taskid, AutoBuffer& _body, AutoBuffer& _extension, const ConnectProfile& _connect_profile) {
    ...
    int err_code = 0;
    int handle_type = context_->GetStnManager()->Buf2Resp(it->task.taskid, it->task.user_context, it->task.user_id, body, extension, err_code, longlink_meta->Config().link_type);
