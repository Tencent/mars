...
bool ProxyTest::ProxyIsAvailable(const mars::comm::ProxyInfo _proxy_info,
                                 const std::string& _host,
                                 const std::vector<std::string>& _hardcode_ips) {
    xinfo_function(TSF "type:%_ host:%_ ip:%_:%_ username:%_ test_host:%_ hardcode_ip:%_",
                   _proxy_info.type,
                   _proxy_info.host,
                   _proxy_info.ip,
                   _proxy_info.port,
                   _proxy_info.username,
                   _host,
...
SOCKET ProxyTest::__Connect(const mars::comm::ProxyInfo& _proxy_info,
                            const std::string& _host,
                            const std::vector<std::string>& _hardcode_ips) {
...
int ProxyTest::__ReadWrite(SOCKET _sock, const mars::comm::ProxyInfo& _proxy_info, const std::string& _host) {
    ...
    int err_code = 0;
    int send_ret = block_socket_send(_sock,
    ...
    if (send_ret < 0) {
        xerror2(TSF "test proxy Error, ret:%0, errno:%1, nread:%_, nwrite:%_",
                send_ret,
                strerror(err_code),
                socket_nread(_sock),
                socket_nwrite(_sock));
    ...
    if (testproxybreak_.IsBreak()) {
        xwarn2(TSF "test proxy break, sent:%_ nread:%_, nwrite:%_",
               send_ret,
               socket_nread(_sock),
               socket_nwrite(_sock));
        return -1;
    ...
    while (true) {
        ...
        if (recv_ret == 0) {
            xerror2(TSF "remote disconnect, nread:%_, nwrite:%_",
                    err_code,
                    strerror(err_code),
                    socket_nread(_sock),
                    socket_nwrite(_sock));
