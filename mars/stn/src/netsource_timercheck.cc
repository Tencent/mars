...
NetSourceTimerCheck::NetSourceTimerCheck(boot::Context* _context,
                                         std::shared_ptr<NetSource> _net_source,
                                         ActiveLogic& _active_logic,
                                         LongLink& _longlink,
...
void NetSourceTimerCheck::__StartCheck() {
    ...
    if (asyncpost_ != MessageQueue::KNullPost)
        return;

    asyncpost_ = MessageQueue::AsyncInvokePeriod(kTimeCheckPeriod,
                                                 kTimeCheckPeriod,
                                                 boost::bind(&NetSourceTimerCheck::__Check, this),
                                                 asyncreg_.Get(),
...
bool NetSourceTimerCheck::__TryConnnect(const std::string& _host) {
    ...
    if (ip_vec.empty())
        dns_util_.GetDNS().GetHostByName(_host, ip_vec);
    if (ip_vec.empty())
        return false;
