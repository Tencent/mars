...
namespace stn {
    ...
    WeakNetworkLogic::WeakNetworkLogic():is_curr_weak_(false), connect_after_weak_(0)
        ...
        foreground_signal_slot_ = ActiveLogic::Instance()->SignalForeground.connect(std::bind(&WeakNetworkLogic::__SignalForeground, this, std::placeholders::_1));
    }
    
    WeakNetworkLogic::~WeakNetworkLogic() {
        ActiveLogic::Instance()->SignalForeground.disconnect(foreground_signal_slot_);
    }
