...
int LongLinkSpeedTestItem::__HandleSpeedTestResp() {
    ...
    if (nrecv <= 0) {
        xerror2(TSF "recv nrecv <= 0, errno:%0, resp_ab_.Capacity():%1,resp_ab_.Pos():%2",
                strerror(errno),
                resp_ab_.Capacity(),
                resp_ab_.Pos());
        return kLongLinkSpeedTestFail;
...
LongLinkSpeedTest::LongLinkSpeedTest(const std::shared_ptr<NetSource>& _netsource)
...
bool LongLinkSpeedTest::GetFastestSocket(int& _fdSocket,
                                         std::string& _strIp,
                                         unsigned int& _port,
                                         IPSourceType& _type,
    ...
    for (std::vector<LongLinkSpeedTestItem*>::iterator iter = speedTestItemVec.begin(); iter != speedTestItemVec.end();
         ++iter) {
        for (std::vector<IPPortItem>::iterator ipItemIter = ipItemVec.begin(); ipItemIter != ipItemVec.end();
             ++ipItemIter) {
