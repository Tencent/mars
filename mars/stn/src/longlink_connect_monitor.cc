...
static int const reconnect_interval[7] =
    {0, 60, 120, 240, 360, 480, 600};    // if modify this varible ,do not forget to modify varible: "kInternalMaxIndex"
...
static int __CurActiveState(const ActiveLogic& _activeLogic) {
    if (!_activeLogic.IsActive())
        return kInactive;

    if (!_activeLogic.IsForeground())
        return kBackgroundActive;
...
LongLinkConnectMonitor::LongLinkConnectMonitor(boot::Context* _context,
...
, net_source_(_net_source) {
    ...
    if (is_keep_alive_) {
        activelogic_.SignalActive.connect(
            boost::bind(&LongLinkConnectMonitor::__OnSignalActive, this, boost::placeholders::_1));
        activelogic_.SignalForeground.connect(
            boost::bind(&LongLinkConnectMonitor::__OnSignalForeground, this, boost::placeholders::_1));
        longlink_.SignalConnection.connect(boost::bind(&LongLinkConnectMonitor::__OnLongLinkStatuChanged,
                                                       this,
...
LongLinkConnectMonitor::~LongLinkConnectMonitor() {
    ...
    __StopTimer();
#endif
    longlink_.SignalConnection.disconnect(boost::bind(&LongLinkConnectMonitor::__OnLongLinkStatuChanged,
                                                      this,
                                                      boost::placeholders::_1,
                                                      boost::placeholders::_2));
    activelogic_.SignalForeground.disconnect(
        boost::bind(&LongLinkConnectMonitor::__OnSignalForeground, this, boost::placeholders::_1));
    activelogic_.SignalActive.disconnect(
        boost::bind(&LongLinkConnectMonitor::__OnSignalActive, this, boost::placeholders::_1));
...
bool LongLinkConnectMonitor::NetworkChange() {
    ...
    do {
        if (LongLink::kConnected != status_ || (::gettickcount() - last_connect_time_) <= 10 * 1000)
            break;

        if (kMobile != last_connect_net_type_)
            break;
...
uint64_t LongLinkConnectMonitor::__IntervalConnect(int _type) {
    ...
    if (LongLink::kConnecting == longlink_.ConnectStatus() || LongLink::kConnected == longlink_.ConnectStatus())
        return 0;
    ...
    if (activelogic_.IsActive() || _type == kNetworkChangeConnect) {
        ...
        if ((posttime + buffer) >= interval) {
            ...
            xinfo2(TSF
                   "process active made interval connect interval:%0, posttime:%_, newone:%_, connectstatus:%_, ret:%_",
                   interval,
                   posttime,
                   newone,
                   longlink_.ConnectStatus(),
                   ret);
            return 0;
        } else {
    ...
    if (interval_final == 0) {
        bool newone = false;
        bool ret = longlink_.MakeSureConnected(&newone);
        xinfo2(TSF "rebuild now, connect interval:%0, posttime:%_, newone:%_, connectstatus:%_, ret:%_",
               interval,
               posttime,
               newone,
               longlink_.ConnectStatus(),
...
uint64_t LongLinkConnectMonitor::__AutoIntervalConnect() {
    ...
    xinfo_trace(TSF
                "longlink_progress set rebuild alarm, reason: rebuild longlink alarm, current time: %_, next rebuild "
                "time interval: %_",
                gettickcount(),
                remain);
#ifdef __ANDROID__
...
void LongLinkConnectMonitor::__OnSignalForeground(bool _isForeground) {
    ...
    if (_isForeground) {
        xinfo2(TSF "longlink:%_ time:%_ %_ %_",
               longlink_.ConnectStatus(),
               tickcount_t().gettickcount().get(),
...
void LongLinkConnectMonitor::__OnAlarm(bool _rebuild_longlink) {
    ...
    __AutoIntervalConnect();
    if (rebuild_longlink_) {
        xinfo_trace(TSF "longlink_progress onalarm log, reason: %_, alarm after %_ millsecond, spend time %_",
                    alarm_reason,
                    rebuild_alarm_.After(),
                    rebuild_alarm_.ElapseTime());
    }
}
...
void LongLinkConnectMonitor::OnHeartbeatAlarmReceived(bool _is_noop_timeout) {
    xinfo_trace(TSF "longlink_progress on_heartbeat_alarm, time: %_, is_noop_timeout:%_",
                gettickcount(),
                _is_noop_timeout);
}
