...
//            it->remain_retry_count++;
//        }
//        _worker->OnSingleRespHandle(*it, _err_type, _status, kTaskFailHandleDefault, _body.Length(), _conn_profile);
//        return;
//    }
//
...
void ShortLinkTaskManager::RedoTasks() {
    ...
    while (first != last) {
        ...
        if (first->running_id) {
            ...
            xinfo2(TSF "task redo, taskid:%_", first->running_id);
            //            ConnectProfile connect_profile;
            //            if(((ShortLinkInterface*)first->running_id)->Profile() == NULL){
            //                connect_profile = ((ShortLinkInterface*)first->running_id)->Profile();
            //            }else{
            //                connect_profile = ConnectProfile();
            //            }

            ((ShortLinkInterface*)first->running_id)
                ->OnSingleRespHandle(*first,
                                     kEctLocal,
                                     kEctLocalCancel,
...
void ShortLinkTaskManager::__BatchErrorRespHandle(ErrCmdType _err_type,
    ...
    while (first != last) {
        ...
        if (_fail_handle == kTaskFailHandleSessionTimeout) {  // retry task when sessiontimeout
            if (_src_taskid != 0 && first->task.taskid == _src_taskid && first->allow_sessiontimeout_retry) {
...
// bool ShortLinkTaskManager::__NewSingleRespHandle(ShortLinkInterface* _worker,
//                                                 TaskProfile& _it,
//                                                 ErrCmdType _err_type,
//                                                 int _err_code,
//                                                 int _fail_handle,
