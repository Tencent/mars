...
ShortLinkTaskManager::~ShortLinkTaskManager() {
    ...
    if (false) {
        ...
        auto event = new EventBatchErrorRespTasks(kEctLocal,
                                                  kEctLocalReset,
                                                  kTaskFailHandleTaskEnd,
                                                  Task::kInvalidTaskID,
                                                  false);
        EventCenter::getInstance().publish(event);
    }
...
bool ShortLinkTaskManager::StartTask(const Task& _task, PrepareProfile _prepare_profile) {
    ...
    if (false) {
        ...
        return NewStartTask(_task, _prepare_profile);
    }
}

bool ShortLinkTaskManager::NewStartTask(const Task& _task, const PrepareProfile& _prepare_profile) {
    ShortlinkConfig config(false, /*use_tls=*/true);
    ...
    // std::bind(&ShortLinkTaskManager::__OnAddWeakNetInfo,
    //                                               this,
    //                                               std::placeholders::_1,
    //                                               std::placeholders::_2);
    ...
    worker->OnHandshakeCompleted =
        on_handshake_ready_;  // boost::bind(&ShortLinkTaskManager::__OnHandshakeCompleted, this, _1, _2);
    ...
    //    }
    worker->choose_protocol_ = choose_protocol_;

    //    worker->SendRequest();

    //    xinfo2_if(task.priority >= 0,
    //              TSF
    ...
    //              task.cmdid,
    //              task.taskid,
    //              (ShortLinkInterface*)running_id,
    //              transfer_profile.send_data_size,
    //              transfer_profile.first_pkg_timeout / 1000,
    //              transfer_profile.read_write_timeout / 1000,
    //              task_timeout / 1000,
    //              remain_retry_count,
    //              task.long_polling,
    //              use_proxy,
    ...
    worker->get_real_host_ = get_real_host_;
    worker->can_use_tls_ = can_use_tls_;
    ...
    worker->OnGetInterceptTaskInfo =
        std::bind(&ShortLinkTaskManager::__GetInterceptTaskInfo, this, std::placeholders::_1, std::placeholders::_2);
    ...
    worker->OnSocketPoolReport = std::bind(&ShortLinkTaskManager::__OnSocketPoolReport,
                                           ...
                                           std::placeholders::_2,
                                           std::placeholders::_3);
    worker->OnSocketPoolAddCache =
        std::bind(&ShortLinkTaskManager::__OnSocketPoolAddCache, this, std::placeholders::_1);
    worker->OnCgiTaskStatistic = std::bind(&ShortLinkTaskManager::__OnCgiTaskStatistic,
                                           this,
                                           std::placeholders::_1,
                                           std::placeholders::_2,
                                           std::placeholders::_3);
    ...
    worker->OnRemoveLst.set(boost::bind(&ShortLinkTaskManager::__OnRemoveLst, this, _1), worker, AYNC_HANDLER);
    worker->Send();

    lst_shortlink_.push_back((intptr_t)worker);
...
bool ShortLinkTaskManager::StopTask(uint32_t _taskid) {
    ...
    if (false) {
        ...
        auto event = new EventStopTask(_taskid);
        if (EventCenter::getInstance().publish(event)) {
            return true;
        }
    }
...
bool ShortLinkTaskManager::HasTask(uint32_t _taskid) const {
    ...
    if (false) {
        ...
        auto event = new EventHasTask(_taskid);
        if (EventCenter::getInstance().publish(event)) {
            return true;
        }
    }
...
void ShortLinkTaskManager::ClearTasks() {
    ...
    if (false) {
        ...
        lst_shortlink_.clear();
        auto event = new EventClearTasks();
        EventCenter::getInstance().publish(event);
    }
}
...
void ShortLinkTaskManager::__RunLoop() {
    xdebug_function();
    if (lst_cmd_.empty() && !EventCenter::getInstance().publish(new EventHasTasks())) {
#ifdef ANDROID
    ...
    if (false) {
        ...
        auto timeoutEvent = new EventOnTimeout();
        EventCenter::getInstance().publish(timeoutEvent);
        auto startTaskEvent = new EventOnStartTask();
        EventCenter::getInstance().publish(startTaskEvent);
    }
    ...
    if (false) {
        ...
        while (first != last) {
            ...
            if (cur_time - first->start_task_time >= first->task_timeout) {
                ...
                xerror2(TSF
                        "task long-polling timeout, taskid:%_, wworker:%_, nStartSendTime:%_, nLongPollingTimeout:%_",
                        first->task.taskid,
                        (void*)first->running_id,
                        first->transfer_profile.start_send_time / 1000,
                ...
                socket_timeout_code = kEctHttpLongPollingTimeout;
                       ...
                       && 0 == first->transfer_profile.last_receive_pkg_time
                       && cur_time - first->transfer_profile.start_send_time
                              >= first->transfer_profile.first_pkg_timeout) {
        ...
        EventOnTimeout* event = new EventOnTimeout();
        xinfo2(TSF "cpan event publish %_", event->getType());
        EventCenter::getInstance().publish(event);
    }
}
...
void ShortLinkTaskManager::__RunOnStartTask() {
    if (false) {
        ...
        auto event = new EventOnStartTask();
        EventCenter::getInstance().publish(event);
    }
}
...
void ShortLinkTaskManager::__BatchErrorRespHandle(ErrCmdType _err_type,
    ...
    if (false) {
        ...
        auto event =
            new EventBatchErrorRespTasks(_err_type, _err_code, _fail_handle, _src_taskid, _callback_runing_task_only);
        EventCenter::getInstance().publish(event);
    }
}
...
void ShortLinkTaskManager::__OnSocketPoolReport(bool _is_reused, bool _has_received, bool _is_decode_ok) {
    socket_pool_.Report(_is_reused, _has_received, _is_reused);
}

bool ShortLinkTaskManager::__OnSocketPoolAddCache(CacheSocketItem& item) {
    return socket_pool_.AddCache(item);
}
...
void ShortLinkTaskManager::__OnRemoveLst(intptr_t& _worker) {
    xinfo_function();
    xinfo2(TSF "cpan event before lst size %_", lst_shortlink_.size());
    lst_shortlink_.remove(_worker);
    __DeleteShortLink(_worker);
    xinfo2(TSF "cpan event after lst size %_", lst_shortlink_.size());
}
