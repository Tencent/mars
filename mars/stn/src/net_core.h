...
class NetCore {
 ...
 public:
    ...
    boost::function<int(int _from, ErrCmdType _err_type, int _err_code, int _fail_handle, const Task& _task)>
        task_callback_hook_;
    boost::signals2::signal<void(uint32_t _cmdid, const AutoBuffer& _buffer)> push_preprocess_signal_;

 public:
    comm::MessageQueue::MessageQueue_t GetMessageQueueId() {
 ...
 private:
    ...
    int __CallBack(int _from,
                   ErrCmdType _err_type,
                   int _err_code,
                   int _fail_handle,
                   const Task& _task,
                   unsigned int _taskcosttime);
    void __OnShortLinkNetworkError(int _line,
                                   ErrCmdType _err_type,
                                   int _err_code,
...
#ifdef USE_LONG_LINK
    void __OnLongLinkNetworkError(const std::string& _name,
                                  int _line,
                                  ErrCmdType _err_type,
                                  int _err_code,
                                  const std::string& _ip,
    ...
    void __OnPush(const std::string& _channel_id,
                  uint32_t _cmdid,
                  uint32_t _taskid,
 ...
 public:
    ...
    void SetGetRealHostFuncWithExtraInfo(
    ...
    void SetLongLinkGetRealHostFunc(
        std::function<size_t(const std::string& _user_id, std::vector<std::string>& _hostlist, bool _strict_match)>
            func);
    void SetLongLinkGetRealHostFuncWithExtraInfo(
    ...
    void SetShortLinkGetRealHostFunc(
        std::function<size_t(const std::string& _user_id, std::vector<std::string>& _hostlist, bool _strict_match)>
            func);
    void SetShortLinkGetRealHostFuncWithExtraInfo(
        std::function<size_t(const std::string& _user_id, std::vector<std::string>& _hostlist, bool _strict_match, const std::map<std::string, std::string>& extra_info)>
            func);
    ...
    void SetShortLinkTaskConnectionDetailWithExtraInfo(std::function<void(const int _error_type, const int _error_code, const int _use_ip_index, const std::map<std::string, std::string>& extra_info)> func);
