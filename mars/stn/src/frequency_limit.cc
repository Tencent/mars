...
bool FrequencyLimit::Check(const mars::stn::Task& _task, const void* _buffer, int _len, unsigned int& _span) {
    ...
    if (RUN_CLEAR_RECORDS_INTERVAL_MINUTE <= interval) {
        xdebug2(TSF "__ClearRecord interval=%0, timeCur=%1, itimeRecordClear=%2",
                interval,
                time_cur,
                itime_record_clear_);
    ...
    if (0 <= find_index) {
        ...
        if (!__CheckRecord(find_index)) {
            xerror2(TSF
                    "Anti-Avalanche had Catch Task, Task Info: ptr=%0, cmdid=%1, need_authed=%2, cgi:%3, "
                    "channel_select=%4, limit_flow=%5",
                    &_task,
                    _task.cmdid,
                    _task.need_authed,
                    _task.cgi,
        ...
        xdebug2(TSF
                "InsertRecord Task Info: ptr=%0, cmdid=%1, need_authed=%2, cgi:%3, channel_select=%4, limit_flow=%5",
                &_task,
                _task.cmdid,
                _task.need_authed,
                _task.cgi,
