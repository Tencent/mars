...
class ShortLinkInterface {
 public:
    ...
    virtual void SetNeedCustomPack(bool flag) = 0;
    virtual bool IsNeedCustomPack() const {
        return false;
    }

    virtual bool OnSingleRespHandle(ErrCmdType _err_type,
                                    int _err_code,
                                    int _fail_handle,
                                    size_t _resp_length,
    ...
    //    CallBack<boost::function<void(ShortLinkInterface* _worker, unsigned int _cached_size, unsigned int
    //    _total_size)>>
    //        OnRecv;
    ...
    std::function<void(intptr_t& _running_id)> on_delete_shortlink_and_erase_cmd_list_;
    ...
    //    std::function<int()> on_get_send_count_;
};
