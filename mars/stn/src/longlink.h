...
class LongLink {
 ...
 public:
    ...
    boost::function<void(const std::string& _name,
                         ...
                         const ConnectProfile& _info)>
        OnResponse;
    boost::function<void(int _line, ErrCmdType _errtype, int _errcode, const std::string& _ip, uint16_t _port)>
        fun_network_report_;
    boost::function<void(uint64_t _interval)> OnNoopAlarmSet;
    boost::function<void(bool _noop_timeout)> OnNoopAlarmReceived;

 public:
    LongLink(boot::Context* _context,
    ...
    void SetDnsFunc(
        const std::function<std::vector<std::string>(const std::string& _host, bool _longlink_host)>& _dns_func) {
 ...
 protected:
    ...
    bool __NoopResp(uint32_t _cmdid,
                    uint32_t _taskid,
                    AutoBuffer& _buf,
                    AutoBuffer& _extension,
                    comm::Alarm& _alarm,
