...
bool StnManagerJniCallback::MakesureAuthed(const std::string& _host, const std::string& _user_id) {
    jnienv_ptr env;
    return j2c_cast(c2j_call(jboolean, callback_inst_, kStnManagerJniCallback_MakesureAuthed, _user_id));
}
...
void StnManagerJniCallback::TrafficData(ssize_t _send, ssize_t _recv) {
    jnienv_ptr env;
    c2j_call(void, callback_inst_, kStnManagerJniCallback_TrafficData, _send, _recv);
}
...
std::vector<std::string> StnManagerJniCallback::OnNewDns(const std::string& host, bool longlink_host) {
    ...
    if (!host.empty()) {
        jobjectArray ip_strs = c2j_call(jobjectArray, callback_inst_, kStnManagerJniCallback_OnNewDns, host, longlink_host);
        if (ip_strs != NULL) {
...
void StnManagerJniCallback::OnPush(const std::string& _channel_id, uint32_t _cmdid, uint32_t _taskid, const AutoBuffer& _body, const AutoBuffer& _extend) {
    jnienv_ptr env;
    c2j_call(void, callback_inst_, kStnManagerJniCallback_OnPush, _channel_id, _cmdid, _taskid,_body, _extend);
...
bool StnManagerJniCallback::Req2Buf(uint32_t _taskid, void* const _user_context, const std::string& _user_id, AutoBuffer& outbuffer, AutoBuffer& extend, int& error_code, const int channel_select, const std::string& host) {
    jnienv_ptr env;
    return c2j_cast(c2j_call(jboolean, callback_inst_, kStnManagerJniCallback_Req2Buf, _taskid, _user_context, _user_id, outbuffer, extend, error_code, channel_select, host));
}
