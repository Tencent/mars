...
DEFINE_FIND_METHOD(KC2Java_reportBindCellularNetworkStatus, KC2Java, "reportBindCellularNetworkStatus", "(Z)V")
void StnManagerJniCallback::ReportBindCellularNetworkStatus(bool status) {
    xverbose_function(TSF "mars2");
    VarCache* cache_instance = VarCache::Singleton();
    ScopeJEnv scope_jenv(cache_instance->GetJvm());
    JNIEnv* env = scope_jenv.GetEnv();
    JNU_CallMethodByMethodInfo(env, callback_inst_, KC2Java_reportBindCellularNetworkStatus, status);
    xdebug2(TSF "status = %_", status);
}

DEFINE_FIND_METHOD(KC2Java_requestBindCellularNetworkPermission,
                   KC2Java,
                   "requestBindCellularNetworkPermission",
                   "(Z)V")
void StnManagerJniCallback::RequestBindCellularNetworkPermission(bool show) {
    xverbose_function(TSF "mars2");
    VarCache* cache_instance = VarCache::Singleton();
    ScopeJEnv scope_jenv(cache_instance->GetJvm());
    JNIEnv* env = scope_jenv.GetEnv();
    JNU_CallMethodByMethodInfo(env, callback_inst_, KC2Java_requestBindCellularNetworkPermission, show);
    xdebug2(TSF "show = %_", show);
}
