...
extern boost::signals2::signal<void(ErrCmdType _err_type, int _err_code, const std::string& _ip, uint16_t _port)>
...
#ifndef NATIVE_CALLBACK
DEFINE_FIND_STATIC_METHOD(KC2Java_onTaskEnd,
                          KC2Java,
                          "onTaskEnd",
...
int C2Java_OnTaskEnd(uint32_t _taskid,
    ...
    xverbose_function();
    xdebug2(TSF "recieve task profile: %_, %_, %_",
...
std::vector<std::string> C2Java_OnNewDns(const std::string& _host) {
    ...
    if (!_host.empty()) {
        jobjectArray ip_strs =
            (jobjectArray)JNU_CallStaticMethodByMethodInfo(env,
                                                           KC2Java_onNewDns,
...
bool C2Java_Req2Buf(uint32_t _taskid,
    ...
    CALL_NATIVE_CALLBACK_RETURN_FUN(
        Req2Buf(_taskid, _user_context, _user_id, _outbuffer, _extend, _error_code, _channel_select, _host),
        false);
#endif
...
int C2Java_Buf2Resp(uint32_t _taskid,
...
#ifdef NATIVE_CALLBACK
    CALL_NATIVE_CALLBACK_RETURN_FUN(
        Buf2Resp(_taskid, _user_context, _user_id, _inbuffer, _extend, _error_code, _channel_select),
        -1);
#endif
    ...
    jint ret = JNU_CallStaticMethodByMethodInfo(env,
                                                KC2Java_buf2Resp,
                                                (jint)_taskid,
                                                _user_context,
                                                resp_buf_jba,
...
bool C2Java_MakesureAuthed(const std::string& _host, const std::string& _user_id) {
    ...
    jboolean ret =
        JNU_CallStaticMethodByMethodInfo(env, KC2Java_makesureAuthed, ScopedJstring(env, _host.c_str()).GetJstr()).z;

    return ret;
};

#ifndef NATIVE_CALLBACK
DEFINE_FIND_STATIC_METHOD(KC2Java_getLongLinkIdentifyCheckBuffer,
                          KC2Java,
...
int C2Java_GetLonglinkIdentifyCheckBuffer(const std::string& _channel_id,
    ...
    xverbose_function();
#ifdef NATIVE_CALLBACK
    CALL_NATIVE_CALLBACK_RETURN_FUN(GetLonglinkIdentifyCheckBuffer(_channel_id, _identify_buffer, _buffer_hash, _cmdid),
                                    -1);
#endif
    VarCache* cache_instance = VarCache::Singleton();
    ScopeJEnv scope_jenv(cache_instance->GetJvm());
    ...
    jint ret = 0;
    ret = JNU_CallStaticMethodByMethodInfo(env,
...
#ifndef NATIVE_CALLBACK
DEFINE_FIND_STATIC_METHOD(KC2Java_onLongLinkIdentifyResp,
                          KC2Java,
...
bool C2Java_OnLonglinkIdentifyResponse(const std::string& _channel_id,
    ...
    xverbose_function();
#ifdef NATIVE_CALLBACK
    CALL_NATIVE_CALLBACK_RETURN_FUN(OnLonglinkIdentifyResponse(_channel_id, _response_buffer, _identify_buffer_hash),
                                    false);
#endif
    VarCache* cache_instance = VarCache::Singleton();
    ...
    jboolean ret = JNU_CallStaticMethodByMethodInfo(env,
                                                    KC2Java_onLongLinkIdentifyResp,
                                                    ScopedJstring(env, _channel_id.c_str()).GetJstr(),
                                                    data_jba,
                                                    hash_jba)
                       .z;
...
#ifndef NATIVE_CALLBACK
DEFINE_FIND_STATIC_METHOD(KC2Java_requestNetCheckShortLinkHosts,
                          KC2Java,
...
void C2Java_RequestNetCheckShortLinkHosts(std::vector<std::string>& _hostlist) {
    ...
    jobjectArray jobj_arr =
        (jobjectArray)JNU_CallStaticMethodByMethodInfo(env, KC2Java_requestNetCheckShortLinkHosts).l;
