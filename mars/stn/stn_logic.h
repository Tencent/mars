...
namespace stn{
    ...
    public:
        ...
        //底层获取task要发送的数据
        virtual bool Req2Buf(uint32_t taskid, void* const user_context, AutoBuffer& outbuffer, AutoBuffer& extend, int& error_code, const int channel_select) = 0;
        //底层回包返回给上层解析
        virtual int Buf2Resp(uint32_t taskid, void* const user_context, const AutoBuffer& inbuffer, const AutoBuffer& extend, int& error_code, const int channel_select) = 0;
        //任务执行结束
        virtual int  OnTaskEnd(uint32_t taskid, void* const user_context, int error_type, int error_code) = 0;
