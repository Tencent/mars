...
static void __initbind_baseprjevent() {
    ...
    GetSignalOnNetworkChange().connect(
        5,
        boost::bind(&onNetworkChange, &mars::comm::OnPlatformNetworkChange));  // define group 5
...
void (*SetShortlinkSvrAddr)(const uint16_t port, const std::string& debugip) = [](const uint16_t port,
...
void (*SetDebugIP)(const std::string& host, const std::string& ip) = [](const std::string& host,
                                                                        const std::string& ip) {
    /* mars2
...
void ReportBindCellularNetworkStatus(bool status, bool request) {
    StnManager* stn_manager = Context::CreateContext("default")->GetManager<StnManager>();
    xassert2(NULL != stn_manager, "mars2 stn_manager is empty.");
    if (stn_manager) {
        stn_manager->ReportBindCellularNetworkStatus(status, request);
    }
}
...
void OnShortLinkNetworkError(ErrCmdType _err_type,
                             int _err_code,
