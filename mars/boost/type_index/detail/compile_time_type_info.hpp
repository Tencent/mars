...
#elif defined(_MSC_VER) && !defined(__clang__) && !defined (BOOST_NO_CXX11_NOEXCEPT)
    // sizeof("const char *__cdecl mars_boost::detail::ctti<") - 1, sizeof(">::n(void) noexcept") - 1
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 1, "T = "); }
    ...
    // sizeof("static cha mars_boost::detail::ctti<T>::s() [with I = 40U, T = ") - 1, sizeof("]") - 1
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(57, 1, ""); }
    ...
    // sizeof("static constexpr char mars_boost::detail::ctti<T>::s() [with unsigned int I = 0u; } T = ") - 1, sizeof("]") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(81, 1, ""); }
#elif defined(__GNUC__) && (__GNUC__ >= 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static constexpr char mars_boost::detail::ctti<T>::s() [with unsigned int I = 0; } T = ") - 1, sizeof("]") - 1
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(0, 0, ""); }
    ...
    BOOST_CXX14_CONSTEXPR inline int constexpr_strcmp(const char *v1, const char *v2) noexcept {
        ...
        if (mars_boost::typeindex::detail::is_constant_string(v1) && mars_boost::typeindex::detail::is_constant_string(v2))
            return mars_boost::typeindex::detail::constexpr_strcmp_loop(v1, v2);
...
struct ctti {
    ...
    constexpr static const char* impl(::mars_boost::typeindex::detail::index_seq<Indexes...> ) noexcept {
        return ::mars_boost::typeindex::detail::cstring<s<Indexes>()...>::data_;
    }
    ...
    #else
        mars_boost::typeindex::detail::failed_to_get_function_name<T>();
    #endif

        mars_boost::typeindex::detail::assert_compile_time_legths<
            (size > mars_boost::typeindex::detail::skip().size_at_begin + mars_boost::typeindex::detail::skip().size_at_end + sizeof("const *") - 1)
        >();
        static_assert(!mars_boost::typeindex::detail::skip().until_runtime_length, "Skipping for GCC in C++14 mode is unsupported");

        typedef typename mars_boost::typeindex::detail::make_index_seq_impl<
            mars_boost::typeindex::detail::skip().size_at_begin,
            size - sizeof("const *") + 1 - mars_boost::typeindex::detail::skip().size_at_begin
        >::type idx_seq;
        return impl(idx_seq());
    }
#else
