...
{
    using std::get;
    mars_boost::hash_combine( seed, get<I>( v ) );

    mars_boost::hash_detail::hash_combine_tuple_like<I + 1>( seed, v );
}
...
inline std::size_t hash_tuple_like( T const& v )
{
    ...
    mars_boost::hash_detail::hash_combine_tuple_like<0>( seed, v );
...
{
    return mars_boost::hash_detail::hash_tuple_like( v );
}
