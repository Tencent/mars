...
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost{ namespace BOOST_REGEX_DETAIL_NS{
...
inline const char* BOOST_REGEX_CALL get_default_error_string(regex_constants::error_type n)
{
   ...
   return (n > ::mars_boost::regex_constants::error_unknown) ? s_default_error_messages[::mars_boost::regex_constants::error_unknown] : s_default_error_messages[n];
}
...
inline bool BOOST_REGEX_CALL is_combining_implementation(mars_boost::uint_least16_t c)
{
   const mars_boost::uint_least16_t combining_ranges[] = { 0x0300, 0x0361,
                           0x0483, 0x0486,
                           0x0903, 0x0903,
                           0x093E, 0x0940,
                           0x0949, 0x094C,
   ...
   const mars_boost::uint_least16_t* p = combining_ranges + 1;
   while (*p < c) p += 2;
   --p;
   if ((c >= *p) && (c <= *(p + 1)))
      return true;
...
inline bool is_separator(charT c)
{
   return BOOST_REGEX_MAKE_BOOL(
      ...
      || (static_cast<mars_boost::uint16_t>(c) == 0x2028u)
      || (static_cast<mars_boost::uint16_t>(c) == 0x2029u)
      || (static_cast<mars_boost::uint16_t>(c) == 0x85u));
}
template <>
...
mars_boost::intmax_t global_toi(const charT*& p1, const charT* p2, int radix, const traits& t)
{
   (void)t; // warning suppression
   mars_boost::intmax_t limit = (std::numeric_limits<mars_boost::intmax_t>::max)() / radix;
   mars_boost::intmax_t next_value = t.value(*p1, radix);
   if((p1 == p2) || (next_value < 0) || (next_value >= radix))
      return -1;
   mars_boost::intmax_t result = 0;
   while(p1 != p2)
   {
...
template <class charT>
inline typename mars_boost::enable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
...
template <class charT>
inline typename mars_boost::disable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
