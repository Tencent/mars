...
// error checking API:
//
inline void BOOST_REGEX_CALL verify_options(mars_boost::regex_constants::syntax_option_type, match_flag_type mf)
{
   //
...
{   
   ...
   const ::mars_boost::regex_traits_wrapper<traits_type>& traits_inst = *(e.m_ptraits);
...
private:
   ...
   {
      return ::mars_boost::is_random_access_iterator<BidiIterator>::value ? match_dot_repeat_fast() : match_dot_repeat_slow();
   }
   bool match_backstep();
...
private:
   ...
   const ::mars_boost::regex_traits_wrapper<traits>& traits_inst;
