...
BOOST_NORETURN void throw_exception( std::exception const & e, mars_boost::source_location const & loc ); // user defined

#endif

// mars_boost::wrapexcept<E>
...
template<class E> struct BOOST_SYMBOL_VISIBLE wrapexcept:
    public detail::wrapexcept_add_base<E, mars_boost::exception_detail::clone_base>::type,
    public E,
    public detail::wrapexcept_add_base<E, mars_boost::exception>::type
{
private:
...
private:
    ...
    void copy_from( mars_boost::exception const* p )
    {
        static_cast<mars_boost::exception&>( *this ) = *p;
    }
...
public:
    ...
    explicit wrapexcept( E const & e, mars_boost::source_location const & loc ): E( e )
    {
        copy_from( &e );
    ...
    virtual mars_boost::exception_detail::clone_base const * clone() const BOOST_OVERRIDE
    {
        wrapexcept * p = new wrapexcept( *this );
        deleter del = { p };

        mars_boost::exception_detail::copy_boost_exception( p, this );
...
template<class E> BOOST_NORETURN void throw_exception( E const & e, mars_boost::source_location const & )
{
    throw_exception_assert_compatibility( e );
    throw e;
}
...
template<class E> BOOST_NORETURN void throw_exception( E const & e, mars_boost::source_location const & loc )
{
    throw_exception_assert_compatibility( e );
...
#define BOOST_THROW_EXCEPTION(x) ::mars_boost::throw_exception(x, BOOST_CURRENT_LOCATION)
...
{
    mars_boost::source_location location_;

    explicit throw_location( mars_boost::source_location const & loc ): location_( loc )
    {
    }
};
...
public:
    ...
    with_throw_location( E const & e, mars_boost::source_location const & loc ): E( e ), throw_location( loc )
    {
    }

#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES)

    with_throw_location( E && e, mars_boost::source_location const & loc ): E( std::move( e ) ), throw_location( loc )
    {
    }
...
template<class E> BOOST_NORETURN void throw_with_location( E && e, mars_boost::source_location const & loc = BOOST_CURRENT_LOCATION )
{
    throw_exception_assert_compatibility( e );
...
template<class E> BOOST_NORETURN void throw_with_location( E const & e, mars_boost::source_location const & loc = BOOST_CURRENT_LOCATION )
{
    throw_exception_assert_compatibility( e );
...
template<class E> BOOST_NORETURN void throw_with_location( E const & e, mars_boost::source_location const & loc = BOOST_CURRENT_LOCATION )
{
    mars_boost::throw_exception( e, loc );
}
...
template<class E> mars_boost::source_location get_throw_location( E const & e )
{
    ...
    else if( mars_boost::exception const* px = dynamic_cast< mars_boost::exception const* >( &e ) )
    {
