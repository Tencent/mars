...
struct swap_selector<true>
{
    ...
    static void optional_swap ( optional<T>& x, optional<T>& y )
    {
        ...
        mars_boost::core::invoke_swap(x.get(), y.get());
...
# define BOOST_OPTIONAL_DETAIL_MOVE(EXPR_) mars_boost::move(EXPR_)
#else
...
struct swap_selector<false>
{
    template <class T>
    static void optional_swap ( optional<T>& x, optional<T>& y ) 
    //BOOST_NOEXCEPT_IF(::mars_boost::is_nothrow_move_constructible<T>::value && BOOST_NOEXCEPT_EXPR(mars_boost::core::invoke_swap(*x, *y)))
    {
        if (x)
        {
            if (y)
            {
                mars_boost::core::invoke_swap(*x, *y);
            }
            else
            {
