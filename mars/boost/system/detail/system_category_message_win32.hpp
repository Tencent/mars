...
inline mars_boost::winapi::UINT_ message_cp_win32()
{
#if defined(BOOST_SYSTEM_USE_UTF8)

    return mars_boost::winapi::CP_UTF8_;

#else

    return mars_boost::winapi::CP_ACP_;
...
inline char const * system_category_message_win32( int ev, char * buffer, std::size_t len ) BOOST_NOEXCEPT
{
    ...
    mars_boost::winapi::UINT_ const code_page = message_cp_win32();

    int r = 0;

#if !defined(BOOST_NO_ANSI_APIS)

    if( code_page == mars_boost::winapi::CP_ACP_ )
    {
        using namespace mars_boost::winapi;

        DWORD_ retval = mars_boost::winapi::FormatMessageA(
...

    {
        ...
        DWORD_ retval = mars_boost::winapi::FormatMessageW(
        ...
        if( retval != 0 )
        {
            r = mars_boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, buffer, static_cast<int>( len ), NULL, NULL );
            mars_boost::winapi::LocalFree( lpMsgBuf );
...
struct local_free
{
    ...
    ~local_free()
    {
        mars_boost::winapi::LocalFree( p_ );
    }
};
...
inline std::string system_category_message_win32( int ev )
{
    ...
    int r = mars_boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, 0, 0, NULL, NULL );
    ...
    r = mars_boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, &buffer[0], r, NULL, NULL );
