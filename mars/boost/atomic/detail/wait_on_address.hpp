...
typedef mars_boost::winapi::BOOL_ BOOST_WINAPI_WINAPI_CC
wait_on_address_t(
...
typedef mars_boost::winapi::VOID_ BOOST_WINAPI_WINAPI_CC
wake_by_address_t(mars_boost::winapi::PVOID_ addr);
...
BOOST_FORCEINLINE void ensure_wait_functions_initialized() BOOST_NOEXCEPT
{
    ...
    if (BOOST_LIKELY(once_flag_operations::load(wait_functions_once_flag.m_flag, mars_boost::memory_order_acquire) == 0u))
        return;
