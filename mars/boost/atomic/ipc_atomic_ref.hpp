...
public:
    ...
    static_assert(sizeof(value_type) > 0u, "mars_boost::ipc_atomic_ref<T> requires T to be a complete type");
#if !defined(BOOST_ATOMIC_DETAIL_NO_CXX11_IS_TRIVIALLY_COPYABLE)
    static_assert(atomics::detail::is_trivially_copyable< value_type >::value, "mars_boost::ipc_atomic_ref<T> requires T to be a trivially copyable type");
#endif
