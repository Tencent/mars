...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(mars_boost::declval<Lhs>() oper mars_boost::declval<Rhs>() ) optype; \
    typedef typename mars_boost::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _, _i )                     \
    template<typename T>                                        \
    binary_expr<ExprType,typename mars_boost::decay<T const>::type,  \
        op::name<ValType,typename mars_boost::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename mars_boost::decay<T const>::type Rhs;       \
