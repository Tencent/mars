...
public:
    ...
    void            setup( const const_string& stream_name,
                           mars_boost::function<void ()> const &cleaner_callback = mars_boost::function<void ()>() )
    {
        ...
        if( stream_name == "stderr" ) {
            m_stream = &std::cerr;
            if(cleaner_callback) {
                m_cleaner = mars_boost::make_shared<callback_cleaner>(cleaner_callback);
            }
            else {
                m_cleaner.reset();
            }
...
private:
    struct callback_cleaner {
        callback_cleaner(mars_boost::function<void ()> cleaner_callback)
        ...
        mars_boost::function<void ()> m_cleaner_callback;
        std::ofstream m_file;
    };

    // Data members
    mars_boost::shared_ptr<callback_cleaner>   m_cleaner;
