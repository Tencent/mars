...
public:
    void    operator()()    { TestCaseTemplate::run( (mars_boost::type<TestType>*)0 ); }
};
...
struct generate_test_case_4_type {
    ...
    void        operator()( mpl::identity<TestType> )
    {
        ...
        full_name += '<';
#if !defined(BOOST_NO_TYPEID) && !defined(BOOST_NO_RTTI)
        full_name += mars_boost::core::demangle(typeid(TestType).name()); // same as execution_monitor.ipp
#else
        ...
        typedef typename mars_boost::remove_reference<TestType>::type TestTypewoRef;
        if( mars_boost::is_const<TestTypewoRef>::value )
            full_name += "_const";
        if( mars_boost::is_volatile<TestTypewoRef>::value )
            full_name += "_volatile";
        if( mars_boost::is_rvalue_reference<TestType>::value )
            full_name += "_refref";
        else if( mars_boost::is_lvalue_reference<TestType>::value )
            full_name += "_ref";
...
class template_test_case_gen<
    TestCaseTemplate,
    C<parameter_pack...>,
    typename std::enable_if<!mars_boost::mpl::is_sequence<C<parameter_pack...>>::value>::type >
