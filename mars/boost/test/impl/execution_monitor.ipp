...
extract( mars_boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = mars_boost::get_error_info<ErrorInfo>( *ex );
...
report_error( execution_exception::error_code ec, mars_boost::exception const* be, char const* format, va_list* args )
{
...
report_error( execution_exception::error_code ec, mars_boost::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return mars_boost::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    mars_boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         mars_boost::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        mars_boost::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( mars_boost::function<int ()> const& F )
{
...
int
execution_monitor::execute( mars_boost::function<int ()> const& F )
{
    ...
    // mars_boost::exception (before std::exception, with extended diagnostic)
    catch( mars_boost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
                              "%s", mars_boost::diagnostic_information(ex).c_str() ); }
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<mars_boost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
struct forward {
    explicit    forward( mars_boost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    mars_boost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( mars_boost::function<void ()> const& F )
{
