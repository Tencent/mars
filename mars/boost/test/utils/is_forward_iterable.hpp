...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(mars_boost::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(mars_boost::declval<U&>())); // does not work with mars_boost::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(mars_boost::declval<U&>())); // does not work with mars_boost::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(mars_boost::declval<
        typename mars_boost::add_const<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename mars_boost::add_const<T>::type T_const;
    typedef typename mars_boost::add_pointer<T_const>::type const_iterator;
