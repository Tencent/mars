...
template <typename T, bool is_cstring = is_cstring_impl<typename mars_boost::decay<T>::type>::value >
...
struct deduce_cstring_transform_impl<T, true> {
    typedef typename mars_boost::add_const<
        typename mars_boost::remove_pointer<
            typename mars_boost::decay<T>::type
        >::type
    >::type U;
    typedef mars_boost::unit_test::basic_cstring<U> type;
};

template <typename T>
struct deduce_cstring_transform_impl< T, false > {
    typedef typename
        mars_boost::remove_const<
...
struct deduce_cstring_transform_impl< std::basic_string<T, std::char_traits<T> >, false > {
    typedef mars_boost::unit_test::basic_cstring<typename mars_boost::add_const<T>::type> type;
};
...
public:
    using type = stringview_cstring_helper<typename mars_boost::add_const<T>::type, sv_t>;
};
#endif
...
template<typename T, bool is_cstring = is_cstring<typename mars_boost::decay<T>::type>::value >
...
template<typename T>
struct is_cstring_comparable< mars_boost::unit_test::basic_cstring<T>, false > : public mpl::true_ {};
...
struct deduce_cstring_transform {
    typedef typename
        mars_boost::remove_const<
            typename mars_boost::remove_reference<T>::type
        >::type U;
    typedef typename ut_detail::deduce_cstring_transform_impl<typename mars_boost::decay<U>::type>::type type;
};
