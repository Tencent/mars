...
/// - exposing an interface to the test framework (as a mars_boost::unit_test::test_observer)
...
///   @c mars_boost::unit_test::unit_test_log.
///
/// Users/developers willing to implement their own formatter need to:
/// - implement a mars_boost::unit_test::unit_test_log_formatter that will output the desired format
/// - register the formatter during a eg. global fixture using the method @c set_formatter (though the framework singleton).
///
/// @warning this observer has a higher priority than the @ref mars_boost::unit_test::results_collector_t. This means
/// that the various @ref mars_boost::unit_test::test_results associated to each test unit may not be available at the time
/// the @c test_unit_start, @c test_unit_finish ... are called.
///
/// @see
/// - mars_boost::unit_test::test_observer
/// - mars_boost::unit_test::unit_test_log_formatter
...
#define BOOST_TEST_LOG_ENTRY( ll )                                                  \
    (::mars_boost::unit_test::unit_test_log                                              \
        << ::mars_boost::unit_test::log::begin( BOOST_TEST_L(__FILE__), __LINE__ ))(ll)  \
/**/
...
#define BOOST_TEST_FRAMEWORK_MESSAGE( M )                       \
   (::mars_boost::unit_test::unit_test_log                           \
        << ::mars_boost::unit_test::log::begin(                      \
                "boost.test framework",                         \
                0 ))                                     \
             ( ::mars_boost::unit_test::log_messages )               \
    << BOOST_TEST_LAZY_MSG( M )                                 \
/**/


#define BOOST_TEST_MESSAGE( M )                                 \
    BOOST_TEST_LOG_ENTRY( ::mars_boost::unit_test::log_messages )    \
    << BOOST_TEST_LAZY_MSG( M )                                 \
/**/
...
#define BOOST_TEST_PASSPOINT()                                  \
    ::mars_boost::unit_test::unit_test_log.set_checkpoint(           \
...
#define BOOST_TEST_CHECKPOINT( M )                              \
    ::mars_boost::unit_test::unit_test_log.set_checkpoint(           \
        BOOST_TEST_L(__FILE__),                                 \
        static_cast<std::size_t>(__LINE__),                     \
        (::mars_boost::wrap_stringstream().ref() << M).str() )       \
/**/
