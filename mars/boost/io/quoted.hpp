    ...
    Char delim)
{
    ...
    if (entry) {
        quoted_state<Char> state = mars_boost::io::detail::quoted_start(string,
            escape, delim);
        std::basic_streambuf<Char, Traits>& buf = *os.rdbuf();
        std::size_t width = static_cast<std::size_t>(os.width());
        if (width <= state.count) {
            if (!mars_boost::io::detail::quoted_put(buf, state.string, state.size,
            ...
            if (!mars_boost::io::detail::quoted_put(buf, state.string, state.size,
                    state.count, escape, delim) ||
                !mars_boost::io::detail::buffer_fill(buf, os.fill(),
                    width - state.count)) {
                return os;
            }
        } else if (!mars_boost::io::detail::buffer_fill(buf, os.fill(),
                width - state.count) ||
            !mars_boost::io::detail::quoted_put(buf, state.string, state.size,
                state.count, escape, delim)) {
            return os;
        }
        os.width(0);
...
{
    return mars_boost::io::detail::quoted_out(os, proxy.string, proxy.escape,
        proxy.delim);
}
