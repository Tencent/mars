...
public:
    ...
    BOOST_CONSTEXPR shared_ptr( mars_boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT : px( 0 ), pn()
    {
    }

#endif

    BOOST_CONSTEXPR shared_ptr( mars_boost::detail::sp_internal_constructor_tag, element_type * px_, mars_boost::detail::shared_count const & pn_ ) BOOST_SP_NOEXCEPT : px( px_ ), pn( pn_ )
    {
    }

#if !defined( BOOST_NO_CXX11_RVALUE_REFERENCES )

    BOOST_CONSTEXPR shared_ptr( mars_boost::detail::sp_internal_constructor_tag, element_type * px_, mars_boost::detail::shared_count && pn_ ) BOOST_SP_NOEXCEPT : px( px_ ), pn( std::move( pn_ ) )
    {
    }
    ...
    template<class D> shared_ptr( mars_boost::detail::sp_nullptr_t p, D d ): px( p ), pn( p, static_cast< D&& >( d ) )
    {
    }
    ...
    template<class D, class A> shared_ptr( mars_boost::detail::sp_nullptr_t p, D d, A a ): px( p ), pn( p, static_cast< D&& >( d ), a )
    {
    }
...
// ... except in C++0x, move disables the implicit copy
    ...
    BOOST_SP_NOEXCEPT : px( 0 ), pn( r.pn, mars_boost::detail::sp_nothrow_tag() )
    {
        if( !pn.empty() )
        {
            px = r.px;
...
// Move support
    ...
    shared_ptr( shared_ptr && r ) BOOST_SP_NOEXCEPT : px( r.px ), pn( static_cast< mars_boost::detail::shared_count && >( r.pn ) )
    {
        r.px = 0;
    }
...
#endif
    BOOST_SP_NOEXCEPT : px( r.px ), pn( static_cast< mars_boost::detail::shared_count && >( r.pn ) )
    {
        mars_boost::detail::sp_assert_convertible< Y, T >();
        r.px = 0;
    }
    ...
    shared_ptr & operator=( mars_boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
    {
        this_type().swap(*this);
        return *this;
    }
    ...
    typename mars_boost::detail::sp_dereference< T >::type operator* () const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        return *px;
    }
    
    typename mars_boost::detail::sp_member_access< T >::type operator-> () const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        return px;
    }
...
#include <boost/smart_ptr/detail/operator_bool.hpp>
    ...
    void * _internal_get_deleter( mars_boost::detail::sp_typeinfo_ const & ti ) const BOOST_SP_NOEXCEPT
    {
        return pn.get_deleter( ti );
    }

    void * _internal_get_local_deleter( mars_boost::detail::sp_typeinfo_ const & ti ) const BOOST_SP_NOEXCEPT
    {
    ...
    mars_boost::detail::shared_count _internal_count() const BOOST_SP_NOEXCEPT
    {
        return pn;
    }
...
template<class T> inline bool operator==( shared_ptr<T> const & p, mars_boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( mars_boost::detail::sp_nullptr_t, shared_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
{
    D * d = mars_boost::detail::basic_get_deleter<D>( p );

    if( d == 0 )
    {
        d = mars_boost::detail::basic_get_local_deleter( d, p );
    }

    if( d == 0 )
    {
        mars_boost::detail::esft2_deleter_wrapper *del_wrapper = mars_boost::detail::basic_get_deleter<mars_boost::detail::esft2_deleter_wrapper>(p);
// The following get_deleter method call is fully qualified because
// older versions of gcc (2.95, 3.2.3) fail to compile it when written del_wrapper->get_deleter<D>()
        if(del_wrapper) d = del_wrapper->::mars_boost::detail::esft2_deleter_wrapper::get_deleter<D>();
    }
...
template< class T > std::size_t hash_value( mars_boost::shared_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
...
template<class T> struct hash< ::mars_boost::shared_ptr<T> >
{
    std::size_t operator()( ::mars_boost::shared_ptr<T> const & p ) const BOOST_SP_NOEXCEPT
    {
        return std::hash< typename ::mars_boost::shared_ptr<T>::element_type* >()( p.get() );
    }
};
