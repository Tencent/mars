...
{
    typedef mars_boost::local_shared_ptr<T> type;
};
...
template<class T, class A, class... Args> typename mars_boost::detail::lsp_if_not_array<T>::type allocate_local_shared( A const & a, Args&&... args )
{
#if !defined( BOOST_NO_CXX11_ALLOCATOR )
    ...
    typedef mars_boost::detail::lsp_ms_deleter<T, A2> D;
    ...
    return mars_boost::local_shared_ptr<T>( mars_boost::detail::lsp_internal_constructor_tag(), pt2, pd );
}

template<class T, class A> typename mars_boost::detail::lsp_if_not_array<T>::type allocate_local_shared_noinit( A const & a )
{
#if !defined( BOOST_NO_CXX11_ALLOCATOR )
    ...
    typedef mars_boost::detail::lsp_ms_deleter< T, std::allocator<T> > D;
...
template<class T, class... Args> typename mars_boost::detail::lsp_if_not_array<T>::type make_local_shared( Args&&... args )
{
    typedef typename mars_boost::remove_const<T>::type T2;
    return mars_boost::allocate_local_shared<T2>( std::allocator<T2>(), std::forward<Args>(args)... );
}

template<class T> typename mars_boost::detail::lsp_if_not_array<T>::type make_local_shared_noinit()
{
    typedef typename mars_boost::remove_const<T>::type T2;
    return mars_boost::allocate_shared_noinit<T2>( std::allocator<T2>() );
}
