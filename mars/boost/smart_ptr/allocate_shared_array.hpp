...
struct sp_array_alignment {
    enum {
        value = sp_max_size<mars_boost::alignment_of<T>::value,
            mars_boost::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename mars_boost::type_with_alignment<sp_array_alignment<T,
...
private:
    typename mars_boost::allocator_rebind<A, type>::type other_;
    std::size_t size_;
};
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        mars_boost::alloc_construct_n(state_.allocator(),
            mars_boost::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        mars_boost::alloc_construct_n(state_.allocator(),
            mars_boost::first_scalar(start), state_.size() * count,
            mars_boost::first_scalar(&list), count);
    }
    ...
    void dispose() BOOST_SP_NOEXCEPT BOOST_OVERRIDE {
        mars_boost::alloc_destroy_n(state_.allocator(),
            mars_boost::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return mars_boost::allocate_shared<T>(mars_boost::noinit_adapt(allocator), count);
}
...
{
    return mars_boost::allocate_shared<T>(mars_boost::noinit_adapt(allocator));
}
