...
public:
    ...
    const mars_boost::core::typeinfo& component_type(int n) const
    {
        if (static_cast<size_type>(n) >= size())
            mars_boost::throw_exception(std::out_of_range("bad chain offset"));
        return (*mars_boost::next(list().begin(), n))->component_type();
    }

    // Deprecated.
    template<int N>
    const mars_boost::core::typeinfo& component_type() const { return component_type(N); }

    template<typename T>
    T* component(int n) const { return component(n, mars_boost::type<T>()); }
...
#endif
    template<typename T>
    T* component(int n, mars_boost::type<T>) const
    {
        if (static_cast<size_type>(n) >= size())
            mars_boost::throw_exception(std::out_of_range("bad chain offset"));
        streambuf_type* link = *mars_boost::next(list().begin(), n);
...
private:
                   ...
                   std::streamsize pback_size = -1)
    {
        ...
        if (is_complete())
            mars_boost::throw_exception(std::logic_error("chain complete"));
        ...
        void close()
            {
                if ((flags_ & f_open) != 0) {
                    ...
                    try {
                        mars_boost::iostreams::detail::execute_foreach(
...
#define BOOST_IOSTREAMS_DECL_CHAIN(name_, default_char_) \
    ...
    class name_ : public mars_boost::iostreams::detail::chain_base< \
    ...
    public: \
        ...
        typedef Mode                                   mode; \
    private: \
        typedef mars_boost::iostreams::detail::chain_base< \
                    name_<Mode, Ch, Tr, Alloc>, \
                    Ch, Tr, Alloc, Mode \
                >                                      base_type; \
    public: \
...
public:
    ...
    // Deprecated.
    template<int N>
    const mars_boost::core::typeinfo& component_type() const
