...
protected:
    ...
    const mars_boost::core::typeinfo& component_type() const { return BOOST_CORE_TYPEID(T); }
...
direct_streambuf<T, Tr>::pbackfail(int_type c)
{
    ...
    mars_boost::throw_exception(bad_putback());
}
...
direct_streambuf<T, Tr>::overflow(int_type c)
{
    using namespace std;
    if (!obeg_)
        mars_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("no write access"));
...
void direct_streambuf<T, Tr>::close_impl(BOOST_IOS::openmode which)
{
    ...
    mars_boost::iostreams::close(*storage_, which);
}
