 ...
 * mars_boost::iostreams::detail::execute_all() and the function template 
...
template< typename Op, 
          typename Result = // VC6.5 workaround.
                  ...
                  typename mars_boost::result_of<Op()>::type
              #else
...
#define BOOST_PP_LOCAL_MACRO(n) \
   ...
   { \
       ...
       try { \
           r = mars_boost::iostreams::detail::execute_all( \
...
Op execute_foreach(InIt first, InIt last, Op op)
{
    ...
    try {
        ...
        try {
            ++first;
            mars_boost::iostreams::detail::execute_foreach(first, last, op);
        } catch (...) { }
        throw;
    }
    ++first;
    return mars_boost::iostreams::detail::execute_foreach(first, last, op);
}
