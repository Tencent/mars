 ...
 * class template mars_boost::iostreams::restriction. If included with the macro
 * BOOST_IOSTREAMS_RESTRICT defined as an identifier, defines the overloaded
 * function template mars_boost::iostreams::BOOST_IOSTREAMS_RESTRICT, and object 
 * generator for mars_boost::iostreams::restriction.
 *
        ...
        if (end_ != -1 && pos_ + n >= end_) {
            if(pos_ < end_)
                pos_ += iostreams::write(this->component(),
                    snk, s, end_ - pos_);
            mars_boost::throw_exception(bad_write());
        }
        std::streamsize result = 
