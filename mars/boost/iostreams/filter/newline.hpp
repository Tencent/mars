...
public:
    ...
    explicit newline_filter(int target) : flags_(target)
    {
        ...
        {
            mars_boost::throw_exception(std::logic_error("bad flags"));
        }
    }
...
private:
    ...
    bool newline(Sink& dest)
    {
        ...
        switch (flags_ & iostreams::newline::platform_mask) {
        case iostreams::newline::posix:
            success = mars_boost::iostreams::put(dest, LF);
            break;
        case iostreams::newline::mac:
            success = mars_boost::iostreams::put(dest, CR);
            break;
        case iostreams::newline::dos:
            if ((flags_ & f_has_LF) != 0) {
                success = mars_boost::iostreams::put(dest, LF);
                if (success)
                    flags_ &= ~f_has_LF;
            } else if (mars_boost::iostreams::put(dest, CR)) {
                success = mars_boost::iostreams::put(dest, LF);
                if (!success)
                    flags_ |= f_has_LF;
            }
...
private:
    void fail() { mars_boost::throw_exception(newline_error(source())); }
