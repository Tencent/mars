...
public:
    ...
    {
        composite_device<filter_ref, Source> cmp(mars_boost::ref(filter2_), src);
    ...
    {
        composite_device<filter_ref, Sink> cmp(mars_boost::ref(filter2_), snk);
    ...
    std::streampos seek( Device& dev, stream_offset off, BOOST_IOS::seekdir way,
                         BOOST_IOS::openmode which =
                             BOOST_IOS::in | BOOST_IOS::out )
    {
        composite_device<filter_ref, Device> cmp(mars_boost::ref(filter2_), dev);
