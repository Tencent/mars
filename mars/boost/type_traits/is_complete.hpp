...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::mars_boost::is_function<typename mars_boost::remove_reference<T>::type>::value || (sizeof(mars_boost::detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail
   {
      ...
      {
         template <class U, class = decltype(sizeof(mars_boost::declval< U >())) >
   ...
   template <class T>
   struct is_complete : mars_boost::integral_constant<bool, ::mars_boost::is_function<typename mars_boost::remove_reference<T>::type>::value || ::mars_boost::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : mars_boost::is_complete<T> {};
   
#else

      template <class T> struct is_complete
         : public mars_boost::integral_constant<bool, true> {};
