...
//
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost{ namespace type_traits_detail{
...
{
  template<typename Q> static mars_boost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static mars_boost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename mars_boost::enable_if_<has_one_operator_call<T>::value>::type> :
     mars_boost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public mars_boost::integral_constant<bool, false> {};
}}
