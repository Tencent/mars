...
//      great as mars_boost::intmax_t can cause some differences to be
...
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost { namespace detail {
  ...
  struct digit_traits<T, false>
  {
      BOOST_STATIC_CONSTANT(int, digits = (
          ...
          - (mars_boost::is_signed<T>::value ? 1 : 0))
          );
  };
#endif

  // Template class integer_traits<Integer> -- traits of various integer types
  // This should probably be rolled into mars_boost::integer_traits one day, but I
      ...
      typedef typename mars_boost::conditional<
        (int(x::is_signed)
          && (!int(x::is_bounded)
             ...
             || (int(x::digits) + 1 >= digit_traits<mars_boost::intmax_t>::digits))),
        Integer,
...
#else
      BOOST_STATIC_ASSERT(mars_boost::is_integral<Integer>::value);

      typedef typename mars_boost::conditional<
        (sizeof(Integer) >= sizeof(intmax_t)),

        mars_boost::conditional<
          (mars_boost::is_signed<Integer>::value),
          Integer,
          mars_boost::intmax_t
        >,
