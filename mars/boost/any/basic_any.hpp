...
/// \brief \copybrief mars_boost::anys::basic_any
...
namespace anys {
    ...
    /// \forcedlink{ValueType} requirements. Use mars_boost::any instead if not sure.
    ///
    /// mars_boost::anys::basic_any is the drop-in replacement for mars_boost::any
    /// that provides control over Small Object Optimization via
    /// `OptimizeForSize` and `OptimizeForAlignment` template parameters.
    ///
    /// There are certain applications that require mars_boost::any
    ...
    /// such applications may create a typedef for mars_boost::anys::basic_any
    ...
    ///
    /// Otherwise just use mars_boost::any.
    ...
    private:
        ...
        static void* small_manager(operation op, basic_any& left, const basic_any* right, const mars_boost::typeindex::type_info* info)
        {
            switch (op)
            {
                case Destroy:
                ...
                case Move: {
                    ...
                    BOOST_ASSERT(right->type() == mars_boost::typeindex::type_id<ValueType>());
                ...
                case AnyCast:
                    BOOST_ASSERT(info);
                    BOOST_ASSERT(!left.empty());
                    return mars_boost::typeindex::type_id<ValueType>() == *info ?
                ...
                case Typeinfo:
                    return const_cast<void*>(static_cast<const void*>(&mars_boost::typeindex::type_id<ValueType>().type_info()));
            }
        ...
        template <typename ValueType>
        static void* large_manager(operation op, basic_any& left, const basic_any* right, const mars_boost::typeindex::type_info* info)
        {
            switch (op)
            {
                case Destroy:
    ...
    public: // structors
        ...
        {
            static_assert(
                !std::is_same<ValueType, mars_boost::any>::value,
                "mars_boost::anys::basic_any shall not be constructed from mars_boost::any"
            );
            static_assert(
                !anys::detail::is_basic_any<ValueType>::value,
                "mars_boost::anys::basic_any<A, B> shall not be constructed from mars_boost::anys::basic_any<C, D>"
            );
          ...
          : man(0), content()
        {
            ...
            static_assert(
                !std::is_same<DecayedType, mars_boost::any>::value,
                "mars_boost::anys::basic_any shall not be constructed from mars_boost::any"
            );
            static_assert(
    ...
    public: // modifiers
        ...
        basic_any & operator=(ValueType&& rhs)
        {
            ...
            static_assert(
                !std::is_same<DecayedType, mars_boost::any>::value,
                "mars_boost::any shall not be assigned into mars_boost::anys::basic_any"
            );
            static_assert(
                !anys::detail::is_basic_any<DecayedType>::value || std::is_same<DecayedType, basic_any>::value,
                "mars_boost::anys::basic_any<A, B> shall not be assigned into mars_boost::anys::basic_any<C, D>"
            );
    ...
    public: // queries
        ...
        {
            return man
                    ? *static_cast<const mars_boost::typeindex::type_info*>(man(Typeinfo, const_cast<basic_any&>(*this), 0, 0))
                    : mars_boost::typeindex::type_id<void>().type_info();
        }
    ...
    private: // representation
        ...
        typedef void*(*manager)(operation op, basic_any& left, const basic_any* right, const mars_boost::typeindex::type_info* info);
    ...
    {
        return operand->man ?
                static_cast<typename std::remove_cv<ValueType>::type *>(operand->man(basic_any<Size, Alignment>::AnyCast, *operand, 0, &mars_boost::typeindex::type_id<ValueType>().type_info()))
                : 0;
    }
    ...
    {
        return mars_boost::anys::any_cast<ValueType>(const_cast<basic_any<OptimizeForSize, OptimizeForAlignment> *>(operand));
    }

    /// \returns ValueType stored in `operand`
    /// \throws mars_boost::bad_any_cast if `operand` does not contain
    /// specified ValueType.
    template<typename ValueType, std::size_t OptimizeForSize, std::size_t OptimizeForAlignment>
    ValueType any_cast(basic_any<OptimizeForSize, OptimizeForAlignment> & operand)
    {
        ...
        nonref * result = mars_boost::anys::any_cast<nonref>(std::addressof(operand));
        if(!result)
...
#   pragma warning(pop)
    ...
    {
        typedef typename std::remove_reference<ValueType>::type nonref;
        return mars_boost::anys::any_cast<const nonref &>(const_cast<basic_any<OptimizeForSize, OptimizeForAlignment> &>(operand));
    }
    ...
    inline ValueType any_cast(basic_any<OptimizeForSize, OptimizeForAlignment>&& operand)
    {
        ...
        return mars_boost::anys::any_cast<ValueType>(operand);
    }
    ...
    {
        return mars_boost::anys::unsafe_any_cast<ValueType>(const_cast<basic_any<OptimizeForSize, OptimizeForAlignment> *>(operand));
    }
    /// @endcond

} // namespace anys

using mars_boost::anys::any_cast;
using mars_boost::anys::unsafe_any_cast;
