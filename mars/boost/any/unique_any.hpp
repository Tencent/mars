...
/// \brief \copybrief mars_boost::anys::unique_any
...
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost { namespace anys {
...
public:
    ...
    template<typename T>
    unique_any(T&& value, typename std::enable_if<!std::is_same<T&&, mars_boost::any&&>::value>::type* = nullptr)
      : content(new holder< typename std::decay<T>::type >(std::forward<T>(value)))
    {
        static_assert(
            !mars_boost::anys::detail::is_basic_any< typename std::decay<T>::type >::value,
            "mars_boost::anys::unique_any could not be constructed from mars_boost::anys::basic_any."
        );

        static_assert(
            !std::is_same<unique_any, typename std::decay<T>::type >::value,
            "mars_boost::anys::unique_any could not be copied, only moved."
        );

        static_assert(
            !std::is_same<mars_boost::any, typename std::decay<T>::type >::value,
            "mars_boost::anys::unique_any could be constructed from an rvalue of mars_boost::any, "
            "not a lvalue."
        );
    }

    /// Moves the content of `mars_boost::any` into *this.
    ///
    /// \throws Nothing.
    /// \post `value.empty()` is true.
    template <class BoostAny>
    unique_any(BoostAny&& value, typename std::enable_if<std::is_same<BoostAny&&, mars_boost::any&&>::value>::type* = nullptr) noexcept
    {
    ...
    typename std::decay<T>::type& emplace(Args&&... args) {
        ...
        content = std::unique_ptr<mars_boost::anys::detail::placeholder>(raw_ptr);
        return raw_ptr->held;
    }
    ...
    /// only at runtime.
    const mars_boost::typeindex::type_info& type() const noexcept
    {
...
private: // types
    /// @cond
    template<typename T>
    class holder final: public mars_boost::anys::detail::placeholder
    {
    public:
        template <class... Args>
        holder(Args&&... args)
        ...
        const mars_boost::typeindex::type_info& type() const noexcept override
        {
...
private: // representation
    ...
    std::unique_ptr<mars_boost::anys::detail::placeholder> content;
    /// @endcond
};
...
{
    return operand && operand->type() == mars_boost::typeindex::type_id<T>()
...
/// \throws mars_boost::bad_any_cast if `operand` does not contain specified `T`.
template<typename T>
