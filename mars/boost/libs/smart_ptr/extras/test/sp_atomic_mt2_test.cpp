...
static mars_boost::shared_ptr<X> ps;
...
void write_access( prim_type pt )
{
    ...
    case pt_atomics:
        {
            mars_boost::shared_ptr<X> p1 = mars_boost::atomic_load( &ps );

            for( ;; )
            {
                mars_boost::shared_ptr<X> p2( new X( *p1 ) );
                p2->set();

                if( mars_boost::atomic_compare_exchange( &ps, &p1, p2 ) ) break;
            }
        }
        break;
    }
...
int main( int ac, char const * av[] )
{
    ...
    std::vector<mars_boost::detail::lw_thread_t> a( m );

    for( int i = 0; i < m; ++i )
    {
        mars_boost::detail::lw_thread_create( a[ i ], mars_boost::bind( worker, i, pt, k, r ) );
    }
