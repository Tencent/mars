...
void test( std::vector< mars_boost::shared_ptr<int> > & v )
{
    using namespace std; // printf, rand

    std::vector< mars_boost::weak_ptr<int> > w( v.begin(), v.end() );
    ...
    for( int i = 0; i < n; ++i )
    {
        ...
        for( int j = 0; j < k; ++j )
        {
            if( mars_boost::shared_ptr<int> px = w[ j ].lock() )
            {
                ++s;
...
int main()
{
    ...
    std::vector< mars_boost::shared_ptr<int> > v( k );
    ...
    mars_boost::detail::lw_thread_t a[ m ];

    for( int i = 0; i < m; ++i )
    {
        mars_boost::detail::lw_thread_create( a[ i ], mars_boost::bind( test, v ) );
    }
