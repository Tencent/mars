...
int main()
{
    {
        myarg a;
        mars_boost::shared_ptr< X > x = mars_boost::make_shared< X >(a);
        BOOST_TEST( x->constructed_by_ == X::ref_constructor);
    }
    {
        const myarg ca;
        mars_boost::shared_ptr< X > x = mars_boost::make_shared< X >(ca);
        BOOST_TEST( x->constructed_by_ == X::const_ref_constructor);
    }
    {
        mars_boost::shared_ptr< X > x = mars_boost::make_shared< X >(myarg());
        BOOST_TEST( x->constructed_by_ == X::move_constructor);
    }
    {
        int value = 1;
        mars_boost::shared_ptr< Y > y = mars_boost::make_shared< Y >(value);
