...
{
    {
        std::unique_ptr<int,
            ...
            mars_boost::allocate_unique_noinit<int>(creator<int>());
    ...
    }
    {
        std::unique_ptr<const int,
            ...
            mars_boost::allocate_unique_noinit<const int>(creator<>());
        BOOST_TEST(result.get() != 0);
    }
    {
        std::unique_ptr<type,
            ...
            mars_boost::allocate_unique_noinit<type>(creator<type>());
        BOOST_TEST(result.get() != 0);
    ...
    }
    {
        std::unique_ptr<const type,
            ...
            mars_boost::noinit_adaptor<creator<> > > > result =
            mars_boost::allocate_unique_noinit<const type>(creator<>());
        BOOST_TEST(result.get() != 0);
        BOOST_TEST(type::instances == 1);
        result.reset();
        BOOST_TEST(type::instances == 0);
