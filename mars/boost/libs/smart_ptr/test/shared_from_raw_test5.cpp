...
public:
    ...
    explicit X( int expected, mars_boost::shared_ptr<X> *early_px = 0 ): destroyed_( 0 ), deleted_( 0 ), expected_( expected )
    {
        ++instances;
...
bool are_shared_owners(const mars_boost::shared_ptr<T> &a, const mars_boost::shared_ptr<U> &b)
{
    return !(a < b) && !(b < a);
}

struct Y: public mars_boost::enable_shared_from_raw
{};
...
int main()
{
    ...
    {
        mars_boost::shared_ptr<X> early_px;
        X* x = new X( 1, &early_px );
        BOOST_TEST( early_px.use_count() > 0 );
        BOOST_TEST( mars_boost::get_deleter<X::deleter_type>(early_px) == 0 );
        BOOST_TEST( early_px.get() == x );
        mars_boost::shared_ptr<X> px( x, &X::deleter2 );
        ...
        X::deleter_type *pd = mars_boost::get_deleter<X::deleter_type>(early_px);
...

    {
        mars_boost::shared_ptr<X> early_px;
        X* x = new X( 1, &early_px );
        mars_boost::weak_ptr<X> early_weak_px = early_px;
        early_px.reset();
...

    {
        ...
        X x( 2, &early_px );
        BOOST_TEST( early_px.use_count() > 0 );
        mars_boost::shared_ptr<X> px( &x, &X::deleter );
        BOOST_TEST( early_px.use_count() == 2 && px.use_count() == 2 );
        early_px.reset();
        BOOST_TEST( px.use_count() == 1 );
        BOOST_TEST( X::instances == 1 );
    ...
    {
        mars_boost::weak_ptr<X> early_weak_px;
        {
            mars_boost::shared_ptr<X> early_px;
            X x( 0, &early_px );
    ...
    {
        mars_boost::shared_ptr<Y> px(new Y());
        Y y(*px);
        px.reset();
        try
        {
