...
template<class T> void test(T * = 0)
{
    ...
    {
        std::vector< mars_boost::shared_ptr<T> > v;

        for(int i = 0; i < n; ++i)
        {
            mars_boost::shared_ptr<T> pi(new T(i));
            v.push_back(pi);
        }
...
public:
    ...
    {
        return mars_boost::detail::quick_allocator<Y>::alloc(n);
    }

    void operator delete(void * p, std::size_t n)
    {
        mars_boost::detail::quick_allocator<Y>::dealloc(p, n);
    }
