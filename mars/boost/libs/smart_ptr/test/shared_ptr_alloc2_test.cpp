...
int main()
{
    ...
    mars_boost::shared_ptr<void> pv( new X, mars_boost::checked_deleter<X>(), std::allocator<X>() );

    BOOST_TEST( X::instances == 1 );

    pv.reset( new X, mars_boost::checked_deleter<X>(), test_allocator<float>( 42 ) );
    ...
    pv.reset( new X, mars_boost::checked_deleter<X>(), test_allocator<void>( 43 ) );

    BOOST_TEST( X::instances == 1 );
    BOOST_TEST( test_allocator_base::last_global_id_ == 43 );

    pv.reset( new X, mars_boost::checked_deleter<X>(), std::allocator<void>() );
