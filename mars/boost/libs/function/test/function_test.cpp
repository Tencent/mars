...
{
  mars_boost::function<int (X*)> f1(&X::twice);
  ...
  mars_boost::function<int (X*)> f1_2;
  f1_2 = &X::twice;

  BOOST_CHECK(f1_2(&one) == 2);
  BOOST_CHECK(f1_2(&five) == 10);

  mars_boost::function<int (X&, int)> f2(&X::plus);
...
test_ref()
{
  ...
  try {
    mars_boost::function<int (int, int)> f(mars_boost::ref(atc));
    BOOST_CHECK(f(1, 3) == 4);
  }
...
static void test_empty_ref()
{
  mars_boost::function<void()> f1;
  mars_boost::function<void()> f2(mars_boost::ref(f1));
...
{
  mars_boost::function<int (int, int)> f;
  try {
    f(5, 4);
    BOOST_CHECK(false);
  }
  catch(mars_boost::bad_function_call const&) {
    // okay
  }
}

typedef mars_boost::function< void * (void * reader) > reader_type;
...
static void test_call_obj(mars_boost::function<int (int, int)> f)
{
  BOOST_CHECK(!f.empty());
}

static void test_call_cref(const mars_boost::function<int (int, int)>& f)
{
  BOOST_CHECK(!f.empty());
}
...
int main()
{
  ...
  test_move_semantics<mars_boost::function0<void> >();
