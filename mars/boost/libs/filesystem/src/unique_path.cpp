...
void system_crypt_random(void* buf, std::size_t len, mars_boost::system::error_code* ec)
{
    ...
    if (BOOST_UNLIKELY(err != 0))
        emit_error(err, ec, "mars_boost::filesystem::unique_path");
    ...
    {
    fail:
        emit_error(translate_ntstatus(status), ec, "mars_boost::filesystem::unique_path");
        return;
    }

    status = mars_boost::winapi::BCryptGenRandom(handle, static_cast< mars_boost::winapi::PUCHAR_ >(buf), static_cast< mars_boost::winapi::ULONG_ >(len), 0);
    ...
    mars_boost::winapi::HCRYPTPROV_ handle;
    mars_boost::winapi::DWORD_ err = 0u;
    if (BOOST_UNLIKELY(!mars_boost::winapi::CryptAcquireContextW(&handle, NULL, NULL, mars_boost::winapi::PROV_RSA_FULL_, mars_boost::winapi::CRYPT_VERIFYCONTEXT_ | mars_boost::winapi::CRYPT_SILENT_)))
    {
        err = mars_boost::winapi::GetLastError();
    ...
    mars_boost::winapi::BOOL_ gen_ok = mars_boost::winapi::CryptGenRandom(handle, static_cast< mars_boost::winapi::DWORD_ >(len), static_cast< mars_boost::winapi::BYTE_* >(buf));

    if (BOOST_UNLIKELY(!gen_ok))
        err = mars_boost::winapi::GetLastError();

    mars_boost::winapi::CryptReleaseContext(handle, 0);
