...
BOOST_FILESYSTEM_DECL void directory_entry::refresh_impl(system::error_code* ec) const
{
    ...
    if (!filesystem::is_symlink(m_symlink_status))
    {
        ...
        {
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("mars_boost::filesystem::directory_entry::refresh", m_path, local_ec));
...
error_code dir_itr_create(mars_boost::intrusive_ptr< detail::dir_itr_imp >& imp, fs::path const& dir, unsigned int opts, directory_iterator_params* params, fs::path& first_filename, fs::file_status&, fs::file_status&)
{
    ...
    mars_boost::intrusive_ptr< detail::dir_itr_imp > pimpl(new (extra_size) detail::dir_itr_imp());
    ...
    default:
        {
            ...
            if (data->NextEntryOffset == 0u)
            {
                ...
                mars_boost::winapi::NTSTATUS_ status = filesystem::detail::atomic_load_relaxed(nt_query_directory_file_api)
                (
                    imp.handle,
                    NULL, // Event
                    NULL, // ApcRoutine
...
error_code dir_itr_create(mars_boost::intrusive_ptr< detail::dir_itr_imp >& imp, fs::path const& dir, unsigned int opts, directory_iterator_params* params, fs::path& first_filename, fs::file_status& sf, fs::file_status& symlink_sf)
{
    mars_boost::intrusive_ptr< detail::dir_itr_imp > pimpl(new (dir_itr_extra_size) detail::dir_itr_imp());
    if (BOOST_UNLIKELY(!pimpl))
        return make_error_code(system::errc::not_enough_memory);
    ...
    fallback_to_file_directory_information_format:
        {
            NtQueryDirectoryFile_t* nt_query_directory_file = filesystem::detail::atomic_load_relaxed(mars_boost::filesystem::detail::nt_query_directory_file_api);
            if (BOOST_UNLIKELY(!nt_query_directory_file))
                return error_code(ERROR_NOT_SUPPORTED, system_category());

            io_status_block iosb;
            mars_boost::winapi::NTSTATUS_ status = nt_query_directory_file
            (
                iterator_handle,
                NULL, // Event
                NULL, // ApcRoutine
...
void directory_iterator_construct(directory_iterator& it, path const& p, unsigned int opts, directory_iterator_params* params, system::error_code* ec)
{
    ...
    {
        emit_error(not_found_error_code, p, ec, "mars_boost::filesystem::directory_iterator::construct");
        return;
    }
    ...
    BOOST_TRY
    {
        mars_boost::intrusive_ptr< detail::dir_itr_imp > imp;
        path filename;
        ...
        while (true)
        {
            if (result)
            {
                ...
                {
                    if (!ec)
                        BOOST_FILESYSTEM_THROW(filesystem_error("mars_boost::filesystem::directory_iterator::construct", p, result));
                    *ec = result;
                }
    ...
    BOOST_TRY
    {
        ...
        while (true)
        {
            ...
            if (BOOST_UNLIKELY(!!increment_ec)) // happens if filesystem is corrupt, such as on a damaged optical disc
            {
                ...
                if (!ec)
                    BOOST_FILESYSTEM_THROW(filesystem_error("mars_boost::filesystem::directory_iterator::operator++", error_path, increment_ec));
...
void recursive_directory_iterator_construct(recursive_directory_iterator& it, path const& dir_path, unsigned int opts, system::error_code* ec)
{
    ...
    mars_boost::intrusive_ptr< detail::recur_dir_itr_imp > imp;
    if (!ec)
    {
...
void recursive_directory_iterator_pop(recursive_directory_iterator& it, system::error_code* ec)
{
    ...
    while (true)
    {
        ...
        if (BOOST_UNLIKELY(!!increment_ec))
        {
            ...
            if (!ec)
                BOOST_FILESYSTEM_THROW(filesystem_error("mars_boost::filesystem::recursive_directory_iterator::pop", increment_ec));
