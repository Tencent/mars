...
using mars_boost::filesystem::detail::colon;
using mars_boost::filesystem::detail::questionmark;

inline bool is_alnum(wchar_t c)
{
    return mars_boost::filesystem::detail::is_letter(c) || (c >= L'0' && c <= L'9');
}
...
inline size_type find_separator(const wchar_t* p, size_type size) BOOST_NOEXCEPT
{
    ...
    for (; pos < size; ++pos)
    {
        ...
        if (mars_boost::filesystem::detail::is_directory_separator(c))
            break;
    }
    return pos;
}
...
std::locale default_locale()
{
    ...
    return std::locale(global_loc, new mars_boost::filesystem::detail::windows_file_codecvt());
#elif defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__) || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__HAIKU__)
...
const mars_boost::filesystem::path* g_dot_path = NULL;
const mars_boost::filesystem::path* g_dot_dot_path = NULL;

inline void schedule_path_locale_cleanup() BOOST_NOEXCEPT
{
}

inline mars_boost::filesystem::path const& get_dot_path()
{
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    atomic_ns::atomic_ref< const mars_boost::filesystem::path* > a(g_dot_path);
    const mars_boost::filesystem::path* p = a.load(atomic_ns::memory_order_acquire);
    if (BOOST_UNLIKELY(!p))
    {
        const mars_boost::filesystem::path* new_p = new mars_boost::filesystem::path(dot_path_literal);
        if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel, atomic_ns::memory_order_acquire))
            p = new_p;
        else
            delete new_p;
    ...
    return *p;
#else // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    const mars_boost::filesystem::path* p = g_dot_path;
    if (BOOST_UNLIKELY(!p))
        g_dot_path = p = new mars_boost::filesystem::path(dot_path_literal);
    return *p;
#endif // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
}

inline mars_boost::filesystem::path const& get_dot_dot_path()
{
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    atomic_ns::atomic_ref< const mars_boost::filesystem::path* > a(g_dot_dot_path);
    const mars_boost::filesystem::path* p = a.load(atomic_ns::memory_order_acquire);
    if (BOOST_UNLIKELY(!p))
    {
        const mars_boost::filesystem::path* new_p = new mars_boost::filesystem::path(dot_dot_path_literal);
        if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel, atomic_ns::memory_order_acquire))
            p = new_p;
        else
            delete new_p;
    ...
    return *p;
#else // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    const mars_boost::filesystem::path* p = g_dot_dot_path;
    if (BOOST_UNLIKELY(!p))
        g_dot_dot_path = p = new mars_boost::filesystem::path(dot_dot_path_literal);
    return *p;
#endif // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
}
...
BOOST_FILESYSTEM_INIT_FUNC init_path_globals()
{
    ...
    mars_boost::filesystem::codecvt_error_category();
#endif
...
const mars_boost::filesystem::path g_dot_path(dot_path_literal);
BOOST_FILESYSTEM_INIT_PRIORITY(BOOST_FILESYSTEM_PATH_GLOBALS_INIT_PRIORITY)
const mars_boost::filesystem::path g_dot_dot_path(dot_dot_path_literal);
...
inline mars_boost::filesystem::path const& get_dot_path()
{
    static const mars_boost::filesystem::path g_dot_path(dot_path_literal);
    return g_dot_path;
}

inline mars_boost::filesystem::path const& get_dot_dot_path()
{
    static const mars_boost::filesystem::path g_dot_dot_path(dot_dot_path_literal);
    return g_dot_dot_path;
}
