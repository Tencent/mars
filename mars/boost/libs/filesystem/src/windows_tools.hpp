...
inline mars_boost::filesystem::perms make_permissions(mars_boost::filesystem::path const& p, DWORD attr)
{
    mars_boost::filesystem::perms prms = mars_boost::filesystem::owner_read | mars_boost::filesystem::group_read | mars_boost::filesystem::others_read;
    if ((attr & FILE_ATTRIBUTE_READONLY) == 0u)
        prms |= mars_boost::filesystem::owner_write | mars_boost::filesystem::group_write | mars_boost::filesystem::others_write;
    mars_boost::filesystem::path ext = detail::path_algorithms::extension_v4(p);
    ...
    if (equal_extension(q, L".exe", L".EXE") || equal_extension(q, L".com", L".COM") || equal_extension(q, L".bat", L".BAT") || equal_extension(q, L".cmd", L".CMD"))
        prms |= mars_boost::filesystem::owner_exe | mars_boost::filesystem::group_exe | mars_boost::filesystem::others_exe;
    return prms;
}

ULONG get_reparse_point_tag_ioctl(HANDLE h, mars_boost::filesystem::path const& p, mars_boost::system::error_code* ec);
...
struct io_status_block
{
    union
    {
        mars_boost::winapi::NTSTATUS_ Status;
        PVOID Pointer;
    };
...
typedef mars_boost::winapi::NTSTATUS_ (NTAPI NtCreateFile_t)(
...
typedef mars_boost::winapi::NTSTATUS_ (NTAPI NtQueryDirectoryFile_t)(
...
inline HANDLE create_file_handle(mars_boost::filesystem::path const& p, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile = NULL)
{
