...
mbpath_traits::to_external(const mbpath& ph, const internal_string_type& src)
{
    ...
    mars_boost::scoped_array< char > work(new char[work_size]);
    std::mbstate_t state;
    ...
    if (cvt->out(
        ...
        mars_boost::throw_exception< fs::basic_filesystem_error< mbpath > >(
            fs::basic_filesystem_error< mbpath >(
                "user::mbpath::to_external conversion error",
                ph, mars_boost::system::error_code(EINVAL, mars_boost::system::errno_ecat)));
    *to_next = '\0';
...
mbpath_traits::to_internal(const external_string_type& src)
{
    ...
    mars_boost::scoped_array< wchar_t > work(new wchar_t[work_size]);
    std::mbstate_t state;
    ...
    if (cvt->in(
        ...
        mars_boost::throw_exception< fs::basic_filesystem_error< mbpath > >(
            fs::basic_filesystem_error< mbpath >(
                ...
                mars_boost::system::error_code(EINVAL, mars_boost::system::errno_ecat)));
    *to_next = L'\0';
