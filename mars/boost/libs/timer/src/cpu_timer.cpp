...
using mars_boost::timer::nanosecond_type;
using mars_boost::timer::cpu_times;
...
namespace
{
  ...
  //  be as low as 10, although will be 15 for many common platforms.
  {
    if (places > 9)
      ...
      places = mars_boost::timer::default_places;
 
    mars_boost::io::ios_flags_saver ifs(os);
    mars_boost::io::ios_precision_saver ips(os);
...
mars_boost::long_long_type query_performance_frequency()
{
    LARGE_INTEGER li;
...
void get_cpu_times( mars_boost::timer::cpu_times& current )
{
    static const mars_boost::long_long_type freq = query_performance_frequency();

    LARGE_INTEGER li;
    ::QueryPerformanceCounter( &li ); // never fails

    mars_boost::long_long_type ctr = li.QuadPart;

    mars_boost::long_long_type const nano = INT64_C( 1000000000 ); // ns

    // ctr * nano / freq, but with less overflow

    mars_boost::long_long_type whole = (ctr / freq) * nano;
    mars_boost::long_long_type part  = (ctr % freq) * nano / freq;

    current.wall = whole + part;
    current.user = mars_boost::timer::nanosecond_type( -1 );
    current.system = mars_boost::timer::nanosecond_type( -1 );
    ...
    current.system = ( ( mars_boost::timer::nanosecond_type( kernel.dwHighDateTime ) << 32 ) + kernel.dwLowDateTime ) * 100;
    current.user = ( ( mars_boost::timer::nanosecond_type( user.dwHighDateTime ) << 32 ) + user.dwLowDateTime ) * 100;
...
mars_boost::int_least64_t tick_factor()
{
    mars_boost::int_least64_t tf = ::sysconf( _SC_CLK_TCK );
    if( tf <= 0 ) return -1;
...
void get_cpu_times( mars_boost::timer::cpu_times& current )
{
    current.wall = mars_boost::timer::nanosecond_type( -1 );
    current.user = mars_boost::timer::nanosecond_type( -1 );
    current.system = mars_boost::timer::nanosecond_type( -1 );

    static mars_boost::int_least64_t tf = tick_factor();
    ...
    current.wall = mars_boost::timer::nanosecond_type( c ) * tf;
    current.system = mars_boost::timer::nanosecond_type( tm.tms_stime + tm.tms_cstime ) * tf;
    current.user = mars_boost::timer::nanosecond_type( tm.tms_utime + tm.tms_cutime ) * tf;
}
