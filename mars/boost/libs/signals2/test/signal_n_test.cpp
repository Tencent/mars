...
test_zero_args()
{
  ...
  {
    mars_boost::signals2::signal0<int, max_or_default<int>, std::string> s0;
    mars_boost::signals2::connection c2 = s0.connect(i2);
    mars_boost::signals2::connection c72 = s0.connect("72", i72);
    mars_boost::signals2::connection c62 = s0.connect("6x", i62);
    mars_boost::signals2::connection c42 = s0.connect(i42);
    mars_boost::signals2::connection c37 = s0.connect(&get_37);
  ...
  {
    mars_boost::signals2::signal0<int, max_or_default<int> > s0;
    mars_boost::signals2::connection c2 = s0.connect(i2);
    mars_boost::signals2::connection c72 = s0.connect(i72);
    mars_boost::signals2::connection c62 = s0.connect(i62);
    mars_boost::signals2::connection c42 = s0.connect(i42);

    const mars_boost::signals2::signal0<int, max_or_default<int> >& cs0 = s0;
    BOOST_CHECK(cs0() == 72);
  }
...
test_one_arg()
{
  mars_boost::signals2::signal1<int, int, max_or_default<int> > s1;
...
{
  typedef mars_boost::signals2::signal1<int, int, max_or_default<int> > signal_type;
  signal_type s1;
...
{
  EventCounter ec;
  mars_boost::signals2::signal0<void> s;

  {
    mars_boost::signals2::scoped_connection c(s.connect(mars_boost::ref(ec)));
...
{
  mars_boost::signals2::signal0<int> sig;
  mars_boost::optional<int> result;
  result = sig();
  BOOST_CHECK(!result);
...
{
  {
    typedef mars_boost::signals2::signal1<ResultType, int> signal_type;
  ...
  { // test 0 arg signal
    typedef mars_boost::signals2::signal0<ResultType> signal_type;
    typedef typename signal_type::extended_slot_type slot_type;
    signal_type sig;
    // attempting to work around msvc 7.1 bug by explicitly assigning to a function pointer
    ResultType (*fp)(const mars_boost::signals2::connection &conn, int) = &disconnecting_slot<ResultType>;
...
test_set_combiner()
{
  typedef mars_boost::signals2::signal0<int, dummy_combiner> signal_type;
