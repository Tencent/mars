...
BOOST_AUTO_TEST_CASE(test_main)
{
  ...
  sig_type s1;
  mars_boost::signals2::connection connection;

  // Test auto-disconnection
  BOOST_CHECK(s1(5) == 0);
  {
    mars_boost::shared_ptr<int> shorty(new int());
  ...
  {
    mars_boost::shared_ptr<int> shorty(new int(1));
...
//    sig_type::slot_type slot(swallow(), shorty.get(), _1);
  ...
  {
    mars_boost::shared_ptr<int> shorty(new int(2));
    mars_boost::signals2::slot<int (double)> other_slot(&myfunc, mars_boost::cref(*shorty.get()), _1);
  ...
  BOOST_CHECK(s1(2) == 0);
  {
    mars_boost::shared_ptr<int> shorty((int*)(0));
    s1.connect(sig_type::slot_type(swallow(), shorty.get(), _1).track(shorty));
    BOOST_CHECK(s1(2) == 2);
  }
  BOOST_CHECK(s1(2) == 0);
