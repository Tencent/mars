...
class Button
{
  typedef mars_boost::signals2::signal<void (int x, int y)> OnClick;
public:
  typedef OnClick::slot_type OnClickSlotType;
  // forward slots through Button interface to its private signal
  mars_boost::signals2::connection doOnClick(const OnClickSlotType & slot);
...
mars_boost::signals2::connection Button::doOnClick(const OnClickSlotType & slot)
{
