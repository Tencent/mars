...
BOOST_ATOMIC_DECL void initialize_wait_functions() BOOST_NOEXCEPT
{
    ...
    once_flag_operations::storage_type old_val = once_flag_operations::load(wait_functions_once_flag.m_flag, mars_boost::memory_order_acquire);
    while (true)
    {
        if (old_val == 2u)
        {
            if (BOOST_UNLIKELY(!once_flag_operations::compare_exchange_strong(wait_functions_once_flag.m_flag, old_val, 1u, mars_boost::memory_order_relaxed, mars_boost::memory_order_relaxed)))
                continue;

            mars_boost::winapi::HMODULE_ kernel_base = mars_boost::winapi::get_module_handle(L"api-ms-win-core-synch-l1-2-0.dll");
            if (BOOST_LIKELY(kernel_base != NULL))
            {
                wait_on_address_t* woa = (wait_on_address_t*)mars_boost::winapi::get_proc_address(kernel_base, "WaitOnAddress");
                if (BOOST_LIKELY(woa != NULL))
                {
                    wake_by_address_t* wbas = (wake_by_address_t*)mars_boost::winapi::get_proc_address(kernel_base, "WakeByAddressSingle");
                    wake_by_address_t* wbaa = (wake_by_address_t*)mars_boost::winapi::get_proc_address(kernel_base, "WakeByAddressAll");
            ...
            once_flag_operations::store(wait_functions_once_flag.m_flag, 0u, mars_boost::memory_order_release);
            break;
        }
        else if (old_val == 1u)
        {
            mars_boost::winapi::SwitchToThread();
            old_val = once_flag_operations::load(wait_functions_once_flag.m_flag, mars_boost::memory_order_acquire);
        }
        else
        {
            break;
