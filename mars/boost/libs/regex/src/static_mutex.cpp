...
BOOST_STATIC_ASSERT(sizeof(LONG) == sizeof(mars_boost::int32_t));
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
      ...
      while(0 != InterlockedCompareExchange(reinterpret_cast<void**>((mars_boost::uint_least16_t*)&(m_mutex.m_mutex)), (void*)1, 0))
#else
...
mars_boost::recursive_mutex* static_mutex::m_pmutex = 0;
mars_boost::once_flag static_mutex::m_once = BOOST_ONCE_INIT;

extern "C" BOOST_REGEX_DECL void mars_boost_regex_free_static_mutex()
{
...
{
   m_pmutex = new mars_boost::recursive_mutex();
   int r = atexit(mars_boost_regex_free_static_mutex);
   BOOST_ASSERT(0 == r);
}
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
       mars_boost::call_once(static_mutex::m_once,&static_mutex::init);
      if(0 == m_plock)
         m_plock = new mars_boost::unique_lock<mars_boost::recursive_mutex>(*static_mutex::m_pmutex, mars_boost::defer_lock);
      m_plock->lock();
      m_have_lock = true;
   }
}
