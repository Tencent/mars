...
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost { namespace iostreams { namespace example {
...
public:
    ...
    int get(Source& src)
    {
        int c;
        while (true) {
            if ((c = mars_boost::iostreams::get(src)) == EOF || c == WOULD_BLOCK)
                break;
...
public:
    ...
    std::streamsize read(Source& src, char* s, std::streamsize n)
    {
        for (std::streamsize z = 0; z < n; ++z) {
            int c;
            while (true) {
                if ((c = mars_boost::iostreams::get(src)) == EOF)
                    return z != 0 ? z : -1;
