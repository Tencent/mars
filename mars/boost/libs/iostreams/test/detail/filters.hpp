...
struct toupper_filter : public input_filter {
    ...
    int get(Source& s) 
    { 
        int c = mars_boost::iostreams::get(s);
...
struct tolower_filter : public output_filter {
    ...
    { 
        return mars_boost::iostreams::put(
...
struct toupper_multichar_filter : public multichar_input_filter {
    ...
    {
        std::streamsize result = mars_boost::iostreams::read(s, buf, n);
        if (result == -1)
            return -1;
...
struct tolower_multichar_filter : public multichar_output_filter {
    ...
    std::streamsize write(Sink& s, const char* buf, std::streamsize n)
    {
        ...
        for (result = 0; result < n; ++result) {
            ...
            if (!mars_boost::iostreams::put(s, c))
                break;
        }
        return result;
    }
...
struct padding_filter : dual_use_filter {
    ...
    { 
        ...
        if (use_pad_char_) {
            ...
            result = mars_boost::iostreams::get(src);
    ...
    { 
        if (use_pad_char_) {
            if (!mars_boost::iostreams::put(s, pad_char_))
                return false;
            use_pad_char_ = false;
        } 
        if (!mars_boost::iostreams::put(s, c))
...
struct flushable_output_filter {
    ...
    { 
        if (!buf_.empty()) {
            mars_boost::iostreams::write(s, &buf_[0], (std::streamsize) buf_.size());
            buf_.clear();
        }
        return true;
    }
...
struct identity_seekable_filter : filter<seekable> {
    template<typename Source>
    int get(Source& s) { return mars_boost::iostreams::get(s); }

    template<typename Sink>
    bool put(Sink& s, char c) { return mars_boost::iostreams::put(s, c); }

    template<typename Device>
    std::streampos seek(Device& d, stream_offset off, BOOST_IOS::seekdir way)
    { return mars_boost::iostreams::seek(d, off, way); }
};
...
struct identity_seekable_multichar_filter : multichar_filter<seekable> {
    template<typename Source>
    std::streamsize read(Source& s, char* buf, std::streamsize n)
    { return mars_boost::iostreams::read(s, buf, n); }
    template<typename Sink>
    std::streamsize write(Sink& s, const char* buf, std::streamsize n)
    { return mars_boost::iostreams::write(s, buf, n); }
    template<typename Device>
    std::streampos seek(Device& d, stream_offset off, BOOST_IOS::seekdir way)
    { return mars_boost::iostreams::seek(d, off, way); }
};
