...
mars_boost::iostreams::detail::file_handle open_file_handle(std::string const& name)
{
    HANDLE handle = 
...
void close_file_handle(mars_boost::iostreams::detail::file_handle handle)
{
...
bool is_handle_open(mars_boost::iostreams::detail::file_handle handle)
{
    return ::fcntl(handle, F_GETFD) != -1;
}

#define BOOST_CHECK_HANDLE_CLOSED(handle) \
    BOOST_CHECK(!::mars_boost::iostreams::test::is_handle_open(handle))
#define BOOST_CHECK_HANDLE_OPEN(handle) \
    BOOST_CHECK(::mars_boost::iostreams::test::is_handle_open(handle))
