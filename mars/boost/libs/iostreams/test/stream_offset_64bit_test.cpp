...
{
    mars_boost::int64_t val = mars_boost::int64_t(1) << 31;
    std::streampos pos = mars_boost::iostreams::offset_to_position(val);
    pos -= 2;
    BOOST_CHECK_EQUAL(val - 2, mars_boost::iostreams::position_to_offset(pos));

    val = -val;
    pos = mars_boost::iostreams::offset_to_position(val);
    pos += 2;
    BOOST_CHECK_EQUAL(val + 2, mars_boost::iostreams::position_to_offset(pos));
}
