...
using mars_boost::iostreams::test::chunk_size;
...
bool putback_test_three(Source& src)
{
    try {
        while (true) {
            char buf[chunk_size];
            if (mars_boost::iostreams::read(src, buf, chunk_size) < chunk_size)
                break;
            mars_boost::iostreams::putback(src, 'a');
            if (mars_boost::iostreams::get(src) != 'a')
                return false;
        }
        return true;
    } catch (std::exception&) { return false; }
    ...
    try {
        while (true) {
            ...
            if (mars_boost::iostreams::read(src, buf, chunk_size) < chunk_size)
                break;
            mars_boost::iostreams::putback(src, 'a');
            mars_boost::iostreams::putback(src, 'b');
            mars_boost::iostreams::putback(src, 'c');
            mars_boost::iostreams::putback(src, 'd');
            if ( mars_boost::iostreams::get(src) != 'd' || 
                 mars_boost::iostreams::get(src) != 'c' ||
                 mars_boost::iostreams::get(src) != 'b' || 
                 mars_boost::iostreams::get(src) != 'a' )
            {
                return false;
            }
        }
