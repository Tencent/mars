...
struct replace_lower {
    std::string operator() (const mars_boost::match_results<const char*>&)
    { return "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; }
#ifndef BOOST_IOSTREAMS_NO_WIDE_STREAMS
    std::wstring operator() (const mars_boost::match_results<const wchar_t*>&)
...
void regex_filter_test()
{
    ...
    mars_boost::regex match_lower("[a-z]+");
...

    {
        ...
        filtering_istream
            first(mars_boost::iostreams::regex_filter(match_lower, replace_lower()));
...

    {
        ...
        filtering_istream 
            first(mars_boost::iostreams::regex_filter(match_lower, fmt));
        first.push(file_source(test.name(), in_mode));
        {
            ifstream second(upper.name().c_str(), in_mode);
            BOOST_CHECK_MESSAGE(
...

    {
        ...
        filtering_ostream 
            out(mars_boost::iostreams::regex_filter(match_lower, replace_lower()));
...

    {
        ...
        filtering_ostream  
            out(mars_boost::iostreams::regex_filter(match_lower, fmt));
        out.push(file_sink(dest1.name(), out_mode));
        write_data_in_chars(out);
        out.pop();
        BOOST_CHECK_MESSAGE(
...

    {
        ...
        mars_boost::regex   match_xxx("xxx");
        test_file      test2; 
        filtering_istream
            first(mars_boost::iostreams::regex_filter(match_xxx, replace_lower()));
        first.push(file_source(test.name(), in_mode));
        {
...
void wregex_filter_test()
{
    ...
    mars_boost::wregex match_lower(L"[a-z]+");
...

    {
        ...
        // Test reading from a regex filter based on a function in chars.
        filtering_wistream
            first(mars_boost::iostreams::wregex_filter(match_lower, replace_lower()));
...

    {
        ...
        // Test reading from a regex filter based on a format string in chars.
        filtering_wistream 
            first(mars_boost::iostreams::wregex_filter(match_lower, fmt));
        first.push(wfile_source(test.name(), in_mode));
        {
            wifstream second(upper.name().c_str(), in_mode);
            BOOST_CHECK_MESSAGE(
...

    {
        ...
        // Test writing to a regex filter based on a function in chars.
        filtering_wostream 
            out(mars_boost::iostreams::wregex_filter(match_lower, replace_lower()));
...

    {
        ...
        // Test writing to a regex filter based on a format string in chars.
        filtering_wostream  
            out(mars_boost::iostreams::wregex_filter(match_lower, fmt));
        out.push(wfile_sink(dest1.name(), out_mode));
        write_data_in_chars(out);
        out.pop();
        BOOST_CHECK_MESSAGE(
...

    {
        ...
        // Test reading from a regex filter with no matches; this checks that
        // Ticket #1139 is fixed
        mars_boost::wregex   match_xxx(L"xxx");
        test_file      test2; 
        filtering_wistream
            first(mars_boost::iostreams::wregex_filter(match_xxx, replace_lower()));
        first.push(wfile_source(test.name(), in_mode));
        {
