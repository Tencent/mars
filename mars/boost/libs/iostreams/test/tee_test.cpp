...
void close_test()
{
    ...
    // Tee a sink
    {
        operation_sequence  seq;
        chain<output>       ch;
        ch.push(mars_boost::iostreams::tee(closable_device<output>(seq.new_operation(1))));
    ...
    // Tee a seekable device
    {
        operation_sequence  seq;
        chain<output>       ch;
        ch.push(mars_boost::iostreams::tee(closable_device<seekable>(seq.new_operation(1))));
