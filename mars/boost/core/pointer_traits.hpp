...
-> decltype(mars_boost::pointer_traits<T>::to_address(v))
{
    return mars_boost::pointer_traits<T>::to_address(v);
}
...
{
    return mars_boost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return mars_boost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return mars_boost::to_address(v.operator->());
}
#endif
