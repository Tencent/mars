...
inline void invoke_swap_impl(T (& left)[N], T (& right)[N])
    BOOST_CORE_SWAP_NOEXCEPT_IF(BOOST_NOEXCEPT_EXPR(::mars_boost_swap_impl::invoke_swap_impl(left[0], right[0])))
{
    for (std::size_t i = 0; i < N; ++i)
    {
        ::mars_boost_swap_impl::invoke_swap_impl(left[i], right[i]);
    }
}
...
template<class T>
BOOST_GPU_ENABLED
inline typename enable_if_c< !::mars_boost_swap_impl::is_const<T>::value >::type
invoke_swap(T& left, T& right)
    BOOST_CORE_SWAP_NOEXCEPT_IF(BOOST_NOEXCEPT_EXPR(::mars_boost_swap_impl::invoke_swap_impl(left, right)))
{
    ::mars_boost_swap_impl::invoke_swap_impl(left, right);
}
