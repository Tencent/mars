...
void print_line(stringstream& sout, string line) {
    ...
    if (line.substr(0, 12) == "ZSTDLIB_API ")
        line = line.substr(12);
    spos = line.find("/*");
    if (spos != string::npos) {
...
int main(int argc, char* argv[]) {
    ...
    for (linenum = 0; (size_t)linenum < input.size(); linenum++) {
        ...
        if (!comments.empty())
            comments[0] = line.substr(spos + 3);
        if (!comments.empty())
            comments[comments.size() - 1] =
                comments[comments.size() - 1].substr(0, comments[comments.size() - 1].find("*/"));
        ...
        if (exclam == '!') {
            if (!comments.empty())
                comments.erase(comments.begin()); /* remove first line like "ZSTD_XXX() :" */
            linenum++;
            lines = get_lines(input, linenum, "");
            ...
            for (l = 0; l < lines.size(); l++) {
                ...
                if (fline.substr(0, 12) == "ZSTDLIB_API " || fline.substr(0, 12) == string(12, ' '))
        ...
        } else if (exclam == '=') { /* comments of type /*= and /**= mean: use a <H3> header and show also all functions
