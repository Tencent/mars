...
size_t ZSTD_seekable_frameLog_allocVec(ZSTD_frameLog* fl) {
    ...
    fl->entries = (framelogEntry_t*)malloc(sizeof(framelogEntry_t) * FRAMELOG_STARTING_CAPACITY);
    if (fl->entries == NULL)
...
size_t ZSTD_seekable_initCStream(ZSTD_seekable_CStream* zcs,
    ...
    zcs->maxFrameSize = maxFrameSize ? maxFrameSize : ZSTD_SEEKABLE_MAX_FRAME_DECOMPRESSED_SIZE;
...
size_t ZSTD_seekable_logFrame(ZSTD_frameLog* fl,
    ...
    if (fl->size == fl->capacity) {
        ...
        framelogEntry_t* const newEntries = realloc(fl->entries, sizeof(framelogEntry_t) * newCapacity);
    ...
    fl->entries[fl->size] = (framelogEntry_t){compressedSize, decompressedSize, checksum};
    fl->size++;
...
size_t ZSTD_seekable_endFrame(ZSTD_seekable_CStream* zcs, ZSTD_outBuffer* output) {
    ...
    ret = ZSTD_seekable_logFrame(&zcs->framelog,
                                 zcs->frameCSize,
                                 zcs->frameDSize,
                                 zcs->framelog.checksumFlag ? XXH64_digest(&zcs->xxhState) & 0xFFFFFFFFU : 0);
    if (ret)
        return ret;
...
static inline size_t ZSTD_seekable_seekTableSize(const ZSTD_frameLog* fl) {
    ...
    size_t const seekTableLen = ZSTD_SKIPPABLEHEADERSIZE + sizePerFrame * fl->size + ZSTD_seekTableFooterSize;

    return seekTableLen;
}

static inline size_t ZSTD_stwrite32(ZSTD_frameLog* fl, ZSTD_outBuffer* output, U32 const value, U32 const offset) {
    if (fl->seekTablePos < offset + 4) {
        BYTE tmp[4]; /* so that we can work with buffers too small to write a whole word to */
        size_t const lenWrite = MIN(output->size - output->pos, offset + 4 - fl->seekTablePos);
...
size_t ZSTD_seekable_writeSeekTable(ZSTD_frameLog* fl, ZSTD_outBuffer* output) {
    ...
    while (fl->seekTableIndex < fl->size) {
        ...
        CHECK_Z(ZSTD_stwrite32(fl, output, fl->entries[fl->seekTableIndex].cSize, (U32)start + 0));

        CHECK_Z(ZSTD_stwrite32(fl, output, fl->entries[fl->seekTableIndex].dSize, (U32)start + 4));

        if (fl->checksumFlag) {
            CHECK_Z(ZSTD_stwrite32(fl, output, fl->entries[fl->seekTableIndex].checksum, (U32)start + 8));
        }
    ...
    CHECK_Z(ZSTD_stwrite32(fl, output, fl->size, (U32)seekTableLen - ZSTD_seekTableFooterSize));
    ...
    CHECK_Z(ZSTD_stwrite32(fl, output, ZSTD_SEEKABLE_MAGICNUMBER, (U32)seekTableLen - 4));
