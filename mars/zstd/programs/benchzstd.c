...
static void BMK_initDCtx(ZSTD_DCtx* dctx, const void* dictBuffer, size_t dictBufferSize) {
...
static size_t local_defaultCompress(const void* srcBuffer,
                                    size_t srcSize,
                                    void* dstBuffer,
                                    size_t dstSize,
                                    void* addArgs) {
...
/* `addArgs` is the context */
static size_t local_defaultDecompress(const void* srcBuffer,
                                      size_t srcSize,
                                      void* dstBuffer,
                                      size_t dstCapacity,
                                      void* addArgs) {
...
static BMK_benchOutcome_t BMK_benchMemAdvancedNoAlloc(const void** srcPtrs,
                                                      size_t* srcSizes,
                                                      void** cPtrs,
                                                      size_t* cCapacities,
                                                      size_t* cSizes,
    ...
    if (strlen(displayName) > 17)
        displayName += strlen(displayName) - 17; /* display last 17 characters */
    if (adv->mode == BMK_decodeOnly) {           /* benchmark only decompression : source must be already compressed */
        ...
        for (fileNb = 0; fileNb < nbFiles; fileNb++) {
            ...
            if (fSize64 == 0)
                RETURN_ERROR(32, BMK_benchOutcome_t, "Impossible to determine original size ");
    ...
    /* Init data blocks  */
    {
        ...
        for (nbBlocks = 0, fileNb = 0; fileNb < nbFiles; fileNb++) {
            ...
            U32 const nbBlocksforThisFile =
                (adv->mode == BMK_decodeOnly) ? 1 : (U32)((remaining + (blockSize - 1)) / blockSize);
            ...
            for (; nbBlocks < blockEnd; nbBlocks++) {
                ...
                cCapacities[nbBlocks] =
                    (adv->mode == BMK_decodeOnly) ? thisBlockSize : ZSTD_compressBound(thisBlockSize);
                resPtrs[nbBlocks] = resPtr;
                resSizes[nbBlocks] = (adv->mode == BMK_decodeOnly)
                                         ? (size_t)ZSTD_findDecompressedSize(srcPtr, thisBlockSize)
                                         : thisBlockSize;
        ...
        while (!(compressionCompleted && decompressionCompleted)) {
            if (!compressionCompleted) {
...

                {
                    ...
                    DISPLAYLEVEL(2,
                                 "%2s-%-17.17s :%10u ->%10u (%5.*f),%6.*f MB/s\r",
                                 marks[markNb],
                                 displayName,
                                 (unsigned)srcSize,
                                 (unsigned)cSize,
            ...
            if (!decompressionCompleted) {
...

                {
                    int const ratioAccuracy = (ratio < 10.) ? 3 : 2;
                    DISPLAYLEVEL(2,
                                 "%2s-%-17.17s :%10u ->%10u (%5.*f),%6.*f MB/s ,%6.1f MB/s \r",
                                 marks[markNb],
                                 displayName,
                                 (unsigned)srcSize,
                                 (unsigned)benchResult.cSize,
                                 ratioAccuracy,
                                 ratio,
                                 benchResult.cSpeed < (10 MB) ? 2 : 1,
                                 (double)benchResult.cSpeed / MB_UNIT,
...

{
    ...
    int allocationincomplete = !srcPtrs || !srcSizes || !cPtrs || !cSizes || !cCapacities || !resPtrs || !resSizes
                               || !timeStateCompress || !timeStateDecompress || !cctx || !dctx || !compressedBuffer
                               || !resultBuffer;

    if (!allocationincomplete && !dstParamsError) {
        outcome = BMK_benchMemAdvancedNoAlloc(srcPtrs,
                                              srcSizes,
                                              cPtrs,
                                              cCapacities,
                                              cSizes,
...
BMK_benchOutcome_t BMK_benchMem(const void* srcBuffer,
    ...
    return BMK_benchMemAdvanced(srcBuffer,
                                srcSize,
                                NULL,
                                0,
                                fileSizes,
...
static BMK_benchOutcome_t BMK_benchCLevel(const void* srcBuffer,
                                          size_t benchedSize,
                                          const size_t* fileSizes,
                                          unsigned nbFiles,
                                          int cLevel,
...
static int BMK_loadFiles(void* buffer,
                         size_t bufferSize,
                         size_t* fileSizes,
    ...
    for (n = 0; n < nbFiles; n++) {
        ...
        }
        {
            ...
            if (f == NULL)
                RETURN_ERROR_INT(10, "impossible to open file %s", fileNamesTable[n]);
                ...
                fileSize = bufferSize - pos, nbFiles = n; /* buffer too small - stop after this file */
            {
                ...
                if (readSize != (size_t)fileSize)
                    RETURN_ERROR_INT(11, "could not read %s", fileNamesTable[n]);
                pos += readSize;
            }
...
BMK_benchOutcome_t BMK_benchFilesAdvanced(const char* const* fileNamesTable,
    ...
    /* Load input buffer */
    {
        int const errorCode = BMK_loadFiles(srcBuffer, benchedSize, fileSizes, fileNamesTable, nbFiles, displayLevel);
        if (errorCode) {
            res = BMK_benchOutcome_error();
            goto _cleanUp;
        }
