 ...
 ****************************************/
    ...
    || defined _LP64                                                                                       \
    || defined __LP64__ /* NetBSD, OpenBSD */ || defined __64BIT__ /* AIX */ || defined _ADDR64 /* Cray */ \
 ...
 * (http://pubs.opengroup.org/onlinepubs/7908799/xsh/unistd.h.html). note : there is no simple way to know in advance if
 * <unistd.h> is present or not on target system, Posix specification mandates its presence and its content, but target
 * system must respect this spec. It's necessary to _not_ #include <unistd.h> whenever target OS is not unix-like
 ...
 */
#elif !defined(_WIN32) \
    && (defined(__unix__) || defined(__unix) || defined(__midipix__) || defined(__VMS) || defined(__HAIKU__))

#if defined(__linux__) || defined(__linux)
#ifndef _POSIX_C_SOURCE
#define _POSIX_C_SOURCE \
    200809L /* feature test macro : https://www.gnu.org/software/libc/manual/html_node/Feature-Test-Macros.html */
#endif
#endif
...
#define SET_SPARSE_FILE_MODE(file)                                                                    \
    {                                                                                                 \
        DWORD dw;                                                                                     \
        DeviceIoControl((HANDLE)_get_osfhandle(_fileno(file)), FSCTL_SET_SPARSE, 0, 0, 0, 0, &dw, 0); \
    }
#else
