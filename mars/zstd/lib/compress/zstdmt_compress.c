...
static void ZSTDMT_serialState_update(serialState_t* serialState,
    ...
    if (serialState->nextJobID == jobID) {
        ...
        if (serialState->params.ldmParams.enableLdm) {
            ...
            error = ZSTD_ldm_generateSequences(&serialState->ldmState,
                                               &seqStore,
    ...
    if (job->cdict) {
        ...
        U64 const pledgedSrcSize = job->firstJob ? job->fullFrameSize : job->src.size;
        {
            size_t const forceWindowError =
                ZSTD_CCtxParams_setParameter(&jobParams, ZSTD_c_forceMaxWindow, !job->firstJob);
...
static unsigned ZSTDMT_computeNbJobs(const ZSTD_CCtx_params* params, size_t srcSize, unsigned nbWorkers) {
    assert(nbWorkers > 0);
    {
 ...
 */
static size_t ZSTDMT_compress_advanced_internal(ZSTDMT_CCtx* mtctx,
                                                void* dst,
                                                size_t dstCapacity,
                                                const void* src,
                                                size_t srcSize,
    ...
    assert(avgJobSize
           >= 256 KB); /* condition for ZSTD_compressBound(A) + ZSTD_compressBound(B) <= ZSTD_compressBound(A+B),
...
static size_t ZSTDMT_flushProduced(ZSTDMT_CCtx* mtctx,
    ...
    /* try to flush something */
    {
        ...
        size_t const srcSize =
            mtctx->jobs[wJobID]
                .src.size; /* read-only, could be done after mutex lock, but no-declaration-after-statement */
        ZSTD_pthread_mutex_unlock(&mtctx->jobs[wJobID].job_mutex);
        if (ZSTD_isError(cSize)) {
            DEBUGLOG(5,
...
size_t ZSTDMT_compressStream_generic(ZSTDMT_CCtx* mtctx,
    ...
    {
        size_t const remainingToFlush = ZSTDMT_flushProduced(mtctx,
                                                             output,
...
size_t ZSTDMT_compressStream(ZSTDMT_CCtx* mtctx, ZSTD_outBuffer* output, ZSTD_inBuffer* input) {
    ...
    return mtctx->targetSectionSize - mtctx->inBuff.filled; /* note : could be zero when input buffer is fully filled
                                                               and no more availability to create new job */
}
