...
ZSTD_CCtx* ZSTD_initStaticCCtx(void* workspace, size_t workspaceSize) {
    ...
    cctx->blockState.prevCBlock =
        (ZSTD_compressedBlockState_t*)ZSTD_cwksp_reserve_object(&cctx->workspace, sizeof(ZSTD_compressedBlockState_t));
...
size_t ZSTD_sizeof_CCtx(const ZSTD_CCtx* cctx) {
    ...
    return (cctx->workspace.workspace == cctx ? 0 : sizeof(*cctx)) + ZSTD_cwksp_sizeof(&cctx->workspace)
...
static ZSTD_CCtx_params ZSTD_makeCCtxParamsFromCParams(ZSTD_compressionParameters cParams) {
    ...
    cctxParams.compressionLevel =
        ZSTD_CLEVEL_DEFAULT; /* should not matter, as all cParams are presumed properly defined */
...
static ZSTD_CCtx_params ZSTD_assignParamsToCCtxParams(const ZSTD_CCtx_params* cctxParams, ZSTD_parameters params) {
...
size_t ZSTD_CCtx_setParameter(ZSTD_CCtx* cctx, ZSTD_cParameter param, int value) {
    ...
    switch (param) {
        case ZSTD_c_nbWorkers:
            RETURN_ERROR_IF((value != 0) && cctx->staticSize,
...
static size_t ZSTD_initLocalDict(ZSTD_CCtx* cctx) {
    ...
    ZSTD_compressionParameters const cParams = ZSTD_getCParamsFromCCtxParams(&cctx->requestedParams, 0, dl->dictSize);
    if (dl->dict == NULL) {
        /* No local dictionary. */
    ...
    dl->cdict = ZSTD_createCDict_advanced(dl->dict,
                                          dl->dictSize,
                                          ZSTD_dlm_byRef,
                                          dl->dictContentType,
                                          cParams,
...
static size_t ZSTD_sizeof_matchState(const ZSTD_compressionParameters* const cParams, const U32 forCCtx) {
    ...
    size_t const optPotentialSpace =
        ZSTD_cwksp_alloc_size((MaxML + 1) * sizeof(U32)) + ZSTD_cwksp_alloc_size((MaxLL + 1) * sizeof(U32))
        + ZSTD_cwksp_alloc_size((MaxOff + 1) * sizeof(U32)) + ZSTD_cwksp_alloc_size((1 << Litbits) * sizeof(U32))
...
size_t ZSTD_estimateCCtxSize_usingCCtxParams(const ZSTD_CCtx_params* params) {
    RETURN_ERROR_IF(params->nbWorkers > 0,
                    GENERIC,
                    "Estimate CCtx size is supported for single-threaded compression only.");
    {
        ZSTD_compressionParameters const cParams = ZSTD_getCParamsFromCCtxParams(params, 0, 0);
        ...
        size_t const ldmSeqSpace =
            ZSTD_cwksp_alloc_size(ZSTD_ldm_getMaxNbSeq(params->ldmParams, blockSize) * sizeof(rawSeq));

        size_t const neededSpace =
            entropySpace + blockStateSpace + tokenSpace + matchStateSize + ldmSpace + ldmSeqSpace;
...
size_t ZSTD_estimateCStreamSize_usingCCtxParams(const ZSTD_CCtx_params* params) {
                    ...
                    "Estimate CCtx size is supported for single-threaded compression only.");
    {
        ...
        size_t const streamingSize = ZSTD_cwksp_alloc_size(inBuffSize) + ZSTD_cwksp_alloc_size(outBuffSize);
...
#endif
    (void)cctx;
    return 0; /* over-simplification; could also check if context is currently running in streaming mode, and in which
                 case, report how many bytes are left to be flushed within output buffer */
}
...
static size_t ZSTD_reset_matchState(ZSTD_matchState_t* ms,
    ...
    RETURN_ERROR_IF(ZSTD_cwksp_reserve_failed(ws),
                    memory_allocation,
    ...
    if ((forWho == ZSTD_resetTarget_CCtx) && (cParams->strategy >= ZSTD_btopt)) {
        ...
        ms->opt.priceTable =
            (ZSTD_optimal_t*)ZSTD_cwksp_reserve_aligned(ws, (ZSTD_OPT_NUM + 1) * sizeof(ZSTD_optimal_t));
    }
...
static size_t ZSTD_resetCCtx_internal(ZSTD_CCtx* zc,
    ...
    DEBUGLOG(4, "ZSTD_resetCCtx_internal: pledgedSrcSize=%u, wlog=%u", (U32)pledgedSrcSize, params.cParams.windowLog);
...

    {
        ...
        /* Check if workspace is large enough, alloc a new one if needed */
        {
            ...
            if (workspaceTooSmall || workspaceWasteful) {
                ...
                RETURN_ERROR_IF(zc->blockState.nextCBlock == NULL,
                                memory_allocation,
        ...
        if (params.ldmParams.enableLdm) {
            /* TODO: avoid memset? */
            size_t const ldmBucketSize = ((size_t)1) << (params.ldmParams.hashLog - params.ldmParams.bucketSizeLog);
        ...
        FORWARD_IF_ERROR(ZSTD_reset_matchState(&zc->blockState.matchState,
                                               ws,
                                               &params.cParams,
                                               crp,
                                               needsIndexReset,
...
static int ZSTD_shouldAttachDict(const ZSTD_CDict* cdict, const ZSTD_CCtx_params* params, U64 pledgedSrcSize) {
    ...
    return (pledgedSrcSize <= cutoff || pledgedSrcSize == ZSTD_CONTENTSIZE_UNKNOWN
            || params->attachDictPref == ZSTD_dictForceAttach)
           && params->attachDictPref != ZSTD_dictForceCopy && !params->forceWindow; /* dictMatchState isn't correctly
...
static size_t ZSTD_resetCCtx_byAttachingCDict(ZSTD_CCtx* cctx,
                                              ...
                                              ZSTD_buffered_policy_e zbuff) {
    {
        ...
        FORWARD_IF_ERROR(ZSTD_resetCCtx_internal(cctx, params, pledgedSrcSize, ZSTDcrp_makeClean, zbuff));
...
static size_t ZSTD_resetCCtx_byCopyingCDict(ZSTD_CCtx* cctx,
...

    {
        ...
        params.cParams.windowLog = windowLog;
        FORWARD_IF_ERROR(ZSTD_resetCCtx_internal(cctx, params, pledgedSrcSize, ZSTDcrp_leaveDirty, zbuff));
        assert(cctx->appliedParams.cParams.strategy == cdict_cParams->strategy);
    ...
    /* copy tables */
    {
        ...
        memcpy(cctx->blockState.matchState.hashTable, cdict->matchState.hashTable, hSize * sizeof(U32));
        memcpy(cctx->blockState.matchState.chainTable, cdict->matchState.chainTable, chainSize * sizeof(U32));
    }
...
static size_t ZSTD_copyCCtx_internal(ZSTD_CCtx* dstCCtx,
    ...
    memcpy(&dstCCtx->customMem, &srcCCtx->customMem, sizeof(ZSTD_customMem));
    {
        ...
        ZSTD_resetCCtx_internal(dstCCtx, params, pledgedSrcSize, ZSTDcrp_leaveDirty, zbuff);
    ...
    /* copy tables */
    {
        size_t const chainSize = (srcCCtx->appliedParams.cParams.strategy == ZSTD_fast)
                                     ? 0
        ...
        memcpy(dstCCtx->blockState.matchState.hashTable, srcCCtx->blockState.matchState.hashTable, hSize * sizeof(U32));
...
static size_t ZSTD_noCompressBlock(void* dst, size_t dstCapacity, const void* src, size_t srcSize, U32 lastBlock) {
    ...
    RETURN_ERROR_IF(srcSize + ZSTD_blockHeaderSize > dstCapacity, dstSize_tooSmall);
...
MEM_STATIC size_t ZSTD_compressSequences_internal(seqStore_t* seqStorePtr,
    ...
    /* Compress literals */
    {
        ...
        size_t const cSize = ZSTD_compressLiterals(&prevEntropy->huf,
                                                   &nextEntropy->huf,
    ...
    /* build CTable for Literal Lengths */
    {
        ...
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, llCodeTable, nbSeq, entropyWorkspace, entropyWkspSize); /* can't fail */
        ...
        {
            size_t const countSize = ZSTD_buildCTable(op,
                                                      (size_t)(oend - op),
                                                      CTable_LitLength,
                                                      LLFSELog,
    ...
    /* build CTable for Offsets */
    {
        unsigned max = MaxOff;
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, ofCodeTable, nbSeq, entropyWorkspace, entropyWkspSize); /* can't fail */
    ...
    /* build CTable for MatchLengths */
    {
        unsigned max = MaxML;
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, mlCodeTable, nbSeq, entropyWorkspace, entropyWkspSize); /* can't fail */
    ...
    *seqHead = (BYTE)((LLtype << 6) + (Offtype << 4) + (MLtype << 2));
    ...
    {
        size_t const bitstreamSize = ZSTD_encodeSequences(op,
                                                          (size_t)(oend - op),
                                                          CTable_MatchLength,
                                                          mlCodeTable,
                                                          CTable_OffsetBits,
...
MEM_STATIC size_t ZSTD_compressSequences(seqStore_t* seqStorePtr,
                                         ...
                                         size_t entropyWkspSize,
                                         int bmi2) {
    size_t const cSize = ZSTD_compressSequences_internal(seqStorePtr,
                                                         prevEntropy,
                                                         nextEntropy,
                                                         cctxParams,
                                                         dst,
...
static size_t ZSTD_buildSeqStore(ZSTD_CCtx* zc, const void* src, size_t srcSize) {
    ...
    /* select and store sequences */
    {
        ...
        if (zc->externSeqStore.pos < zc->externSeqStore.size) {
            ...
            lastLLSize = ZSTD_ldm_blockCompress(&zc->externSeqStore,
                                                ms,
                                                &zc->seqStore,
            ...
            FORWARD_IF_ERROR(
                ZSTD_ldm_generateSequences(&zc->ldmState, &ldmSeqStore, &zc->appliedParams.ldmParams, src, srcSize));
            /* Updates ldmSeqStore.pos */
            lastLLSize =
                ZSTD_ldm_blockCompress(&ldmSeqStore, ms, &zc->seqStore, zc->blockState.nextCBlock->rep, src, srcSize);
            assert(ldmSeqStore.pos == ldmSeqStore.size);
        } else { /* not long range mode */
            ZSTD_blockCompressor const blockCompressor =
                ZSTD_selectBlockCompressor(zc->appliedParams.cParams.strategy, dictMode);
    ...
    while (remaining) {
...

        {
            ...
            if (cSize == 0) { /* block is not compressible */
                ...
                const U32 cBlockHeader = cSize == 1 ? lastBlock + (((U32)bt_rle) << 1) + (U32)(blockSize << 3)
...
static size_t ZSTD_writeFrameHeader(void* dst,
    ...
    U32 const fcsCode = params->fParams.contentSizeFlag ? (pledgedSrcSize >= 256) + (pledgedSrcSize >= 65536 + 256)
...
size_t ZSTD_referenceExternalSequences(ZSTD_CCtx* cctx, rawSeq* seq, size_t nbSeq) {
    ...
    RETURN_ERROR_IF(cctx->appliedParams.ldmParams.enableLdm, parameter_unsupported);
...
static size_t ZSTD_compressContinue_internal(ZSTD_CCtx* cctx,
    ...
    DEBUGLOG(5, "ZSTD_compressContinue_internal, stage: %u, srcSize: %u", cctx->stage, (unsigned)srcSize);
    RETURN_ERROR_IF(cctx->stage == ZSTDcs_created, stage_wrong, "missing init (ZSTD_compressBegin)");

    if (frame && (cctx->stage == ZSTDcs_init)) {
        fhSize = ZSTD_writeFrameHeader(dst,
                                       dstCapacity,
                                       &cctx->appliedParams,
    ...
    if (!frame) {
        ...
        ZSTD_overflowCorrectIfNeeded(ms, &cctx->workspace, &cctx->appliedParams, src, (BYTE const*)src + srcSize);
    }

    DEBUGLOG(5, "ZSTD_compressContinue_internal (blockSize=%u)", (unsigned)cctx->blockSize);
    {
        size_t const cSize = frame ? ZSTD_compress_frameChunk(cctx, dst, dstCapacity, src, srcSize, lastFrameChunk)
...
static size_t ZSTD_loadZstdDictionary(ZSTD_compressedBlockState_t* bs,
...

    {
        ...
        size_t const hufHeaderSize =
            HUF_readCTable((HUF_CElt*)bs->entropy.huf.CTable, &maxSymbolValue, dictPtr, dictEnd - dictPtr);
...

    {
        ...
        RETURN_ERROR_IF(FSE_isError(FSE_buildCTable_wksp(bs->entropy.fse.offcodeCTable,
                                                         offcodeNCount,
                                                         MaxOff,
                                                         offcodeLog,
                                                         workspace,
...

    {
        ...
        RETURN_ERROR_IF(FSE_isError(FSE_buildCTable_wksp(bs->entropy.fse.matchlengthCTable,
                                                         matchlengthNCount,
                                                         matchlengthMaxValue,
                                                         matchlengthLog,
                                                         workspace,
...

    {
        ...
        RETURN_ERROR_IF(FSE_isError(FSE_buildCTable_wksp(bs->entropy.fse.litlengthCTable,
                                                         litlengthNCount,
                                                         litlengthMaxValue,
                                                         litlengthLog,
                                                         workspace,
...

    {
        ...
        FORWARD_IF_ERROR(ZSTD_loadDictionaryContent(ms, ws, params, dictPtr, dictContentSize, dtlm));
        return dictID;
    }
}
...
static size_t ZSTD_compressBegin_internal(ZSTD_CCtx* cctx,
    ...
    FORWARD_IF_ERROR(ZSTD_resetCCtx_internal(cctx, *params, pledgedSrcSize, ZSTDcrp_makeClean, zbuff));
    {
        size_t const dictID = cdict ? ZSTD_compress_insertDictionary(cctx->blockState.prevCBlock,
...
size_t ZSTD_compressBegin_advanced(ZSTD_CCtx* cctx,
                                   ...
                                   unsigned long long pledgedSrcSize) {
    ZSTD_CCtx_params const cctxParams = ZSTD_assignParamsToCCtxParams(&cctx->requestedParams, params);
...
size_t ZSTD_compressEnd(ZSTD_CCtx* cctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize) {
    size_t endResult;
    size_t const cSize =
        ZSTD_compressContinue_internal(cctx, dst, dstCapacity, src, srcSize, 1 /* frame mode */, 1 /* last chunk */);
    FORWARD_IF_ERROR(cSize);
...
size_t ZSTD_compress_advanced(ZSTD_CCtx* cctx,
    ...
    FORWARD_IF_ERROR(ZSTD_checkCParams(params.cParams));
    return ZSTD_compress_internal(cctx, dst, dstCapacity, src, srcSize, dict, dictSize, params);
}
...
size_t ZSTD_estimateCDictSize_advanced(size_t dictSize,
    ...
    return ZSTD_cwksp_alloc_size(sizeof(ZSTD_CDict)) + ZSTD_cwksp_alloc_size(HUF_WORKSPACE_SIZE)
           ...
           + (dictLoadMethod == ZSTD_dlm_byRef ? 0 : ZSTD_cwksp_alloc_size(ZSTD_cwksp_align(dictSize, sizeof(void*))));
}
...
static size_t ZSTD_initCDict_internal(ZSTD_CDict* cdict,
    ...
    FORWARD_IF_ERROR(ZSTD_reset_matchState(&cdict->matchState,
                                           &cdict->workspace,
                                           &cParams,
                                           ZSTDcrp_makeClean,
                                           ZSTDirp_reset,
     ...
     */
    {
        ...
        {
            size_t const dictID = ZSTD_compress_insertDictionary(&cdict->cBlockState,
                                                                 &cdict->matchState,
                                                                 &cdict->workspace,
                                                                 &params,
                                                                 cdict->dictContent,
...
ZSTD_CDict* ZSTD_createCDict_advanced(const void* dictBuffer,
...

    {
        ...
        if (ZSTD_isError(
                ZSTD_initCDict_internal(cdict, dictBuffer, dictSize, dictLoadMethod, dictContentType, cParams))) {
...
ZSTD_CDict* ZSTD_createCDict(const void* dict, size_t dictSize, int compressionLevel) {
    ...
    ZSTD_CDict* cdict =
        ZSTD_createCDict_advanced(dict, dictSize, ZSTD_dlm_byCopy, ZSTD_dct_auto, cParams, ZSTD_defaultCMem);
    if (cdict)
...
ZSTD_CDict* ZSTD_createCDict_byReference(const void* dict, size_t dictSize, int compressionLevel) {
    ZSTD_compressionParameters cParams = ZSTD_getCParams(compressionLevel, 0, dictSize);
    return ZSTD_createCDict_advanced(dict, dictSize, ZSTD_dlm_byRef, ZSTD_dct_auto, cParams, ZSTD_defaultCMem);
}
...
const ZSTD_CDict* ZSTD_initStaticCDict(void* workspace,
    ...
    if (ZSTD_isError(ZSTD_initCDict_internal(cdict, dict, dictSize, dictLoadMethod, dictContentType, cParams)))
        return NULL;
...
static size_t ZSTD_resetCStream_internal(ZSTD_CStream* cctx,
    ...
    cctx->inBuffTarget =
        cctx->blockSize
        + (cctx->blockSize == pledgedSrcSize); /* for small input: avoid automatic flush on reaching end of block, since
...
static size_t ZSTD_limitCopy(void* dst, size_t dstCapacity, const void* src, size_t srcSize) {
        ...
        switch (zcs->streamStage) {
            ...
            case zcss_load:
                ...
                DEBUGLOG(5, "stream compression stage (flushMode==%u)", flushMode);
                {
                    ...
                    cSize = lastBlock ? ZSTD_compressEnd(zcs, cDst, oSize, zcs->inBuff + zcs->inToCompress, iSize)
            ...
            case zcss_flush:
                DEBUGLOG(5, "flush stage");
                {
                    ...
                    size_t const flushed =
                        ZSTD_limitCopy(op, (size_t)(oend - op), zcs->outBuff + zcs->outBuffFlushedSize, toFlush);
                    DEBUGLOG(5,
...
size_t ZSTD_compressStream2(ZSTD_CCtx* cctx, ZSTD_outBuffer* output, ZSTD_inBuffer* input, ZSTD_EndDirective endOp) {
    ...
    if (cctx->streamStage == zcss_init) {
        ...
        params.cParams =
            ZSTD_getCParamsFromCCtxParams(&cctx->requestedParams, cctx->pledgedSrcSizePlusOne - 1, 0 /*dictSize*/);
        ...
        if (params.nbWorkers > 0) {
            /* mt context creation */
            if (cctx->mtctx == NULL) {
                DEBUGLOG(4, "ZSTD_compressStream2: creating new mtctx for nbWorkers=%u", params.nbWorkers);
                cctx->mtctx = ZSTDMT_createCCtx_advanced((U32)params.nbWorkers, cctx->customMem);
                RETURN_ERROR_IF(cctx->mtctx == NULL, memory_allocation);
            }
            /* mt compression */
            DEBUGLOG(4, "call ZSTDMT_initCStream_internal as nbWorkers=%u", params.nbWorkers);
            FORWARD_IF_ERROR(ZSTDMT_initCStream_internal(cctx->mtctx,
                                                         prefixDict.dict,
                                                         prefixDict.dictSize,
                                                         ZSTD_dct_rawContent,
                                                         cctx->cdict,
...
size_t ZSTD_compress2(ZSTD_CCtx* cctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize) {
    ZSTD_CCtx_reset(cctx, ZSTD_reset_session_only);
    {
        ...
        size_t iPos = 0;
        size_t const result =
            ZSTD_compressStream2_simpleArgs(cctx, dst, dstCapacity, &oPos, src, srcSize, &iPos, ZSTD_e_end);
...
ZSTD_compressionParameters ZSTD_getCParams(int compressionLevel, unsigned long long srcSizeHint, size_t dictSize) {
        ...
        row = ZSTD_MAX_CLEVEL;
    {
        ...
        return ZSTD_adjustCParams_internal(cp,
                                           srcSizeHint,
