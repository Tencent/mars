...
#define ZSTD_DUBT_UNSORTED_MARK                                                                               \
    1 /* For btlazy2 strategy, index ZSTD_DUBT_UNSORTED_MARK==1 means "unsorted".                             \
         ...
         The benefit is that ZSTD_DUBT_UNSORTED_MARK cannot be mishandled after table re-use with a different \
         strategy. This constant is required by ZSTD_compressBlock_btlazy2() and ZSTD_reduceTable_internal() */
...
struct ZSTD_matchState_t {
    ...
    U32 loadedDictEnd;    /* index of end of dictionary, within context's referential.
                           ...
                           * This relies on a mechanism to set loadedDictEnd=0 when dictionary is no longer within
                           * distance.    Such mechanism is provided within ZSTD_window_enforceMaxDist() and
                           * ZSTD_checkDictValidity().    When dict referential is copied into active context (i.e. not
                           * attached),    loadedDictEnd == dictSize, since referential starts from zero.
                           */
