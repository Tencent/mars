...
const char* mode;
{
    ...
    oflag =
        ...
        (cloexec ? O_CLOEXEC : 0) |
#endif
        (state.state->mode == GZ_READ ? O_RDONLY
                                      : (O_WRONLY | O_CREAT |
#ifdef O_EXCL
                                         (exclusive ? O_EXCL : 0) |
#endif
...
int ZEXPORT gzrewind(file) gzFile file;
{
    ...
    if (state.state->mode != GZ_READ || (state.state->err != Z_OK && state.state->err != Z_BUF_ERROR))
        return -1;
...
int whence;
{
    ...
    if (state.state->mode == GZ_READ && state.state->how == COPY && state.state->x.pos + offset >= 0) {
    ...
    if (state.state->mode == GZ_READ) {
        n = GT_OFF(state.state->x.have) || (z_off64_t)state.state->x.have > offset ? (unsigned)offset
                                                                                   : state.state->x.have;
...
int* errnum;
{
    ...
    return state.state->err == Z_MEM_ERROR ? "out of memory" : (state.state->msg == NULL ? "" : state.state->msg);
}
...
const char* msg;
{
    ...
    if ((state.state->msg = (char*)malloc(strlen(state.state->path) + strlen(msg) + 3)) == NULL) {
        state.state->err = Z_MEM_ERROR;
        return;
    }
#if !defined(NO_snprintf) && !defined(NO_vsnprintf)
    (void)
        snprintf(state.state->msg, strlen(state.state->path) + strlen(msg) + 3, "%s%s%s", state.state->path, ": ", msg);
#else
