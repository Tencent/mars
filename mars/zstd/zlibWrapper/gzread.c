...
local int gz_avail(state) gz_statep state;
{
    ...
    if (state.state->eof == 0) {
        ...
        if (gz_load(state, state.state->in + strm->avail_in, state.state->size - strm->avail_in, &got) == -1)
            return -1;
        strm->avail_in += got;
    ...
    do {
        switch (state.state->how) {
            case LOOK: /* -> LOOK, COPY (only if never GZIP), or GZIP */
                ...
                break;
            case COPY: /* -> COPY */
                if (gz_load(state, state.state->out, state.state->size << 1, &(state.state->x.have)) == -1)
                    return -1;
                state.state->x.next = state.state->out;
                return 0;
...
z_off64_t len;
{
    ...
    while (len)
        ...
        if (state.state->x.have) {
            n = GT_OFF(state.state->x.have) || (z_off64_t)state.state->x.have > len ? (unsigned)len
                                                                                    : state.state->x.have;
