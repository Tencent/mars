...
static int ZWRAP_initializeCStream(ZWRAP_CCtx* zwc,
                                   const void* dict,
                                   size_t dictSize,
                                   unsigned long long pledgedSrcSize) {
...
ZEXTERN int ZEXPORT z_deflateInit2_ OF((z_streamp strm,
                                        int level,
                                        int method,
                                        int windowBits,
                                        int memLevel,
...
ZEXTERN int ZEXPORT z_deflate OF((z_streamp strm, int flush)) {
    ...
    if (zwc->zbc == NULL) {
        ...
        {
            int const initErr =
                ZWRAP_initializeCStream(zwc, NULL, 0, (flush == Z_FINISH) ? strm->avail_in : ZSTD_CONTENTSIZE_UNKNOWN);
            if (initErr != Z_OK)
        ...
        if (zwc->totalInBytes == 0) {
            if (zwc->comprState == ZWRAP_useReset) {
                size_t const resetErr =
                    ZSTD_resetCStream(zwc->zbc, (flush == Z_FINISH) ? strm->avail_in : zwc->pledgedSrcSize);
...
#if ZLIB_VERNUM >= 0x1240
ZEXTERN int ZEXPORT z_inflateReset2 OF((z_streamp strm, int windowBits)) {
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN int ZEXPORT z_inflateSetDictionary OF((z_streamp strm, const Bytef* dictionary, uInt dictLength)) {
...
ZEXTERN int ZEXPORT z_inflate OF((z_streamp strm, int flush)) {
    ...
    zwd->outBuffer.pos = 0;
    {
        ...
        LOG_WRAPPERD("inflate inBuffer.pos=%d inBuffer.size=%d outBuffer.pos=%d outBuffer.size=%d o\n",
                     (int)zwd->inBuffer.pos,
                     (int)zwd->inBuffer.size,
                     (int)zwd->outBuffer.pos,
                     (int)zwd->outBuffer.size);
...
ZEXTERN int ZEXPORT z_deflatePending OF((z_streamp strm, unsigned* pending, int* bits)) {
    if (!g_ZWRAP_useZSTDcompression)
...
ZEXTERN int ZEXPORT z_inflateGetDictionary OF((z_streamp strm, Bytef* dictionary, uInt* dictLength)) {
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN int ZEXPORT z_inflateCopy OF((z_streamp dest, z_streamp source)) {
...
ZEXTERN int ZEXPORT z_inflatePrime OF((z_streamp strm, int bits, int value)) {
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
        return inflatePrime(strm, bits, value);
    return ZWRAPD_finishWithErrorMsg(strm, "inflatePrime is not supported!");
}

ZEXTERN int ZEXPORT z_inflateGetHeader OF((z_streamp strm, gz_headerp head)) {
    if (g_ZWRAPdecompressionType == ZWRAP_FORCE_ZLIB || !strm->reserved)
...
ZEXTERN uLong ZEXPORT z_zlibCompileFlags OF((void)) {
...
ZEXTERN int ZEXPORT z_compress OF((Bytef * dest, uLongf* destLen, const Bytef* source, uLong sourceLen)) {
...

    {
        ...
        size_t const cSize = ZSTD_compress(dest, dstCapacity, source, sourceLen, ZWRAP_DEFAULT_CLEVEL);
...
ZEXTERN int ZEXPORT z_uncompress OF((Bytef * dest, uLongf* destLen, const Bytef* source, uLong sourceLen)) {
