...
static int g_displayLevel = DEFAULT_DISPLAY_LEVEL; /* 0 : no display;   1: errors;   2 : + result + interaction +
                                                      warnings;   3 : + progression;   4 : + information */
...
static int BMK_benchMem(z_const void* srcBuffer,
                        size_t srcSize,
    ...
    if (strlen(displayName) > 17)
        displayName += strlen(displayName) - 17; /* can only display 17 characters */
        ...
        while (!cCompleted | !dCompleted) {
            ...
            if (!cCompleted) { /* still some time to do compression tests */
                ...
                if (compressor == BMK_ZSTD) {
                    ...
                    ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(dictBuffer,
                                                                        dictBufferSize,
                                                                        ZSTD_dlm_byRef,
                                                                        ZSTD_dct_auto,
                                                                        zparams.cParams,
                                                                        cmem);
                    if (cdict == NULL)
                        EXM_THROW(1, "ZSTD_createCDict_advanced() allocation failure");
                        ...
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
                            if (dictBufferSize) {
                                rSize = ZSTD_compress_usingCDict(ctx,
                                                                 blockTable[blockNb].cPtr,
                                                                 blockTable[blockNb].cRoom,
                                                                 blockTable[blockNb].srcPtr,
                            ...
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_compress_usingCDict() failed : %s", ZSTD_getErrorName(rSize));
                    ...
                    size_t rSize;
                    if (zbc == NULL)
                        EXM_THROW(1, "ZSTD_createCStream() allocation failure");
                    rSize = ZSTD_initCStream_advanced(zbc, dictBuffer, dictBufferSize, zparams, avgSize);
                    if (ZSTD_isError(rSize))
                        EXM_THROW(1, "ZSTD_initCStream_advanced() failed : %s", ZSTD_getErrorName(rSize));
                    do {
                        U32 blockNb;
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
                            rSize = ZSTD_resetCStream(zbc, blockTable[blockNb].srcSize);
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_resetCStream() failed : %s", ZSTD_getErrorName(rSize));
                            ...
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_compressStream() failed : %s", ZSTD_getErrorName(rSize));
                            rSize = ZSTD_endStream(zbc, &outBuffer);
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_endStream() failed : %s", ZSTD_getErrorName(rSize));
                    ...
                    if (compressor == BMK_ZLIB_REUSE || compressor == BMK_ZWRAP_ZLIB_REUSE)
                    ...
                    if (ret != Z_OK)
                        EXM_THROW(1, "deflateInit failure");
                    ...
                    do {
                        ...
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
                            if (ZWRAP_isUsingZSTDcompression())
                                ret =
                                    ZWRAP_deflateReset_keepDict(&def); /* reuse dictionary to make compression faster */
                            else
                                ret = deflateReset(&def);
                            if (ret != Z_OK)
                                EXM_THROW(1, "deflateReset failure");
                            if (useSetDict) {
                                ret = deflateSetDictionary(&def, (const z_Bytef*)dictBuffer, dictBufferSize);
                                if (ret != Z_OK)
                                    EXM_THROW(1, "deflateSetDictionary failure");
                                if (ZWRAP_isUsingZSTDcompression())
                                    useSetDict = 0; /* zstd doesn't require deflateSetDictionary after
                            ...
                            if (ret != Z_STREAM_END)
                                EXM_THROW(1,
                                          "deflate failure ret=%d srcSize=%d",
                                          ret,
                    ...
                    ret = deflateEnd(&def);
                    if (ret != Z_OK)
                        EXM_THROW(1, "deflateEnd failure");
                } else {
                    z_stream def;
                    if (compressor == BMK_ZLIB || compressor == BMK_ZWRAP_ZLIB)
                        ZWRAP_useZSTDcompression(0);
                    else
                        ZWRAP_useZSTDcompression(1);
                    do {
                ...
                }
                {
                    ...
                    if (clockSpan < fastestC * nbLoops)
                        fastestC = clockSpan / nbLoops;
            ...
            if (!dCompleted) {
                ...
                if (compressor == BMK_ZSTD) {
                    ...
                    if (!ddict)
                        EXM_THROW(2, "ZSTD_createDDict() allocation failure");
                    do {
                        unsigned blockNb;
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
                    ...
                    size_t rSize;
                    if (zbd == NULL)
                        EXM_THROW(1, "ZSTD_createDStream() allocation failure");
                    rSize = ZSTD_initDStream_usingDict(zbd, dictBuffer, dictBufferSize);
                    if (ZSTD_isError(rSize))
                        EXM_THROW(1, "ZSTD_initDStream() failed : %s", ZSTD_getErrorName(rSize));
                    do {
                        U32 blockNb;
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
                            rSize = ZSTD_resetDStream(zbd);
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_resetDStream() failed : %s", ZSTD_getErrorName(rSize));
                            ...
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_decompressStream() failed : %s", ZSTD_getErrorName(rSize));
                    ...
                    ret = inflateInit(&inf);
                    if (ret != Z_OK)
                        EXM_THROW(1, "inflateInit failure");
                    do {
                        U32 blockNb;
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
                            if (ZWRAP_isUsingZSTDdecompression(&inf))
                                ret = ZWRAP_inflateReset_keepDict(
                                    &inf); /* reuse dictionary to make decompression faster; inflate will return
                                              Z_NEED_DICT only for the first time */
                            else
                                ret = inflateReset(&inf);
                            if (ret != Z_OK)
                                EXM_THROW(1, "inflateReset failure");
                            ...
                            if (ret == Z_NEED_DICT) {
                                ...
                                if (ret != Z_OK)
                                    EXM_THROW(1, "inflateSetDictionary failure");
                                ret = inflate(&inf, Z_FINISH);
                            }
                            if (ret != Z_STREAM_END)
                                EXM_THROW(1, "inflate failure");
                    ...
                    ret = inflateEnd(&inf);
                    if (ret != Z_OK)
                        EXM_THROW(1, "inflateEnd failure");
                } else {
                    z_stream inf;
                    if (compressor == BMK_ZLIB)
                        ZWRAP_setDecompressionType(ZWRAP_FORCE_ZLIB);
                    else
                        ZWRAP_setDecompressionType(ZWRAP_AUTO);
                    do {
                ...
                {
                    U64 const clockSpan = UTIL_clockSpanMicro(clockStart);
                    if (clockSpan < fastestD * nbLoops)
                        fastestD = clockSpan / nbLoops;
                    ...
                    for (u = 0; u < srcSize; u++) {
                        if (((const BYTE*)srcBuffer)[u] != ((const BYTE*)resultBuffer)[u]) {
                            ...
                            for (segNb = 0; segNb < nbBlocks; segNb++) {
                                if (bacc + blockTable[segNb].srcSize > u)
                                    break;
...
static void BMK_benchCLevel(void* srcBuffer,
                            size_t benchedSize,
                            const char* displayName,
                            int cLevel,
                            int cLevelLast,
    ...
    for (l = cLevel; l <= cLevelLast; l++) {
        BMK_benchMem(srcBuffer, benchedSize, displayName, l, fileSizes, nbFiles, dictBuffer, dictBufferSize, BMK_ZSTD);
    }
    ...
    if (cLevelLast > Z_BEST_COMPRESSION)
        cLevelLast = Z_BEST_COMPRESSION;
    ...
    for (l = cLevel; l <= cLevelLast; l++) {
        BMK_benchMem(srcBuffer, benchedSize, displayName, l, fileSizes, nbFiles, dictBuffer, dictBufferSize, BMK_ZLIB);
    }
...
static void BMK_loadFiles(void* buffer,
                          size_t bufferSize,
                          size_t* fileSizes,
    ...
    for (n = 0; n < nbFiles; n++) {
        ...
        if (f == NULL)
            EXM_THROW(10, "impossible to open file %s", fileNamesTable[n]);
    ...
    if (totalSize == 0)
        EXM_THROW(12, "no data to bench");
}

static void BMK_benchFileTable(const char** fileNamesTable,
                               unsigned nbFiles,
    ...
    if (!fileSizes)
        EXM_THROW(12, "not enough memory for fileSizes");
    ...
    if (!srcBuffer)
        EXM_THROW(12, "not enough memory");
...
int main(int argCount, char** argv) {
    int argNb, main_pause = 0, nextEntryIsDictionary = 0, operationResult = 0, nextArgumentIsFile = 0;
                    ...
                    switch (argument[0]) {
                        ...
                        case 'B':
                            argument++;
                            {
                                ...
                                if (toupper(*argument) == 'K')
                                    bSize <<= 10, argument++; /* allows using KB notation */
                                if (toupper(*argument) == 'M')
                                    bSize <<= 20, argument++;
                                if (toupper(*argument) == 'B')
                                    argument++;
                                BMK_setNotificationLevel(g_displayLevel);
