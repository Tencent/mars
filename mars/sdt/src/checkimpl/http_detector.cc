...
: req_(_req), worker_thread_(boost::bind(&HTTPDetector::__Run, this), XLOGGER_TAG "::HTTPDetector"), callback_(NULL) {
}
...
void HTTPDetector::__Detect() {
    ...
    RequestLine::THttpMethod method =
        (0 == strncasecmp(req_.http_method_.c_str(), "POST", 4) ? RequestLine::kPost : RequestLine::kGet);
    ...
    if (req_.prior_ip_.empty()) {
        ...
        if (!is_host_ipaddr) {
            ...
            bool ret = dns_.GetHostByName(host,
                                          vec_ip,
            ...
            if (ret && !vec_ip.empty()) {
                ...
                if (result_.dns_cost_ >= remain_timeout) {
                    result_.dns_errmsg_.append(std::string("GetHostByName timeout, set timeout:")
                                               + string_cast(remain_timeout).str() + "ms");
                } else {
            ...
            if (remain_timeout <= 0) {
                xwarn2(TSF "@%_ HTTPDetect timeout, dns_cost:%_, total_timeout:%_",
                       this,
                       result_.dns_cost_,
                       req_.total_timeout_);
    ...
    if (INVALID_SOCKET == sock) {
        ...
        xerror2(TSF "tcp connect error, isbreak:%_, errno:%_(%_)",
                breaker_.IsBreak(),
                result_.tcp_connect_errno_,
                result_.tcp_connect_errmsg_);
        return;
    }

    if (remain_timeout <= 0) {
        xwarn2(TSF "@%_ HTTPDetect timeout, tcp_connect_cost_:%_, total_timeout:%_",
               this,
               result_.tcp_connect_cost_,
               req_.total_timeout_);
    ...
    int send_ret = block_socket_send(sock,
                                     (const unsigned char*)req_buffer.Ptr(),
                                     (unsigned int)req_buffer.Length(),
                                     breaker_,
                                     err_code);

    if (send_ret < 0) {
        xerror2(TSF "Send Request Error, ret:%_, errno:%_, nread:%_, nwrite:%_",
                send_ret,
                strerror(err_code),
                socket_nread(sock),
                socket_nwrite(sock));
    ...
    if (remain_timeout <= 0) {
        xwarn2(TSF "@%_ HTTPDetect timeout, http_send_req_cost_:%_, total_timeout:%_",
               this,
               result_.http_send_req_cost_,
               req_.total_timeout_);
    ...
    while (true) {
        ...
        if (remain_timeout <= 0) {
            xwarn2(TSF "@%_ HTTPDetect timeout, span:%_, total_timeout:%_",
                   this,
                   (int64_t)detect_tick.gettickspan(),
                   req_.total_timeout_);
        ...
        if (parse_status == http::Parser::kFirstLineError) {
            ...
            xerror2(TSF "parse http head failed, but socket closed, length:%_, nread:%_, nwrite:%_ ",
                    recv_buf.Length(),
                    socket_nread(sock),
                    socket_nwrite(sock));
