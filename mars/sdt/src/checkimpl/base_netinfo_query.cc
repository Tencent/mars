...
BaseNetInfo::BaseNetInfo() : nettype_(kNoNet), ip_stack_(ELocalIPStack_None) {
}
...
void BaseNetInfo::__QueryNetType() {
    ...
    switch (nettype_) {
        ...
        case kMobile: {
            ...
            xmsg(TSF "mobile, ispname:%_, ispcode:%_, ran:%_",
                 info.isp_name,
                 info.isp_code,
...
void BaseNetInfo::__QueryNetConfig() {
    ...
    if (getifaddrs_ipv4_filter(v4_addrs, 0)) {
        for (size_t i = 0; i < v4_addrs.size(); ++i) {
            xmsg3(TSF "interface name:%_, %_(%_), ip:%_",
                  v4_addrs[i].ifa_name,
                  (v4_addrs[i].ifa_family == AF_INET ? "AF_INET" : "XX_INET"),
                  v4_addrs[i].ifa_family,
                  v4_addrs[i].ip);
        }
    } else {
