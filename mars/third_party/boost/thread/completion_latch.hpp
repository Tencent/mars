...
#include <third_party/boost/thread/detail/counter.hpp>
...
#include <third_party/boost/chrono/duration.hpp>
#include <third_party/boost/chrono/time_point.hpp>
#include <third_party/boost/assert.hpp>
//#include <third_party/boost/thread/detail/nullary_function.hpp>
#include <third_party/boost/thread/csbl/functional.hpp>
...
namespace mars_boost {} namespace boost = mars_boost; namespace mars_boost
{
  ...
  private:
    ...
    struct around_wait
    {
      ...
      mars_boost::unique_lock<boost::mutex> &lk_;
      around_wait(completion_latch &that, mars_boost::unique_lock<boost::mutex> &lk)
      : that_(that), lk_(lk)
      {
  ...
  public:
    ...
    void wait()
    {
      mars_boost::unique_lock<boost::mutex> lk(mutex_);
    ...
    {
      mars_boost::lock_guard<boost::mutex> lk(mutex_);
      //BOOST_ASSERT(count_ == 0);
      count_ = count;
    }
