...
#include <third_party/boost/preprocessor/seq/transform.hpp>
#include <third_party/boost/preprocessor/seq/for_each_i.hpp>
#include <third_party/boost/preprocessor/seq/cat.hpp>

#include <third_party/boost/typeof/encode_decode.hpp>
#include <third_party/boost/typeof/int_encoding.hpp>

#include <third_party/boost/typeof/type_template_param.hpp>
#include <third_party/boost/typeof/integral_template_param.hpp>
#include <third_party/boost/typeof/template_template_param.hpp>
...
#define BOOST_TYPEOF_REGISTER_TEMPLATE_IMPL(Name, Params, Size, ID)\
    ...
    {\
        typedef typename mars_boost::type_of::push_back<V, mars_boost::mpl::size_t<ID> >::type V0;\
        BOOST_PP_SEQ_FOR_EACH_I(BOOST_TYPEOF_REGISTER_TEMPLATE_ENCODE_PARAM, ~, Params)\
        typedef BOOST_PP_CAT(V, Size) type;\
    };\
    template<class Iter>\
    struct decode_type_impl<mars_boost::mpl::size_t<ID>, Iter>\
    {\
        typedef decode_type_impl<mars_boost::mpl::size_t<ID>, Iter> self_t;\
        typedef mars_boost::mpl::size_t<ID> self_id;\
        typedef Iter iter0;\
