...
#define XLOGGER_STATIC_ASSERT(...)                                                \
    typedef ::xlogger_static_assert_test<sizeof(                                  \
        ::XLOGGER_STATIC_ASSERTION_FAILURE<((__VA_ARGS__) == 0 ? false : true)>)> \
...
#endif
    XMessage&
    operator()(const char* _format, ...);

#ifdef __GNUC__
    __attribute__((__format__(printf, 2, 0)))
#endif
    XMessage&
    VPrintf(const char* _format, va_list _list);
...
#define XLOGGER_TYPESAFE_FORMAT_IMPLEMENT(n, m)                                                                \
    inline XMessage& XMessage::operator()(const TypeSafeFormat&, const char* _format XLOGGER_FORMAT_ARGS(n)) { \
        if (_format != NULL) {                                                                                 \
            const string_cast* args[16] = {XLOGGER_VARIANT_ARGS(n) PP_COMMA_IF(PP_AND(n, m))                   \
...
#endif
__inline void
__xlogger_c_write(const XLoggerInfo* _info, const char* _log, ...) {
...
#define __xlogger_c_impl_if(level, exp, ...) \
    xlogger2_if(exp, level, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, __VA_ARGS__)
...
#define xassert2(exp, ...)                                                                                            \
    if (((exp) || !xlogger_IsEnabledFor(kLevelFatal)))                                                                \
        ;                                                                                                             \
    else {                                                                                                            \
        XLoggerInfo info = {kLevelFatal, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, {0, 0}, -1, -1, -1, false}; \
        gettimeofday(&info.m_tv, NULL);                                                                               \
...
#define __xlogger_cpp_impl_if(level, exp, ...) \
    xlogger2_if(exp, level, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, __VA_ARGS__)
#define __xlogger_cpp_impl_trace(level, ...) \
    xlogger_trace(level, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, __VA_ARGS__)
...
#define xgroup2(...) \
    XLogger(kLevelAll, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, false, XLOGGER_HOOK)(__VA_ARGS__)
#define xgroup2_if(exp, ...) \
    if ((!(exp)))            \
        ;                    \
    else                     \
        XLogger(kLevelAll, XLOGGER_TAG, __XFILE__, __XFUNCTION__, __LINE__, false, XLOGGER_HOOK)(__VA_ARGS__)
...
#define xmessage2_define(name, ...) \
    XMessage name;                  \
    name XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(__VA_ARGS__), (__VA_ARGS__), PP_NUM_PARAMS(__VA_ARGS__))
#define xmessage2(...) \
    XMessage() XLOGGER_ROUTER_OUTPUT(.WriteNoFormat(__VA_ARGS__), (__VA_ARGS__), PP_NUM_PARAMS(__VA_ARGS__))
...
#define __xscope_impl(level, name, ...)  \
    XScopeTracer __ANONYMOUS_VARIABLE__( \
        _tracer_)(level, XLOGGER_TAG, name, __XFILE__, __XFUNCTION__, __LINE__, XLOGGER_SCOPE_MESSAGE(__VA_ARGS__))
...
#define __xfunction_scope_impl(level, name, ...) \
    XScopeTracer __ANONYMOUS_VARIABLE__(         \
        _xfunction_)(level, XLOGGER_TAG, name, __XFILE__, __XFUNCTION__, __LINE__, XLOGGER_SCOPE_MESSAGE(__VA_ARGS__))
