...
struct MessageTitle_t {
    ...
    template<typename T> MessageTitle_t(const T& _title): title((uintptr_t)_title) { static_assert(sizeof(T) <= sizeof(uintptr_t));}
...
class AsyncResult <const R&> {
  ...
  public:
    ...
    AsyncResult(const T& _func, const void* _place_holder = NULL)
        : wrapper_(new AsyncResultWrapper()) {
#if __cplusplus >= 201103L
        static_assert(std::is_same<typename std::result_of<T()>::type, const R&>::value);
#endif
        wrapper_->invoke_function = _func;
    }
