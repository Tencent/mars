...
#define FATAL_ASSERT2(e, fmt, ...) \
    ((e) ? (void)0 : __FATAL_ASSERT2(__FILE__, __LINE__, __func__, #e, fmt, ##__VA_ARGS__))
...
__attribute__((__nonnull__(1, 3, 4, 5))) __attribute__((__format__(printf, 5, 6))) __inline void
...
#pragma pack(pop)
    ...
    int offset =
        snprintf(extra_info.data, sizeof(extra_info.data), "%s, %s, %d:[%s],", _file, _func, _line, _expression);
