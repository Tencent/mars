...
- (void)Run:(NSTimer*)timer {
    ...
    for (std::list<TimerRecord>::iterator it = gs_lst_timer_record.begin();
         it != gs_lst_timer_record.end(); ++it) {
        if (timer == it->timer) {
            if (nil != it->timer) {
                [it->timer invalidate];
                [it->timer release];
...
bool StartAlarm(long long _id, int after) {
    ...
    tr.timer = [NSTimer timerWithTimeInterval:interval
                                       target:tr.timer_runer
