...
struct TaskInfo {
 public:
    TaskInfo(boost::intrusive_ptr<Wrapper> _wrapper, ::SocketPoll& _poll) : wrapper_(_wrapper), poll_(_poll) {
    }
...
class Multiplexing {
 public:
    Multiplexing() : poll_(breaker_, true) {
    }
...
RunloopCond::~RunloopCond() {
    delete multiplexing_;
}
