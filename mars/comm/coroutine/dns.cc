    ...
    if (NULL == dnsfunc) {
        ...
        if (error != 0) {
            ...
            if (_result.empty()) {
                ...
                for (std::vector<socket_address>::iterator iter = dnssvraddrs.begin(); iter != dnssvraddrs.end();
                     ++iter) {
...
bool DNS::GetHostByName(const std::string& _host_name,
    ...
    lock.lock();
    dnsinfo_vec_.erase(std::remove_if(dnsinfo_vec_.begin(),
                                      dnsinfo_vec_.end(),
                                      [](const dnsinfo& _v) {
