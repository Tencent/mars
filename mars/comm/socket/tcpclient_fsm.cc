...
void TcpClientFSM::Close(bool _notify) {
    ...
    if (_notify)
        _OnClose(last_status_, 0, remote_close_);
}
...
void TcpClientFSM::AfterSelect(SocketSelect& _sel, XLogger& _log) {
    if (EConnecting == status_)
        AfterConnectSelect(_sel, _log);
    else if (EReadWrite == status_)
        AfterReadWriteSelect(_sel, _log);
...
void TcpClientFSM::PreConnectSelect(SocketSelect& _sel, XLogger& _log) {
    ...
    xinfo2(TSF "addr:(%_:%_), is_bind_cellular_network:%_, ",
           addr_.ip(),
           addr_.port(),
           addr_.is_bind_cellular_network())
        >> _log;
    ...
    if (addr_.is_bind_cellular_network()) {
        ...
        if (!ret) {
            ...
            status_ = EEnd;
            _OnClose(last_status_, error_, false);
            xerror2(TSF "bind socket to cellular network fail.");
            return;
        } else {
            xinfo2(TSF "bind socket to cellular network success.");
        }
    }
    ...
    if (0 != ret && !IS_NOBLOCK_CONNECT_ERRNO(socket_errno)) {
        ...
        error_ = socket_errno;
        xwarn2(TSF "close connect err:(%_, %_), localip:%_",
               error_,
               socket_strerror(error_),
               socket_address::getsockname(sock_).ip())
            >> _log;
    } else {
        xinfo2("connect") >> _log;
        _sel.Write_FD_SET(sock_);
...
void TcpClientFSM::AfterConnectSelect(const SocketSelect& _sel, XLogger& _log) {
    ...
    if (0 >= timeout) {
        end_connecttime_ = gettickcount();
        xinfo2(TSF "sock:%_, (%_:%_), ", sock_, addr_.ip(), addr_.port()) >> _log;
        xwarn2(TSF "close connect timeout:(%_, %_), (%_, %_)",
               ConnectAbsTimeout(),
               -timeout,
               SOCKET_ERRNO(ETIMEDOUT),
               socket_strerror(SOCKET_ERRNO(ETIMEDOUT)))
            >> _log;
...
void TcpClientFSM::AfterReadWriteSelect(const SocketSelect& _sel, XLogger& _log) {
    ...
    if (_sel.Write_FD_ISSET(sock_)) {
        ...
        if (0 < ret) {
            send_buf_.Move(-ret);
            xinfo2_if(0 == send_buf_.Length(), TSF "all buffer send:%_, m_send_buf:%_", ret, send_buf_.Length())
                >> _log;
            _OnSend(send_buf_, ret);
        } else if (IS_NOBLOCK_SEND_ERRNO(socket_errno)) {
            xwarn2(TSF "buffer full wait for next select, send err:(%_, %_, %_)",
                   ret,
                   socket_errno,
            ...
            status_ = EEnd;
            xinfo2(TSF "sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
            xwarn2(TSF "close send err:(%_, %_, %_), localip:%_",
                   ret,
                   error_,
                   socket_strerror(error_),
                   socket_address::getsockname(sock_).ip())
    ...
    if (_sel.Read_FD_ISSET(sock_)) {
        ...
        ssize_t ret =
            recv(sock_, ((char*)recv_buf_.Ptr() + recv_buf_.Length()), recv_buf_.Capacity() - recv_buf_.Length(), 0);

        if (0 < ret) {
            xinfo2_if(0 == recv_buf_.Length(), TSF "first buffer recv:%_, m_recv_buf:%_", ret, recv_buf_.Length())
                >> _log;
            ...
            xwarn2(TSF "buffer empty wait for next select, recv err:(%_, %_, %_)",
                   ret,
                   socket_errno,
                   socket_strerror(socket_errno))
                >> _log;
            ...
            status_ = EEnd;
            xinfo2(TSF "sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
            xwarn2(TSF "close recv %_:(%_, %_, %_), localip:%_",
                   "err",
                   ret,
                   error_,
                   socket_strerror(error_),
    ...
    if (!_sel.Write_FD_ISSET(sock_) && !_sel.Read_FD_ISSET(sock_) && 0 >= timeout) {
        xinfo2(TSF "sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
        xwarn2(TSF "close readwrite timeout:(%_, %_), (%_, %_)",
               ReadWriteAbsTimeout(),
               -timeout,
               SOCKET_ERRNO(ETIMEDOUT),
               socket_strerror(SOCKET_ERRNO(ETIMEDOUT)))
...
int TcpClientFSM::ConnectTimeout() const {
    return INT_MAX;
}
int TcpClientFSM::ReadWriteTimeout() const {
    return INT_MAX;
}
int TcpClientFSM::ConnectAbsTimeout() const {
    return INT_MAX;
}
int TcpClientFSM::ReadWriteAbsTimeout() const {
    return INT_MAX;
}
int TcpClientFSM::Rtt() const {
    return int(end_connecttime_ - start_connecttime_);
}
