...
char* tcpinfo2str(struct tcp_info* _info, char* _info_str_buf, size_t _buf_len) {
    ...
    ss << "tcpi_state=0x"                  <<  ((uint32_t)_info->tcpi_state)
    << "， tcpi_ca_state=0x"                << ((uint32_t)_info->tcpi_ca_state)
    << "， tcpi_retransmits=0x"             << ((uint32_t)_info->tcpi_retransmits)
    << "， tcpi_probes=0x"                  << ((uint32_t)_info->tcpi_probes)
    << "， tcpi_backoff=0x"                 << ((uint32_t)_info->tcpi_backoff)
    << "， tcpi_options=0x"                 << ((uint32_t)_info->tcpi_options)
    << "， tcpi_rto=0x"                     << (_info->tcpi_rto)
    << "， tcpi_snd_mss=0x"                 << (_info->tcpi_snd_mss)
    << "， tcpi_rcv_mss=0x"                 << (_info->tcpi_rcv_mss)
    << "， tcpi_unacked=0x"                 << (_info->tcpi_unacked)
    << "， tcpi_sacked=0x"                  << (_info->tcpi_sacked)
    << "， tcpi_lost=0x"                    << (_info->tcpi_lost)
    << "， tcpi_retrans=0x"                 << (_info->tcpi_retrans)
    << "， tcpi_fackets=0x"                 << (_info->tcpi_fackets)
    << "， tcpi_last_data_sent=0x"          << (_info->tcpi_last_data_sent)
    << "， tcpi_last_ack_sent=0x"           << (_info->tcpi_last_ack_sent)
    << "， tcpi_last_data_recv=0x"          << (_info->tcpi_last_data_recv)
    << "， tcpi_last_ack_recv=0x"           << (_info->tcpi_last_ack_recv)
    << "， tcpi_pmtu=0x"                    << (_info->tcpi_pmtu)
    << "， tcpi_rcv_ssthresh=0x"            << (_info->tcpi_rcv_ssthresh)
    << "， tcpi_rtt=0x"                     << (_info->tcpi_rtt)
    << "， tcpi_rttvar=0x"                  << (_info->tcpi_rttvar)
    << "， tcpi_snd_ssthresh=0x"            << (_info->tcpi_snd_ssthresh)
    << "， tcpi_snd_cwnd=0x"                << (_info->tcpi_snd_cwnd)
    << "， tcpi_advmss=0x"                  << (_info->tcpi_advmss)
    << "， tcpi_reordering=0x"              << (_info->tcpi_reordering)
    << "， tcpi_rcv_rtt=0x"                 << (_info->tcpi_rcv_rtt)
    << "， tcpi_rcv_space=0x"               << (_info->tcpi_rcv_space)
    << "， tcpi_total_retrans=0x"           << (_info->tcpi_total_retrans)
    << " .";
#endif
