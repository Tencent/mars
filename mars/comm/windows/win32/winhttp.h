...
BOOLAPI
WinHttpTimeFromSystemTime(__in CONST SYSTEMTIME* pst,                                // input GMT time
                          __out_bcount(WINHTTP_TIME_FORMAT_BUFSIZE) LPWSTR pwszTime  // output string buffer
);

BOOLAPI
WinHttpTimeToSystemTime(__in LPCWSTR pwszTime,  // NULL terminated string
...
BOOLAPI
WinHttpCrackUrl(__in_ecount(dwUrlLength) LPCWSTR pwszUrl,
                __in DWORD dwUrlLength,
                __in DWORD dwFlags,
                __inout LPURL_COMPONENTS lpUrlComponents);

BOOLAPI
WinHttpCreateUrl(__in LPURL_COMPONENTS lpUrlComponents,
                 __in DWORD dwFlags,
...
#define ICU_ESCAPE_AUTHORITY \
    0x00002000  // causes InternetCreateUrlA to escape chars in authority components (user, pwd, host)
...
BOOLAPI
WinHttpReadData(IN HINTERNET hRequest,
                IN __out_data_source(NETWORK) LPVOID lpBuffer,
                IN DWORD dwNumberOfBytesToRead,
                OUT LPDWORD lpdwNumberOfBytesRead);

BOOLAPI
WinHttpWriteData(IN HINTERNET hRequest,
                 IN LPCVOID lpBuffer,
                 IN DWORD dwNumberOfBytesToWrite,
                 OUT LPDWORD lpdwNumberOfBytesWritten);

BOOLAPI
WinHttpQueryDataAvailable(IN HINTERNET hRequest,
                          OUT __out_data_source(NETWORK) LPDWORD lpdwNumberOfBytesAvailable OPTIONAL);

BOOLAPI
WinHttpQueryOption(IN HINTERNET hInternet,
                   IN DWORD dwOption,
...
typedef VOID(CALLBACK* WINHTTP_STATUS_CALLBACK)(IN HINTERNET hInternet,
...
WINAPI
WinHttpSetStatusCallback(IN HINTERNET hInternet,
...
BOOLAPI
WinHttpAddRequestHeaders(IN HINTERNET hRequest, IN LPCWSTR pwszHeaders, IN DWORD dwHeadersLength, IN DWORD dwModifiers);
...
BOOLAPI
WinHttpSendRequest(IN HINTERNET hRequest,
...
BOOLAPI
WinHttpQueryHeaders(IN HINTERNET hRequest,
...
BOOLAPI
WinHttpDetectAutoProxyConfigUrl(__in DWORD dwAutoDetectFlags, __out LPWSTR* ppwszAutoConfigUrl);

BOOLAPI
WinHttpGetProxyForUrl(IN HINTERNET hSession,
