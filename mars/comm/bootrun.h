...
__inline int __boot_run_atexit(void (*func)(void)) {
    return atexit(func);
}
...
#define BOOT_RUN_STARTUP(func) \
    VARIABLE_IS_NOT_USED static int __anonymous_run_variable_startup_##func = __boot_run_atstartup(func)
#define BOOT_RUN_EXIT(func) \
    VARIABLE_IS_NOT_USED static int __anonymous_run_variable_exit_##func = __boot_run_atexit(func)

#define BOOT_RUN_BLOCK_START _BOOT_RUN_BLOCK_START(__LINE__)
#define BOOT_RUN_BLOCK_END \
    if (0)                 \
        ;                  \
    else                   \
        return 0;          \
