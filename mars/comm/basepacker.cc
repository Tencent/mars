...
class Endian<T, 1> {
 public:
    inline static T hton(T t) {
        return t;
    }
    inline static T ntoh(T t) {
        return t;
    }
};
...
template <class T>
inline T hton(T t) {
    return Endian<T>::hton(t);
}

template <class T>
inline T ntoh(T t) {
    return Endian<T>::ntoh(t);
}
...
inline int SimpleUnpack(const void* _rawbuf, size_t _rawlen, size_t& _packlen, AutoBuffer& _data) {
    if (sizeof(T) > _rawlen)
        return SIMPLE_CONTINUE;

    _packlen = ntoh(*(T*)_rawbuf);

    if (_packlen > _rawlen)
        return SIMPLE_CONTINUE_DATA;
...
void Packer_Pack(const char* _url,
...
int Packer_Unpack(const void* _rawbuf,
                  size_t _rawlen,
                  std::string& _url,
                  unsigned int& _sequence,
                  size_t& _packlen,
                  AutoBuffer& _data) {
    if (_rawlen < sizeof(LongLinkPack))
        return LONGLINKPACK_CONTINUE;
    ...
    if (((st.head_length + st.url_length + st.total_length) & 0xFF) != st.magic)
        return __LINE__;

    if (st.url_length + st.head_length > st.total_length)
        return __LINE__;

    if (1024 * 1024 < st.total_length)
        return __LINE__;

    if (st.url_length + st.head_length > _rawlen)
        return LONGLINKPACK_CONTINUE_HEAD;

    _url.assign((const char*)_rawbuf + st.head_length, st.url_length);
    _sequence = st.sequence;
    _packlen = st.total_length;

    if (st.total_length > _rawlen)
        return LONGLINKPACK_CONTINUE_data;
