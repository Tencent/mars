...
DWORD SetFilePointer(_In_ HANDLE hFile,
...
HANDLE
CreateFileW(_In_ LPCWSTR lpFileName,
            _In_ DWORD dwDesiredAccess,
            _In_ DWORD dwShareMode,
...
HANDLE
WINAPI
CreateFileA(_In_ LPCSTR lpFileName,
            _In_ DWORD dwDesiredAccess,
            _In_ DWORD dwShareMode,
            _In_opt_ LPSECURITY_ATTRIBUTES lpSecurityAttributes,
            _In_ DWORD dwCreationDisposition,
...
HANDLE
CreateFileMappingA(_In_ HANDLE hFile,
                   _In_opt_ LPSECURITY_ATTRIBUTES lpFileMappingAttributes,
                   _In_ DWORD flProtect,
                   _In_ DWORD dwMaximumSizeHigh,
                   _In_ DWORD dwMaximumSizeLow,
...
_In_opt_ LPCWSTR lpName
    ...
    HANDLE hTemp =
        ::CreateFileMapping(hFile, lpFileMappingAttributes, flProtect, dwMaximumSizeHigh, dwMaximumSizeLow, strName);
    delete[] strName;
    return hTemp;
}
...
LPVOID
WINAPI
MapViewOfFileEx(_In_ HANDLE hFileMappingObject,
                _In_ DWORD dwDesiredAccess,
...
bool interruptible_wait(detail::win32::handle handle_to_wait_for, detail::timeout target_time) {
    ...
    do {
        ...
        if (handle_count) {
            unsigned long const notified_index = ::WaitForMultipleObjectsEx(
                handle_count,
                (const HANDLE*)handles,
                false,
                time_left.milliseconds,
                false);  //((DWORD)handle_count,(const HANDLE*)handles,false,(DWORD)time_left.milliseconds);
