...
class path {
 ...
 public:
    ...
    path path::filename() const {
        ...
        return (m_pathname.size() && pos && is_separator(m_pathname[pos]) && !is_root_separator(m_pathname, pos))
                   ? path(".")
    ...
    path path::extension() const {
        path name(filename());
        if (strcmp(name.string().c_str(), ".") == 0 || strcmp(name.string().c_str(), "..") == 0)
            return path();
    ...
    path path::parent_path() const {
        ...
        return end_pos == string_type::npos ? path() : path(m_pathname.substr(0, end_pos));
    }
 ...
 private:
    ...
    string_type::size_type path::m_parent_path_end() const {
        ...
        bool filename_was_separator(m_pathname.size() && is_separator(m_pathname[end_pos]));

        // skip separators unless root directory
        size_type root_dir_pos(root_directory_start(m_pathname, end_pos));
        for (; end_pos > 0 && (end_pos - 1) != root_dir_pos && is_separator(m_pathname[end_pos - 1]); --end_pos) {
        }
    ...
    // return npos if no root_directory found
    {
        ...
        // case "\\?\"
        if (size > 4 && is_separator(path[0]) && is_separator(path[1]) && path[2] == questionmark
            && is_separator(path[3])) {
        ...
        // case "//net {/}"
        if (size > 3 && is_separator(path[0]) && is_separator(path[1]) && !is_separator(path[2])) {
    ...
    string_type::size_type path::m_append_separator_if_needed() {
        if (!m_pathname.empty() && *(m_pathname.end() - 1) != colon && !is_separator(*(m_pathname.end() - 1))) {
