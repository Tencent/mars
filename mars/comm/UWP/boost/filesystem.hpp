...
class file_status {
 public:
    ...
    explicit file_status(file_type v) BOOST_NOEXCEPT : m_value(v), m_perms(perms_not_known) {
    }
    file_status(file_type v, perms prms) BOOST_NOEXCEPT : m_value(v), m_perms(prms) {
    }
    ...
    file_status(const file_status& rhs) BOOST_NOEXCEPT : m_value(rhs.m_value), m_perms(rhs.m_perms) {
    }
    ...
    // observers
    file_type type() const BOOST_NOEXCEPT {
        return m_value;
    }
    perms permissions() const BOOST_NOEXCEPT {
        return m_perms;
    }

    // modifiers
    void type(file_type v) BOOST_NOEXCEPT {
        m_value = v;
    }
...
inline perms make_permissions(const path& p, DWORD attr) {
...
inline file_status status(const path& p) {
    ...
    return (attr & FILE_ATTRIBUTE_DIRECTORY) ? file_status(directory_file, make_permissions(p, attr))
...
inline mars_boost::uintmax_t file_size(const path& p) {
    std::wstring ws = p.wstring();
    WIN32_FILE_ATTRIBUTE_DATA fad;
    BOOL ret = GetFileAttributesExW(ws.c_str(), GetFileExInfoStandard, &fad);
    if (ret != TRUE) {
    ...
    return (static_cast<mars_boost::uintmax_t>(fad.nFileSizeHigh) << (sizeof(fad.nFileSizeLow) * 8)) + fad.nFileSizeLow;
}

inline std::time_t to_time_t(const FILETIME& ft) {
...
inline std::time_t last_write_time(const path& p) {
    ...
    handle_wrapper hw(CreateFile2(ws.c_str(),
                                  GENERIC_READ,
...
inline bool create_directories(const path& p) {
    ...
    if ((filename.native().size() == 1 && filename.native()[0] == '.')
        || (filename.native().size() == 2 && filename.native()[0] == '.' && filename.native()[1] == '.'))
...
inline void dir_itr_close(void*& handle) {
    if (handle != 0) {
        FindClose(handle);
        handle = 0;
    }
...
class directory_entry {
 public:
    ...
    directory_entry(const mars_boost::filesystem::path& p, file_status st, file_status symlink_st = file_status())
    ...
    const mars_boost::filesystem::path& path() const {
        return m_path;
    }
    operator const mars_boost::filesystem::path&() const {
        return m_path;
    }
    ...
    bool operator==(const directory_entry& rhs) const {
        return m_path.string().compare(rhs.path().string()) == 0;
    }
    bool operator!=(const directory_entry& rhs) const {
        return m_path.string().compare(rhs.path().string()) != 0;
    }
 ...
 private:
    ...
    bool dir_itr_first(void*& handle, const path& dir, std::string& target, file_status& sf, file_status& symlink_sf) {
        ...
        dirpath += (dirpath.empty()
                    || (dirpath[dirpath.size() - 1] != L'\\' && dirpath[dirpath.size() - 1] != L'/'
    ...
    void directory_iterator_construct(directory_iterator& it, const path& p) {
        ...
        else                   // not eof
        {
            ...
            if (filename[0] == '.'  // dot or dot-dot
                && (filename.size() == 1 || (filename[1] == '.' && filename.size() == 2))) {
