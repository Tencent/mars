...
struct slot_tracker {
    ...
    virtual void disconnect(char group) = 0;
};
...
public:
	trackable():this_(0) {}
	~trackable() { disconnect_all_tracks(); }
...
public:
	...
	virtual void disconnect(char group) {
        iterator_type it = slot_list_->list.iterator();
        while (it.has_next()) {
            slot_function& s = it.next();
            if (s.holder_.group == group) {
                s.deleted_ = true;
                has_deleted_slot_ = true;
                if (s.trackable_) {
                    const trackable* t = static_cast<const trackable*>(s.trackable_);
    ...
    void disconnect_all() {
        ...
        while (it.has_next()) {
            ...
            if (slot.trackable_) {
                const trackable* t = static_cast<const trackable*>(slot.trackable_);
                t->remove_tracker(this);
            }
        }
    }
