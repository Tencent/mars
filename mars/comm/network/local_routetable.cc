...
struct bits {
    uint32_t b_mask;
    char b_val;
} bits[] = {{RTF_UP, 'U'},        {RTF_GATEWAY, 'G'},
            {RTF_HOST, 'H'},      {RTF_REJECT, 'R'},
            {RTF_DYNAMIC, 'D'},   {RTF_MODIFIED, 'M'},
            {RTF_MULTICAST, 'm'}, {RTF_DONE, 'd'}, /* Completed -- for routing messages only */
            {RTF_CLONING, 'C'},   {RTF_XRESOLVE, 'X'},
            {RTF_LLINFO, 'L'},    {RTF_STATIC, 'S'},
            {RTF_PROTO1, '1'},    {RTF_PROTO2, '2'},
            {RTF_WASCLONED, 'W'}, {RTF_PRCLONING, 'c'},
            {RTF_PROTO3, '3'},    {RTF_BLACKHOLE, 'B'},
            {RTF_BROADCAST, 'b'}, {RTF_IFSCOPE, 'I'},
            {RTF_IFREF, 'i'},     {RTF_PROXY, 'Y'},
            {RTF_ROUTER, 'r'},    {0}};
...
std::string netname(uint32_t in, uint32_t mask) {
    ...
    switch (dmask) {
        ...
        case IN_CLASSC_NET:
            ...
            /* FALLTHROUGH */
        default:
            snprintf(line, sizeof(line), "%u.%u.%u.%u", C(i >> 24), C(i >> 16), C(i >> 8), C(i));
            break;
    }
...
#define WID_DST(af) ((af) == AF_INET6 ? (true ? 39 : (true ? 39 : 18)) : 18)
#define WID_GW(af) ((af) == AF_INET6 ? (true ? 31 : (true ? 31 : 18)) : 18)
#define WID_IF(af) ((af) == AF_INET6 ? 8 : 7)

/*
 * Print header for routing table columns.
 */
std::string pr_rthdr(int af) {
    char line[256];
    ...
    if (af == AF_INET)
        snprintf(line,
                 sizeof(line),
                 "%-*.*s %-*.*s %-10.10s %6.6s %8.8s %6.6s %*.*s %6s\n",
                 WID_DST(af),
                 WID_DST(af),
                 "Destination",
                 WID_GW(af),
...
static std::string np_rtentry(struct rt_msghdr2* rtm) {
    ...
    result += p_sockaddr(&addr.u_sa, &mask.u_sa, rtm->rtm_flags, WID_DST(addr.u_sa.sa_family));

    result += p_sockaddr(rti_info[RTAX_GATEWAY], NULL, RTF_HOST, WID_GW(addr.u_sa.sa_family));
    ...
    snprintf(line, sizeof(line), "%*.*s", WID_IF(addr.u_sa.sa_family), WID_IF(addr.u_sa.sa_family), ifname);
    result += line;
    ...
    switch (sa->sa_family) {
        case AF_INET: {
            ...
            if ((sin->sin_addr.s_addr == INADDR_ANY) && mask
                ...
                result = netname(sin->sin_addr.s_addr, ntohl(((struct sockaddr_in*)mask)->sin_addr.s_addr));
            else
        ...
        case AF_INET6: {
             ...
             */
            if (IN6_IS_ADDR_LINKLOCAL(in6) || IN6_IS_ADDR_MC_NODELOCAL(in6) || IN6_IS_ADDR_MC_LINKLOCAL(in6)) {
                /* XXX: override is ok? */
