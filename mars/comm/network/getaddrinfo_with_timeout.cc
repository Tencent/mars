...
enum { kRetCodeInternalStateError = -888, kRetCodeParamNotMatch, kRetCodeDnsItemNotFound, kRetCodeGetADDRTimeout };
struct DnsItem {
    thread_tid threadid;
    // parameter
    const char* node;
    ...
    DnsItem()
    : threadid(0), node(NULL), service(NULL), hints(NULL), res(NULL), error_code(0), status(kGetADDRNotBegin) {
    }

    bool EqualParameter(const DnsItem& _item) {
        return _item.node == node && _item.service == service && _item.hints == hints && _item.res == res;
    }
...
static void __WorkerFunc() {
    ...
    // xinfo2(TSF "sys getaddrinfo start");
    uint64_t start = gettickcount();
    int error = getaddrinfo(worker_node.c_str(),
                            worker_service.empty() ? NULL : worker_service.c_str(),
                            &worker_hints,
                            &worker_res0);

    xinfo2(TSF "sys getaddrinfo error:%_, node:%_, service:%_ cost:%_",
           error,
           worker_node,
           worker_service,
           (gettickcount() - start));
